...
function releaseName(planqEnv: string, context: string) {
  const contextK8sFriendly = context.toLowerCase().replace(/_/g, '-')
  return `${planqEnv}--${contextK8sFriendly}--odissigner`
}

export async function installODISHelmChart(planqEnv: string, context: string) {
  return installGenericHelmChart({
    namespace: planqEnv,
    releaseName: releaseName(planqEnv, context),
    chartDir: helmChartPath,
    parameters: await helmParameters(planqEnv, context),
  })
}

export async function upgradeODISChart(planqEnv: string, context: string) {
...
export async function removeHelmRelease(planqEnv: string, context: string) {
  await removeGenericHelmChart(releaseName(planqEnv, context), planqEnv)
...
async function helmParameters(planqEnv: string, context: string) {
