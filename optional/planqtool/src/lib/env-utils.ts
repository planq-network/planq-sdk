...
export interface PlanqEnvArgv extends yargs.Argv {
  planqEnv: string
}
...
export enum envVar {
  ...
  PLQCLI_STANDALONE_IMAGE_REPOSITORY = 'PLQCLI_STANDALONE_IMAGE_REPOSITORY',
  PLQCLI_STANDALONE_IMAGE_TAG = 'PLQCLI_STANDALONE_IMAGE_TAG',
  PLQSTATS_BANNED_ADDRESSES = 'PLQSTATS_BANNED_ADDRESSES',
  PLQSTATS_FRONTEND_DOCKER_IMAGE_REPOSITORY = 'PLQSTATS_FRONTEND_DOCKER_IMAGE_REPOSITORY',
  PLQSTATS_FRONTEND_DOCKER_IMAGE_TAG = 'PLQSTATS_FRONTEND_DOCKER_IMAGE_TAG',
  PLQSTATS_RESERVED_ADDRESSES = 'PLQSTATS_RESERVED_ADDRESSES',
  PLQSTATS_SERVER_DOCKER_IMAGE_REPOSITORY = 'PLQSTATS_SERVER_DOCKER_IMAGE_REPOSITORY',
  PLQSTATS_SERVER_DOCKER_IMAGE_TAG = 'PLQSTATS_SERVER_DOCKER_IMAGE_TAG',
  PLQSTATS_TRUSTED_ADDRESSES = 'PLQSTATS_TRUSTED_ADDRESSES',
  PLQTOOL_PLQENV = 'PLQTOOL_PLQENV',
  PLQTOOL_CONFIRMED = 'PLQTOOL_CONFIRMED',
  PLQTOOL_DOCKER_IMAGE_REPOSITORY = 'PLQTOOL_DOCKER_IMAGE_REPOSITORY',
  PLQTOOL_DOCKER_IMAGE_TAG = 'PLQTOOL_DOCKER_IMAGE_TAG',
  ...
  E2E_TESTS_FORCE_USE_MYPLQ = 'E2E_TESTS_FORCE_USE_MYPLQ',
  ...
  GETH_USE_MYPLQ = 'GETH_USE_MYPLQ',
  GETH_MYPLQ_COMMIT = 'GETH_MYPLQ_COMMIT',
...
export enum DynamicEnvVar {
  ...
  KOMENCI_PLQLABS_REWARDS_ADDRESS_AZURE_KEY_VAULTS = '{{ context }}_KOMENCI_PLQLABS_REWARDS_ADDRESS_AZURE_KEY_VAULTS',
...
export function validateAndSwitchToEnv(planqEnv: string) {
  if (!isValidPlanqEnv(planqEnv)) {
    console.error(
      `${planqEnv} does not conform to specification ^[a-z][a-z0-9]*$. We need to it to conform to that regex because it is used as URL components, Kubernetes namespace names, keys in configuration objects, etc.`
    )
    process.exit(1)
  }

  const envResult = config({ path: getEnvFile(planqEnv) })
  const envMemonicResult = config({ path: getEnvFile(planqEnv, '.mnemonic') })
  ...
  process.env.PLQTOOL_PLQENV = planqEnv
}
...
export function isValidPlanqEnv(planqEnv: string) {
  return new RegExp('^[a-z][a-z0-9]*$').test(planqEnv)
}
...
function planqEnvMiddleware(argv: PlanqEnvArgv) {
  validateAndSwitchToEnv(argv.planqEnv)
}

export async function doCheckOrPromptIfStagingOrProduction() {
  if (process.env.PLQTOOL_CONFIRMED !== 'true' && isProduction()) {
    await confirmAction(
      `You are about to apply a possibly irreversible action on a production env: ${process.env.PLQTOOL_PLQENV}. Are you sure?`
    )
    process.env.PLQTOOL_CONFIRMED = 'true'
  }
}
...
export function addPlanqEnvMiddleware(argv: yargs.Argv) {
  return (
    argv
      .option('planq-env', {
        demand: 'Please specify a valid PLQ_ENV',
        alias: 'e',
        required: true,
        description: 'the environment in which you want to execute this command',
      })
      // @ts-ignore Since we pass it right above, we know that planqEnv will be there at runtime
      .middleware([planqEnvMiddleware])
  )
}
