...
export async function linkSAForWorkloadIdentity(planqEnv: string, context: string) {
  if (
    ...
    await execCmd(
      `gcloud iam service-accounts add-iam-policy-binding --project ${fetchEnv(
        ...
        --member "serviceAccount:${fetchEnv(
          envVar.TESTNET_PROJECT_NAME
        )}.svc.id.goog[${planqEnv}/gcloud-storage-access]" chaindata-download@${fetchEnv(
        envVar.TESTNET_PROJECT_NAME
...
export async function delinkSAForWorkloadIdentity(planqEnv: string, context: string) {
  if (
    getDynamicEnvVarValue(
      DynamicEnvVar.FULL_NODES_USE_GSTORAGE_DATA,
      { context },
...
export async function kubectlAnnotateKSA(planqEnv: string, context: string) {
  if (
    getDynamicEnvVarValue(
      DynamicEnvVar.FULL_NODES_USE_GSTORAGE_DATA,
      { context },
...
export async function removeKubectlAnnotateKSA(planqEnv: string, context: string) {
  if (
    getDynamicEnvVarValue(
      DynamicEnvVar.FULL_NODES_USE_GSTORAGE_DATA,
      { context },
