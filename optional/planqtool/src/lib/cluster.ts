...
export async function switchToClusterFromEnv(
  ...
  if (!skipClusterSetup) {
    if (!isPlanqtoolHelmDryRun()) {
      ...
      await setupCluster(planqEnv, createdCluster)
    } else {
...
export async function createClusterIfNotExists() {
  ...
  if (!clusterExists) {
    const network = networkName(fetchEnv(envVar.PLQTOOL_PLQENV))
...
export async function createNamespaceIfNotExists(namespace: string) {
  ...
  if (!namespaceExists) {
    const cmd = `kubectl create namespace ${namespace} ${
      isPlanqtoolHelmDryRun() ? ' --dry-run=server' : ''
    }`
    await execCmdWithExitOnFailure(cmd)
  }
}

export async function setupCluster(planqEnv: string, createdCluster: boolean) {
  const envType = fetchEnv(envVar.ENV_TYPE)

  await checkHelmVersion()

  await createNamespaceIfNotExists(planqEnv)
  ...
  await createAndUploadBackupSecretIfNotExists(blockchainBackupServiceAccountName, planqEnv)
  ...
  await installCertManagerAndNginx(planqEnv)
  ...
  await setClusterLabels(planqEnv)
}
...
export async function setClusterLabels(planqEnv: string) {
  const envType = fetchEnv(envVar.ENV_TYPE)
  ...
  await labelfn('envinstance', planqEnv)
}
...
export async function switchToClusterFromEnvOrContext(argv: any, skipClusterSetup = false) {
  if (argv.context === undefined) {
    ...
    await switchToClusterFromEnv(argv.planqEnv, true, skipClusterSetup)
  } else {
    await switchToContextCluster(argv.planqEnv, argv.context, skipClusterSetup)
