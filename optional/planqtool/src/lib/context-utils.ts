...
export async function switchToContextCluster(
  ...
  const clusterManager: BaseClusterManager = getClusterManagerForContext(planqEnv, context)
  await clusterManager.switchToClusterContext(skipClusterSetup, context)
  return clusterManager
}

export function getClusterManagerForContext(planqEnv: string, context: string) {
  ...
  return getClusterManager(cloudProvider, planqEnv, clusterConfig)
}
...
export function addOptionalContextMiddleware(argv: Argv) {
  return addPlanqEnvMiddleware(argv).option('context', {
