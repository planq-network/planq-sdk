...
interface LinkArgEnv extends PlanqEnvArgv {
  open: boolean
  explanation: boolean
}

export const builder = (yargs: Argv) => {
  const config = addPlanqEnvMiddleware(yargs)
    .option('open', {
      alias: 'o',
      type: 'boolean',
    ...
    .command('all', 'prints out all links', {}, async (rawArgs: Arguments) => {
      commands.forEach(async (cmd) => {
        ...
        console.info(`$ planqtooljs links ${cmd.command}\n`)
        if (argv.explanation) {
...
const commands = [
  {
    ...
    explanation:
      'This links to the Google Cloud Console that lists all the Kubernetes Workloads running in the specified PLQ_ENV. That currently includes things like our geth nodes, Blockscout, EthStats, CronJobs that check healthiness of the network etc. This is a good first place to check that workloads are running as we expect',
  },
  ...
  },
  {
    ...
    url: (argv: PlanqEnvArgv) => getBlockscoutUrl(argv.planqEnv),
    explanation:
  ...
  },
  {
    ...
    url: (argv: PlanqEnvArgv) => getEthstatsUrl(argv.planqEnv),
    explanation:
  ...
  },
  {
    ...
    url: (argv: PlanqEnvArgv) => getBlockchainApiUrl(argv.planqEnv),
    explanation:
  ...
  },
  {
    ...
    url: (argv: LinkArgEnv) => getEnvFile(argv.planqEnv),
    explanation:
      'The path to the .env file that is used with the current specification of PLQ_ENV',
  },
  {
