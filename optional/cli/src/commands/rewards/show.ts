...
import { GroupVoterReward, VoterReward } from '@planq-network/contractkit/lib/wrappers/Election'
import { AccountSlashed } from '@planq-network/contractkit/lib/wrappers/LockedPlanq'
import { Validator, ValidatorReward } from '@planq-network/contractkit/lib/wrappers/Validators'
  ...
  async run() {
    ...
    for (
      ...
      if (res.flags.slashing) {
        const epochAccountsSlashed = await lockedPlanq.getAccountsSlashed(epochNumber)
    ...
    if (res.flags.slashing) {
      const epochAccountsSlashed = await lockedPlanq.getAccountsSlashed(currentEpoch)
      const address = res.flags.voter || res.flags.validator || res.flags.group
      accountsSlashed = accountsSlashed.concat(
        address ? filterAccountsSlashed(epochAccountsSlashed, address) : epochAccountsSlashed
      )
    ...
    // At the end of each epoch: R, the total amount of rewards in planq to be allocated to stakers
    ...
    //
    // Let T be the total planq voting for groups eligible for rewards in this epoch. For each account
    // holder, for each group, the amount of planq the account holder has voting for that group is increased
    // by average_epoch_score_of_elected_validators_in_group * account_planq_voting_for_group * R * M / T.
    ...
    // stake, Validators are paid Pv * Sv * M * (1 - C) Planq Dollars where
    ...
    // elected as a member of is paid Pv * Sv * C * M Planq Dollars where:
