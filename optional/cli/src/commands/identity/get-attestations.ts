...
export default class GetAttestations extends BaseCommand {
  ...
  static flags = {
    ...
    from: flags.string({
      required: false,
      description:
        "Account whose balance to use for querying ODIS for the pepper lookup",
    }),
    pepper: flags.string({
      required: false,
    ...
    network: flags.string({
      required: false,
      description: "The ODIS service to hit: mainnet, atlas, atlasstaging",
    }),
  };
  ...
  async run() {
    ...
    if (!identifier) {
      ...
      if (!pepper) {
        pepper = await this.getPhoneNumberPepper(
          this.kit,
          phoneNumber!,
          account!,
          network
    ...
    const accounts = await attestationsContract.lookupAccountsForIdentifier(
      identifier
    );
    accounts.forEach(async (accountAddress) => {
      ...
      console.log(
        "\tWallet address: " +
          (await accountsContract.getWalletAddress(accountAddress))
      );
      console.log(
        "\tData-Encryption Key: " +
          (await accountsContract.getDataEncryptionKey(accountAddress))
      );
    });
  }
