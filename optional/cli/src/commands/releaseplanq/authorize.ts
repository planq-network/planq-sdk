...
export default class Authorize extends ReleasePlanqBaseCommand {
  static description =
  ...
  static flags = {
    ...
    signer: Flags.address({
      required: true,
      description: 'The signer key that is to be used for voting through the ReleasePlanq instance',
    }),
  ...
  async run() {
    ...
    const checker = newCheckBuilder(this).isAccount(this.releasePlanqWrapper.address)
    if (flags.role === 'validator' && !flags.force) {
      if (flags.blsKey && flags.blsPop) {
        checker.isValidator(this.releasePlanqWrapper.address)
      } else {
        checker.isNotValidator(this.releasePlanqWrapper.address)
      }
    }
    ...
    const sig = accounts.parseSignatureOfAddress(
      this.releasePlanqWrapper.address,
      flags.signer,
      flags.signature
    )
    ...
    let tx: any
    if (role === 'vote') {
      tx = await this.releasePlanqWrapper.authorizeVoteSigner(flags.signer, sig)
    } else if (role === 'validator' && flags.blsKey && flags.blsPop) {
      tx = await this.releasePlanqWrapper.authorizeValidatorSignerAndBls(
        flags.signer,
        sig,
        flags.blsKey,
        flags.blsPop
      ...
      tx = await this.releasePlanqWrapper.authorizeValidatorSigner(flags.signer, sig)
    } else if (role === 'attestation') {
      tx = await this.releasePlanqWrapper.authorizeAttestationSigner(flags.signer, sig)
    } else {
