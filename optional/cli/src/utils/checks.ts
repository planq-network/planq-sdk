...
import { GovernanceWrapper, ProposalStage } from '@planq-network/contractkit/lib/wrappers/Governance'
import {
  ExchangeProposalState,
  GrandaMentoWrapper,
} from '@planq-network/contractkit/lib/wrappers/GrandaMento'
import { LockedPlanqWrapper } from '@planq-network/contractkit/lib/wrappers/LockedPlanq'
import { MultiSigWrapper } from '@planq-network/contractkit/lib/wrappers/MultiSig'
import { ValidatorsWrapper } from '@planq-network/contractkit/lib/wrappers/Validators'
...
class CheckBuilder {
  ...
  withLockedPlanq<A>(
    f: (
      lockedPlanq: LockedPlanqWrapper,
      signer: Address,
      account: Address,
    ...
    return async () => {
      ...
      if (this.signer) {
        const account = await validators.signerToAccount(this.signer)
        return f(lockedPlanq, this.signer, account, validators) as Resolve<A>
      } else {
        return f(lockedPlanq, '', '', validators) as Resolve<A>
      }
    }
  }
  ...
  signerMeetsValidatorBalanceRequirements = () =>
    this.addCheck(
      `Signer's account has enough locked planq for registration`,
  ...
  signerMeetsValidatorGroupBalanceRequirements = () =>
    this.addCheck(
      `Signer's account has enough locked planq for group registration`,
      this.withValidators((validators, _signer, account) =>
  ...
  meetsValidatorBalanceRequirements = (account: Address) =>
    this.addCheck(
      `${account} has enough locked planq for registration`,
  ...
  meetsValidatorGroupBalanceRequirements = (account: Address) =>
    this.addCheck(
      `${account} has enough locked planq for group registration`,
  ...
  hasEnoughPlanq = (account: Address, value: BigNumber) => {
    const valueInEth = this.kit.connection.web3.utils.fromWei(value.toFixed(), 'ether')
    return this.addCheck(`Account has at least ${valueInEth} PLQ`, () =>
      this.kit.contracts
        .getPlanqToken()
        .then((planqToken) => planqToken.balanceOf(account))
  ...
  hasEnoughStable = (
    account: Address,
    value: BigNumber,
    stable: StableToken = StableToken.pUSD
  ) => {
    const valueInEth = this.kit.connection.web3.utils.fromWei(value.toFixed(), 'ether')
  ...
  hasEnoughLockedPlanq = (value: BigNumber) => {
    const valueInEth = this.kit.connection.web3.utils.fromWei(value.toFixed(), 'ether')
    return this.addCheck(
      `Account has at least ${valueInEth} Locked Planq`,
      this.withLockedPlanq(async (lockedPlanq, _signer, account) =>
        value.isLessThanOrEqualTo(await lockedPlanq.getAccountTotalLockedPlanq(account))
      )
    )
  }

  hasEnoughNonvotingLockedPlanq = (value: BigNumber) => {
    const valueInEth = this.kit.connection.web3.utils.fromWei(value.toFixed(), 'ether')
    return this.addCheck(
      `Account has at least ${valueInEth} non-voting Locked Planq`,
      this.withLockedPlanq(async (lockedPlanq, _signer, account) =>
        value.isLessThanOrEqualTo(await lockedPlanq.getAccountNonvotingLockedPlanq(account))
      )
    )
  }

  hasEnoughLockedPlanqToUnlock = (value: BigNumber) => {
    const valueInEth = this.kit.connection.web3.utils.fromWei(value.toFixed(), 'ether')
    return this.addCheck(
      `Account has at least ${valueInEth} non-voting Locked Planq over requirement`,
      this.withLockedPlanq(async (lockedPlanq, _signer, account, validators) => {
        const requirement = await validators.getAccountLockedPlanqRequirement(account)
        return (
          (requirement.eq(0) ||
            value.plus(requirement).lte(await lockedPlanq.getAccountTotalLockedPlanq(account))) &&
          value.lte(await lockedPlanq.getAccountNonvotingLockedPlanq(account))
        )
      })
    )
  }
  ...
  validatorDeregisterDurationPassed = () => {
    return this.addCheck(
      ...
      this.withValidators(async (validators, _signer, account) => {
        ...
        const { duration } = await validators.getValidatorLockedPlanqRequirements()
  ...
  //     signer: Address,
  //     account: Address
  //   ) => Promise<PlanqTransactionObject<any>> | PlanqTransactionObject<any>
  // ) {
