import {ReadOnlyWallet} from "@planq-network/connect";
import {
  ContractKit,
  newKitFromWeb3,
  StableToken,
...
import {AzureHSMWallet} from "@planq-network/wallet-hsm-azure";
import {
  AddressValidation,
  newLedgerWalletWithSetup,
} from "@planq-network/wallet-ledger";
...
export abstract class BaseCommand extends Command {
  static flags = {
    ...
    node: flags.string({
      ...
      parse: (nodeUrl) => {
        switch (nodeUrl) {
          ...
          case "baklava":
            return "https://evm-atlas.planq.network";
          case "atlas":
            return "https://evm-atlas.planq.network";
          case "mainnet":
          case "forno":
    ...
    ledgerAddresses: flags.integer({
      ...
      description:
        "If --useLedger is set, this will get the first N addresses for local signing",
    }),
    ...
    azureVaultName: flags.string({
      hidden: true,
      description:
        "If --useAKV is set, this is used to connect to the Azure KeyVault",
    }),
  ...
  get web3() {
    if (!this._web3) {
      ...
      const nodeUrl =
        (res.flags && res.flags.node) || getNodeUrl(this.config.configDir);
      this._web3 =
    ...
    if (gasCurrencyConfig === gasOptions.auto && this.kit.defaultAccount) {
      ...
      if (balances.PLQ!.isZero()) {
        ...
        for (const stable of stables) {
          ...
          // has balance
          if (
            (balances as any)[stableName] &&
            !(balances as any)[stableName].isZero()
          ) {
            break;
          }
        }
