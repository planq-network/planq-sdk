...
import {initDatabase as initSignerDatabase} from "@planq-network/phone-number-privacy-signer/dist/common/database/database";
import {
  DefaultKeyName,
  KeyProvider,
} from "@planq-network/phone-number-privacy-signer/dist/common/key-management/key-provider-base";
import {MockKeyProvider} from "@planq-network/phone-number-privacy-signer/dist/common/key-management/mock-key-provider";
import {
  SignerConfig,
  SupportedDatabase,
  SupportedKeystore,
} from "@planq-network/phone-number-privacy-signer/dist/config";
import {startSigner} from "@planq-network/phone-number-privacy-signer/dist/server";
...
const signerConfig: SignerConfig = {
  ...
  quota: {
    ...
    minPlanqBalance: new BigNumber(5e15),
    // Equivalent to 0.001 aUSD/query
    queryPriceInAUSD: new BigNumber(0.001),
  },
  api: {
    domains: {
      enabled: false,
  ...
  blockchain: {
    provider: "https://evm-atlas.planq.network",
    apiKey: undefined,
  },
  db: {
...
const mockOdisPaymentsTotalPaidAUSD = jest.fn<BigNumber, []>();
...
const mockContractKit = createMockContractKit({
  ...
  [ContractRetrieval.getOdisPayments]: createMockOdisPayments(
    mockOdisPaymentsTotalPaidAUSD
  ),
});
...
jest.mock("@planq-network/contractkit", () => ({
  ...jest.requireActual("@planq-network/contractkit"),
...
describe("pnpService", () => {
  ...
  const getSignRequest = (
    ...
    return {
      ...
      blindedQueryPhoneNumber: Buffer.from(_blindedMsgResult.message).toString(
        "base64"
      ),
  ...
  describe("with n=3, t=2", () => {
    ...
    const expectedSignature =
      expectedSignatures[config.phoneNumberPrivacy.keys.currentVersion - 1];
    ...
    describe("when signers are operating correctly", () => {
      ...
      describe(`${CombinerEndpoint.PNP_QUOTA}`, () => {
        const totalQuota = 10;
        const weiToausd = new BigNumber(1e18);
        beforeAll(async () => {
          mockOdisPaymentsTotalPaidAUSD.mockReturnValue(
            weiToausd
              .multipliedBy(totalQuota)
              .multipliedBy(signerConfig.quota.queryPriceInAUSD)
          );
        });
        ...
        queryCountParams.forEach(
          ({signerQueries, expectedQueryCount, expectedWarnings}) => {
        ...
        it("Should respond with a warning when there are slight discrepancies in total quota", async () => {
          mockOdisPaymentsTotalPaidAUSD.mockReturnValueOnce(
            weiToausd
        ...
        it("Should respond with 500 when there are large discrepancies in total quota", async () => {
          mockOdisPaymentsTotalPaidAUSD.mockReturnValueOnce(
            weiToausd.multipliedBy(totalQuota + 15)
          );
          const req = {
            account: ACCOUNT_ADDRESS1,
          };
      ...
      describe(`${CombinerEndpoint.PNP_SIGN}`, () => {
        ...
        beforeEach(async () => {
          mockOdisPaymentsTotalPaidAUSD.mockReturnValue(onChainPaymentsDefault);
        ...
        for (let i = 1; i <= 3; i++) {
          it(`Should respond with 200 on valid request with key version header ${i}`, async () => {
            const authorization = getPnpRequestAuthorization(req, PRIVATE_KEY1);
            const res = await sendPnpSignRequest(
              req,
              authorization,
              app,
              i.toString()
        ...
        it("Should respond with 200 on repeated valid requests", async () => {
          ...
          expect(res2.body).toStrictEqual<SignMessageResponseSuccess>(
            expectedResponse
          );
        });
        ...
        it("Should get the same unblinded signatures from the same message (different seed)", async () => {
          ...
          const req2 = {...req};
          const blindedMsgResult2 = threshold_bls.blind(
            message,
            secondUserSeed
          );
          req2.blindedQueryPhoneNumber = Buffer.from(
            blindedMsgResult2.message
          ).toString("base64");

          // Sanity check
          expect(req2.blindedQueryPhoneNumber).not.toEqual(
        ...
        it("Should respond with 401 on failed DEK auth", async () => {
          ...
          req.authenticationMethod = AuthenticationMethod.ENCRYPTION_KEY;
          const differentPk =
            "0x00000000000000000000000000000000000000000000000000000000ddddbbbb";
        ...
        it("Should respond with 403 on out of quota", async () => {
          mockOdisPaymentsTotalPaidAUSD.mockReturnValue(new BigNumber(0));
          const authorization = getPnpRequestAuthorization(req, PRIVATE_KEY1);
          const res = await sendPnpSignRequest(req, authorization, app);
        ...
        describe("functionality in case of errors", () => {
          it("Should return 401 on failure to fetch DEK", async () => {
            ...
            const combinerConfigWithFailOpenDisabled: typeof combinerConfig =
              JSON.parse(JSON.stringify(combinerConfig));
    ...
    describe("when signers are not operating correctly", () => {
      ...
      describe("when 2/3 signers return correct signatures", () => {
        beforeEach(async () => {
          ...
          const badKeyProvider1 = new MockKeyProvider(
            new Map([
              [`${DefaultKeyName.PHONE_NUMBER_PRIVACY}-1`, badBlsShare1],
            ])
          );
          signer1 = startSigner(
            signerConfig,
      ...
      describe("when 1/3 signers return correct signatures", () => {
        beforeEach(async () => {
          ...
          const badKeyProvider2 = new MockKeyProvider(
            new Map([
              [`${DefaultKeyName.PHONE_NUMBER_PRIVACY}-1`, badBlsShare2],
            ])
          );
      ...
      describe("when 2/3 of signers are disabled", () => {
        beforeEach(async () => {
          const configWithApiDisabled: SignerConfig = JSON.parse(
            JSON.stringify(signerConfig)
          );
      ...
      describe("when signers timeout", () => {
        beforeEach(async () => {
          ...
          const configWithShortTimeout: SignerConfig = JSON.parse(
            JSON.stringify(signerConfig)
          );
  ...
  describe("with n=5, t=4", () => {
    ...
    const combinerConfigLargerN: typeof config = JSON.parse(
      JSON.stringify(combinerConfig)
    );
    combinerConfigLargerN.phoneNumberPrivacy.odisServices.signers =
      JSON.stringify([
        {
