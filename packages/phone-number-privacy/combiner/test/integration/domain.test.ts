...
import {SignerConfig} from "@planq-network/phone-number-privacy-signer/dist/config";
import {
  defined,
  noBool,
  noNumber,
...
describe("domainService", () => {
  ...
  const DEFAULT_PUB_KEY =
    TestUtils.Values.DOMAINS_THRESHOLD_DEV_PUBKEYS[
      config.domains.keys.currentVersion - 1
    ];
  ...
  const quotaRequest = async (): Promise<
    DomainQuotaStatusRequest<SequentialDelayDomain>
  > => {
    ...
    req.options.signature = defined(
      await wallet.signTypedData(
        walletAddress,
        domainQuotaStatusRequestEIP712(req)
      )
    );
    return req;
  };
  ...
  describe("with n=3, t=2", () => {
    ...
    beforeAll(async () => {
      ...
      app = startCombiner(
        combinerConfig,
        getContractKitWithAgent(combinerConfig.blockchain)
      );
    });
    ...
    describe("when signers are operating correctly", () => {
      beforeEach(async () => {
        signer1 = startSigner(signerConfig, signerDB1, keyProvider1).listen(
          3001
        );
        signer2 = startSigner(signerConfig, signerDB2, keyProvider2).listen(
          3002
        );
        signer3 = startSigner(signerConfig, signerDB3, keyProvider3).listen(
          3003
        );
      });
      ...
      describe(`${CombinerEndpoint.DOMAIN_SIGN}`, () => {
        ...
        it("Should respond with 200 on repeated valid requests", async () => {
          ...
          const eval1 = poprfClient.unblindResponse(
          ...
          req1.options.signature = defined(
            await wallet.signTypedData(
              walletAddress,
              domainRestrictedSignatureRequestEIP712(req1)
            )
          );
          const res2 = await request(app)
            .post(CombinerEndpoint.DOMAIN_SIGN)
          ...
          const eval2 = poprfClient.unblindResponse(
            Buffer.from(res1.body.signature, "base64")
          );
        ...
        it("Should respond with 429 when requesting a signature from a disabled domain", async () => {
          ...
          expect(resDisable.body).toStrictEqual<DisableDomainResponse>({
            ...
            status: {
              ...
              counter: 0,
              timer: 0,
              now: resDisable.body.status.now,
            },
          });
        });
