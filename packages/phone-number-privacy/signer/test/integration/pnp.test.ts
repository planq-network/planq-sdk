...
describe('pnp', () => {
  ...
  describe(`${SignerEndpoint.PNP_QUOTA}`, () => {
    describe('quota calculation logic', () => {
      quotaCalculationTestCases.forEach(({ pusdOdisPaymentInWei, expectedTotalQuota }) => {
        it(`Should get totalQuota=${expectedTotalQuota} 
           for pUSD (wei) payment=${pusdOdisPaymentInWei.toString()}`, async () => {
          mockOdisPaymentsTotalPaidCUSD.mockReturnValue(pusdOdisPaymentInWei)
  ...
  describe(`${SignerEndpoint.PNP_SIGN}`, () => {
    describe('quota calculation logic', () => {
      quotaCalculationTestCases.forEach(({ expectedTotalQuota, pusdOdisPaymentInWei }) => {
        it(`Should get totalQuota=${expectedTotalQuota} 
           for pUSD (wei) payment=${pusdOdisPaymentInWei.toString()}`, async () => {
          mockOdisPaymentsTotalPaidCUSD.mockReturnValue(pusdOdisPaymentInWei)
