...
import {BLINDED_PHONE_NUMBER} from "@planq-network/phone-number-privacy-common/lib/test/values";
...
describe("pnp", () => {
  ...
  const expectedSignature =
    expectedSignatures[_config.keystore.keys.phoneNumberPrivacy.latest - 1];
  ...
  beforeEach(async () => {
    ...
    mockOdisPaymentsTotalPaidAUSD.mockReset();
  ...
  describe(`${SignerEndpoint.PNP_QUOTA}`, () => {
    describe("quota calculation logic", () => {
      quotaCalculationTestCases.forEach(
        ({ausdOdisPaymentInWei, expectedTotalQuota}) => {
          it(`Should get totalQuota=${expectedTotalQuota} 
           for aUSD (wei) payment=${ausdOdisPaymentInWei.toString()}`, async () => {
            mockOdisPaymentsTotalPaidAUSD.mockReturnValue(ausdOdisPaymentInWei);
    ...
    describe("endpoint functionality", () => {
      ...
      beforeEach(async () => {
        mockOdisPaymentsTotalPaidAUSD.mockReturnValue(onChainBalance);
      });
      ...
      it("Should respond with 503 on disabled api", async () => {
        const configWithApiDisabled: typeof _config = JSON.parse(
      ...
      describe("functionality in case of errors", () => {
        ...
        it("Should respond with 500 on blockchain totalQuota query failure", async () => {
          mockOdisPaymentsTotalPaidAUSD.mockImplementation(() => {
          ...
          const spy = jest
            ...
            .mockImplementation(async () => {
              await new Promise((resolve) =>
                setTimeout(resolve, testTimeoutMS + delay)
              );
  ...
  describe(`${SignerEndpoint.PNP_SIGN}`, () => {
    describe("quota calculation logic", () => {
      quotaCalculationTestCases.forEach(
        ({expectedTotalQuota, ausdOdisPaymentInWei}) => {
          it(`Should get totalQuota=${expectedTotalQuota} 
           for aUSD (wei) payment=${ausdOdisPaymentInWei.toString()}`, async () => {
            mockOdisPaymentsTotalPaidAUSD.mockReturnValue(ausdOdisPaymentInWei);
    ...
    describe("endpoint functionality", () => {
      ...
      it("Should respond with 200 and warning on repeated valid requests", async () => {
        ...
        expect(res2.status).toBe(200);
        res1.body.warnings.push(
          WarningMessage.DUPLICATE_REQUEST_TO_GET_PARTIAL_SIG
        );
      ...
      it("Should respond with 403 if totalQuota and performedQueryCount are zero", async () => {
        mockOdisPaymentsTotalPaidAUSD.mockReturnValue(zeroBalance);
      ...
      describe("functionality in case of errors", () => {
        ...
        it("Should return 500 on blockchain totalQuota query failure", async () => {
          ...
          const configWithFailOpenDisabled: typeof _config = JSON.parse(
            JSON.stringify(_config)
          );
        ...
        it("Should return 500 on failure to increment query count", async () => {
          ...
          expect(
            await getPerformedQueryCount(db, ACCOUNT_ADDRESS1, logger)
        ...
        it("Should return 500 on bls signing error", async () => {
          const spy = jest
            .spyOn(
              jest.requireActual("blind-threshold-bls"),
        ...
        it("Should return 500 on generic error in sign", async () => {
          const spy = jest
            .spyOn(
              jest.requireActual(
                "../../src/common/bls/bls-cryptography-client"
              ),
