...
export function getBlindedPhoneNumber(
  phoneNumber: string,
  blindingFactor: Buffer
): string {
  const blindedPhoneNumber = threshold_bls.blind(
...
export function getTestParamsForContext(): E2ETestParams {
  switch (process.env.CONTEXT_NAME) {
    ...
    case "atlas":
      return {
        blockchainProviderURL: process.env.ATLAS_ODIS_BLOCKCHAIN_PROVIDER!,
        pnpPolynomial: process.env.ATLAS_PHONE_NUMBER_PRIVACY_POLYNOMIAL!,
        pnpKeyVersion: process.env.ODIS_PNP_TEST_KEY_VERSION!,
        domainsPolynomial: process.env.ATLAS_DOMAINS_POLYNOMIAL!,
        domainsPubKey: process.env.ATLAS_DOMAINS_PUBKEY!,
        domainsKeyVersion: process.env.ODIS_DOMAINS_TEST_KEY_VERSION!,
      };
    case "mainnet":
      return {
    ...
    default:
      throw new Error(
        `No parameter settings stored for context: ${process.env.CONTEXT_NAME}`
      );
  }
}
