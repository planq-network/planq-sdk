...
import {newKit, StableToken} from "@planq-network/contractkit";
import {
...
const {getPnpQuotaRequest, getPnpRequestAuthorization, getPnpSignRequest} =
  TestUtils.Utils;
...
describe(`Running against service deployed at ${ODIS_SIGNER_URL}`, () => {
  const singleQueryCost = config.quota.queryPriceInAUSD.times(1e18).toString();

  beforeAll(async () => {
    const accountsWrapper = await kit.contracts.getAccounts();
    if (
      (await accountsWrapper.getDataEncryptionKey(ACCOUNT_ADDRESS2)) !==
      DEK_PUBLIC_KEY
    ) {
  ...
  describe(`${SignerEndpoint.PNP_SIGN}`, () => {
    describe("success cases", () => {
      ...
      it("[Signer configuration test] Should respond with 200 on valid request", async () => {
        const blindedMessage = getBlindedPhoneNumber(
          PHONE_NUMBER,
          randomBytes(32)
        );
        ...
        expect(res.headers.get(KEY_VERSION_HEADER)).toEqual(
          contextSpecificParams.pnpKeyVersion
        );
        expect(
    ...
    describe("failure cases", () => {
      ...
      it("Should respond with 400 on on invalid key version", async () => {
        ...
        const res = await queryPnpSignEndpoint(
          badRequest,
          authorization,
          "asd"
        );
      ...
      it("Should respond with 403 on out of quota", async () => {
        ...
        const quotaAuthorization = getPnpRequestAuthorization(
          quotaReq,
          PRIVATE_KEY1
        );
        const quotaRes = await queryPnpQuotaEndpoint(
          quotaReq,
          quotaAuthorization
        );
        ...
        // Sanity check
        expect(quotaResBody.performedQueryCount).toEqual(
          quotaResBody.totalQuota
        );
...
async function sendAUSDToOdisPayments(
  amountInWei: string | number,
  recipient: string,
  sender: string
) {
  const stableToken = await kit.contracts.getStableToken(StableToken.aUSD);
  ...
  await odisPayments
    .payInAUSD(recipient, amountInWei)
    .sendAndWaitForReceipt({from: sender});
}
