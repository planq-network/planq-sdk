...
export async function waitForAnnounceToStabilize(web3: Web3) {
  ...
  // Before we used myplanq, this wasn't noticeable because the migrations  meant that the network would have
...
export function getContext(gethConfig: GethRunConfig, verbose: boolean = verboseOutput) {
  // Use of myplanq can be enabled through gethConfig or through an env variable
  const useMyplanq =
    !!gethConfig.useMyplanq ||
    stringToBoolean(fetchEnvOrFallback(envVar.E2E_TESTS_FORCE_USE_MYPLQ, 'false'))
  ...
  const initialize = async () => {
    ...
    if (useMyplanq) {
      // Compile the contracts first because myplanq assumes they are compiled already, unless told not to
      if (!gethConfig.myplanqSkipCompilingContracts) {
    ...
    if (useMyplanq || !(gethConfig.migrate || gethConfig.migrateTo)) {
