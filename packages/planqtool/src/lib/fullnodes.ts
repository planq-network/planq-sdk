...
/**
 * Gets the appropriate cloud platform's full node deployer given the planqEnv
 * and context.
 */
export function getFullNodeDeployerForContext(
  ...
  return getFullNodeDeployer(cloudProvider, planqEnv, deploymentConfig)
}
...
export async function installFullNodeChart(
  ...
  const deployer = getFullNodeDeployerForContext(planqEnv, context, staticNodes, createNEG)
  const enodes = await deployer.installChart(context)
  if (enodes) {
    await uploadStaticNodeEnodes(planqEnv, context, enodes)
  }
}
...
export async function upgradeFullNodeChart(
  ...
  const deployer = getFullNodeDeployerForContext(planqEnv, context, generateNodeKeys, createNEG)
 ...
 * node chart.
 */
export async function removeFullNodeChart(planqEnv: string, context: string) {
  const deployer = getFullNodeDeployerForContext(planqEnv, context, false, false)
  await deployer.removeChart()
  // Remove any previous static nodes
  await uploadStaticNodeEnodes(planqEnv, context, [])
}

function uploadStaticNodeEnodes(planqEnv: string, context: string, enodes: string[]) {
  const suffix = getStaticNodesFileSuffix(context)
  // Use mainnet instead of rc1
  const env = planqEnv === 'rc1' ? 'mainnet' : planqEnv
