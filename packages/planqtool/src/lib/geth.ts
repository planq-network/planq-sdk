...
import { PlanqTxReceipt, TransactionResult } from '@planq-network/connect'
import { PlanqContract, ContractKit, newKitFromWeb3 } from '@planq-network/contractkit'
...
export function getPrivateTxNodeClusterIP(planqEnv: string) {
  return retrieveClusterIPAddress('service', 'tx-nodes-private', planqEnv)
}
...
export const checkGethStarted = (dataDir: string) => {
  if (!fs.existsSync(path.resolve(dataDir, GETH_IPC))) {
    ...
    console.info(
      `Please, make sure you specified correct data directory, you could also run the geth node by "planqtooljs geth run"`
    )
    process.exit(1)
  }
}
...
const transferAndTrace = async (
  ...
  const token = getRandomToken(planqToken, stableToken)
  const feeCurrencyToken = getRandomToken(planqToken, stableToken)
...
export const traceTransactions = async (
  ...
  await transferAndTrace(kit, planqToken, stableToken, addresses[0], addresses[1], '', blockscoutUrl)

  await transferAndTrace(kit, planqToken, stableToken, addresses[1], addresses[0], '', blockscoutUrl)
...
export const transferPlanqPlanq = async (
  kit: ContractKit,
  fromAddress: string,
  toAddress: string,
  amount: BigNumber,
  ...
  const kitPlanqToken = await kit.contracts.getPlanqToken()
  return kitPlanqToken.transfer(toAddress, amount.toString()).send({
    from: fromAddress,
    gas: txOptions.gas,
    gasPrice: txOptions.gasPrice,
    nonce: txOptions.nonce,
...
export const transferPlanqDollars = async (
  kit: ContractKit,
  fromAddress: string,
  toAddress: string,
  amount: BigNumber,
...
export const simulateClient = async (
  ...
  while (true) {
    ...
    let txOptions
    const feeCurrency = await getFeeCurrency(kit, txConf.feeCurrencyPlanq, baseLogMessage)
...
const getTxConf = async (testMode: TestMode) => {
  ...
  const usePlanq = Boolean(Math.round(Math.random()))
  const _transferFn = usePlanq ? transferPlanqPlanq : transferPlanqDollars
  const _tokenName = usePlanq ? 'cGLD' : 'pUSD'

  // randomly choose which gas currency to use
  const _feeCurrencyPlanq = Boolean(Math.round(Math.random()))
  return {
    feeCurrencyPlanq: _feeCurrencyPlanq,
...
const getFeeCurrency = async (kit: ContractKit, feeCurrencyPlanq: boolean, baseLogMessage: any) => {
  try {
    return feeCurrencyPlanq ? '' : await kit.registry.addressFor(PlanqContract.StableToken)
  } catch (error: any) {
    tracerLog({
      tag: LOG_TAG_CONTRACT_ADDRESS_ERROR,
      error: error.toString(),
...
export const transferERC20Token = async (
  ...
  onReceipt?: (receipt: PlanqTxReceipt) => void,
...
export async function checkoutGethRepo(branch: string, gethPath: string) {
  ...
  await spawnCmdWithExitOnFailure('git', [
    ...
    '--depth',
    '1',
    'https://github.com/planq-network/planq.git',
    gethPath,
    '-b',
    branch,
  ])
