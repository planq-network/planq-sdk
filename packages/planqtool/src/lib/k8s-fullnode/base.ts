...
const helmChartPath = 'oci://us-west1-docker.pkg.dev/devopsre/clabs-public-oci/planq-fullnode'
...
export abstract class BaseFullNodeDeployer {
  ...
  constructor(deploymentConfig: BaseFullNodeDeploymentConfig, planqEnv: string) {
  ...
  async upgradeChart(context: string, reset: boolean): Promise<string[] | void> {
    if (isPlanqtoolHelmDryRun()) {
      ...
      if (reset) {
        await scaleResource(this.planqEnv, 'StatefulSet', `${this.planqEnv}-fullnodes`, 0)
        await deletePersistentVolumeClaims(this.planqEnv, ['planq-fullnode'])
      }
  ...
  async helmParameters(context: string) {
    ...
    const customValuesFile = `${helmChartPath}/${this._planqEnv}-${readableContext(
      context
    )}-values.yaml`
    return [
      ...
      `--set genesis.network=${this.planqEnv}`,
  ...
  get releaseName() {
    return `${this.planqEnv}-fullnodes`
  }
