...
export class AksFullNodeDeployer extends BaseFullNodeDeployer {
  ...
  async allocateStaticIPs() {
    console.info(`Creating static IPs on Azure for ${this.planqEnv}`)
    ...
    for (let i = existingStaticIPsCount - 1; i > replicas - 1; i--) {
      await deleteResource(this.planqEnv, 'service', `${this.planqEnv}-fullnodes-${i}`, false)
  ...
  async deallocateAllIPs() {
    console.info(`Deallocating static IPs on Azure for ${this.planqEnv}`)
