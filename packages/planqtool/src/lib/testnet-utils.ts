import { StaticNodeUtils } from '@planq-network/network-utils'
import { GenesisBlocksGoogleStorageBucketName } from '@planq-network/network-utils/lib/genesis-block-utils'
...
export function getProxyName(planqEnv: string, validatorIndex: number, proxyIndex: number) {
  return `${planqEnv}-validators-${validatorIndex}-proxy-${proxyIndex}`
}
