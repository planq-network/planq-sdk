...
import { blsPrivateKeyToProcessedPrivateKey } from '@planq-network/cryptographic-utils/lib/bls'
...
import { MyplanqGenesisConfig } from 'src/lib/interfaces/myplanq-genesis-config'
...
export const generateGenesisFromEnv = (enablePetersburg: boolean = true) => {
  ...
  // Planq hard fork activation blocks.  Default is undefined, which means not activated.
...
export const generateGenesis = ({
  ...
  if (consensusType === ConsensusType.CLIQUE) {
    ...
    genesis.config.istanbul = {
      // see github.com/planq-network/planq/blob/master/consensus/istanbul/config.go#L21-L25
...
// This function assumes that myplanq has already been built using 'make all'
export const generateGenesisWithMigrations = async ({
  ...
  numValidators,
  verbose,
}: MyplanqGenesisConfig): Promise<string> => {
  const tmpDir = path.join(tmpdir(), `myplanq-genesis-${Date.now()}`)
  fs.mkdirSync(tmpDir)
  const envFile = path.join(tmpDir, 'env.json')
  const configFile = path.join(tmpDir, 'genesis-config.json')
  const myplanqBinaryPath = path.join(gethRepoPath!, '/build/bin/myplanq')
