...
import { Address, ContractKit, newKitFromWeb3 } from '@planq-network/contractkit'
import {
  ensureLeading0x,
  eqAddress,
...
interface SimulateVotingArgv {
  planqProvider: string
  excludedGroups?: string[]
}

export const builder = (yargs: Argv) => {
  return yargs
    .option('planqProvider', {
      type: 'string',
...
export const handler = async function simulateVoting(argv: SimulateVotingArgv) {
  try {
    ...
    const kit = newKitFromWeb3(new Web3(argv.planqProvider))
...
async function castVote(
  ...
  const lockedPlanqAmount = await lockedPlanq.getAccountTotalLockedPlanq(botAccount)
  if (lockedPlanqAmount.isZero()) {
    console.info(`No locked planq exists for ${botAccount}, skipping...`)
    return
  }
  ...
  const voteAmount = BigNumber.minimum(lockedPlanqAmount, groupCapacity)
