...
export const handler = async (argv: ListArgv) => {
  const domain = getBlockscoutUrl(argv.planqEnv)
  ...
  console.log(`Getting list of users for "${argv.planqEnv}" environment`)
  const resp = await fetch(listUsersUrl)
  const jsonResp = await resp.json()
  await handleListOfUsers(domain, jsonResp, process.env.PLQTOOL_VERBOSE === 'true')
}
