// This is a more unusual Planqtool command. It basically helps you to execute Hotfixes on testnets. Because constructing proposals is difficult to do via a CLI, you should define them here in code. There are two examples below that you can start from.

import { newKitFromWeb3 } from '@planq-network/contractkit'
import { hotfixToHash, ProposalBuilder, proposalToJSON } from '@planq-network/governance'
...
export const handler = async (argv: EthstatsArgv) => {
  ...
  try {
    const kit = newKitFromWeb3(new Web3(getFornoUrl(argv.planqEnv)))
    ...
    // Example A: Update a var on a Planq Core Contract
    ...
    // Example B: Repoint a Planq Core Contract proxy
    // const validatorsProxyAddress = await kit.registry.addressFor(PlanqContract.Validators)
    ...
    //   contract: PlanqContract.Attestations,
    ...
    // If your proposal is just made of Planq Registry contract methods, you can print it out
