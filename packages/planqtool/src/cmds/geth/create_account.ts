...
import { addPlanqGethMiddleware } from 'src/lib/utils'
...
interface CreateAccountArgv extends PlanqEnvArgv, GethArgv {
  faucet: boolean
  password: string
...
export const builder = (argv: yargs.Argv) => {
  return addPlanqGethMiddleware(addPlanqEnvMiddleware(argv))
    .option('faucet', {
      type: 'boolean',
      alias: 'f',
      default: false,
      description:
        'whether to faucet created account with 100 planq dollars and 10 planq planq or not',
    })
    .option('password', {
      type: 'string',
...
export const handler = async (argv: CreateAccountArgv) => {
  ...
  if (!fs.existsSync(path.resolve(datadir, 'keystore'))) {
    ...
    console.info(`Try to running "planqtooljs geth init"`)
    process.exit(1)
  }
