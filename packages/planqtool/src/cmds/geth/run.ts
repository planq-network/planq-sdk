...
import { addPlanqGethMiddleware, ensure0x, validateAccountAddress } from 'src/lib/utils'
...
export const builder = (argv: yargs.Argv) => {
  return addPlanqGethMiddleware(argv)
    .option('network-id', {
      type: 'string',
...
export const handler = async (argv: RunArgv) => {
  ...
  if (!fs.existsSync(path.resolve(datadir, STATIC_NODES_FILE_NAME))) {
    ...
    console.info(`Try running "planqtooljs geth static-nodes" or "planqtooljs geth init"`)
    process.exit(1)
  }
