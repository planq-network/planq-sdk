...
export default class Withdraw extends BaseCommand {
  ...
  async run() {
    ...
    while (true) {
      ...
      const pendingWithdrawals = await lockedplanq.getPendingWithdrawals(flags.from)
      for (let i = 0; i < pendingWithdrawals.length; i++) {
        ...
        if (pendingWithdrawal.time.isLessThan(currentTime)) {
          ...
          await displaySendTx('withdraw', lockedplanq.withdraw(i))
          madeWithdrawal = true
          break
        }
      }
    ...
    const remainingPendingWithdrawals = await lockedplanq.getPendingWithdrawals(flags.from)
