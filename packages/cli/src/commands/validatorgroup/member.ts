    ...
    if (res.flags.accept) {
      if (!res.flags.yes) {
        const response = await prompts({
          ...
          name: 'confirmation',
          message:
            'Are you sure you want to accept this member?\nValidator Group Locked Planq requirements increase per member. Adding an additional member could result in an increase in Locked Planq requirements of up to 10,000 PLQ for 180 days. (y/n)',
        })
