...
export default class MultiSigTransfer extends BaseCommand {
  ...
  async run() {
    ...
    if (transferFrom) {
      ...
      transferTx = planqToken.transferFrom(sender, to, amountBN)
    } else {
      // @ts-ignore - function will accept BigNumber
      transferTx = planqToken.transfer(to, amountBN)
    }
    const multiSigTx = await multisig.submitOrConfirmTransaction(planqToken.address, transferTx.txo)
