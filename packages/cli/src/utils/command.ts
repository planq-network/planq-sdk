...
import { PlanqContract, RegisteredContracts } from '@planq-network/contractkit'
import { BLS_POP_SIZE, BLS_PUBLIC_KEY_SIZE } from '@planq-network/cryptographic-utils/lib/bls'
import { isE164NumberStrict } from '@planq-network/phone-utils/lib/phoneNumbers'
import { ensureLeading0x, trimLeading0x } from '@planq-network/utils/lib/address'
import { POP_SIZE } from '@planq-network/utils/lib/signatureUtils'
...
const parseCoreContract: ParseFn<string> = (input) => {
  if (RegisteredContracts.includes(input as PlanqContract)) {
    return input
  } else {
...
export const Flags = {
  ...
  contract: flags.build({
    ...
    helpValue: `${PlanqContract.BlockchainParameters}`,
  }),
  contractsArray: flags.build({
    parse: parseArray(parseCoreContract),
    description: 'Array of Registered Core Contracts',
    helpValue: `\'["${PlanqContract.BlockchainParameters}", "${PlanqContract.Governance}", "${PlanqContract.Validators}"]\'`,
  }),
