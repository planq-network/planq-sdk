...
export function codegen(contract: Contract, abi: RawAbiDefinition[]) {
    const template = `
  import { AbiItem, Callback, PlanqTxObject, Contract, EventLog } from '@planq-network/connect'
...
function generateFunction(fn: FunctionDeclaration): string {
    return `
  ${fn.name}(${generateInputTypes(fn.inputs)}): PlanqTxObject<${generateOutputTypes(fn.outputs)}>;
`
}
