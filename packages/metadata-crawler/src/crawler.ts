...
import {
  verifyAccountClaim,
  verifyDomainRecord,
} from '@planq-network/contractkit/lib/identity/claims/verify'
import { normalizeAddressWith0x, trimLeading0x } from '@planq-network/utils/lib/address'
...
async function addDatabaseVerificationClaims(address: string, domain: string, verified: boolean) {
  try {
    const query = `INSERT INTO planq_claims (address, type, element, verified, timestamp, inserted_at, updated_at) VALUES
...
async function processDomainClaims() {
  let items: { address: string; url: string }[] = await jsonQuery(
    `SELECT address, url FROM planq_account WHERE url is NOT NULL `
  )
