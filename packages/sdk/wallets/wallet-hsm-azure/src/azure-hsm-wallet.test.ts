...
import {publicKeyPrefix, Signature} from "@planq-network/wallet-hsm";
...
const AZURE_VAULT_NAME = USING_MOCK
  ? "mockVault"
...
const keyVaultAddresses: Map<string, {address: string; privateKey: string}> =
  new Map([
    [
      "secp",
      {
...
describe("AzureHSMWallet class", () => {
            ...
            signMessage: async (
              ...
              if (keyVaultAddresses.has(keyName)) {
                const trimmedKey = trimLeading0x(
                  keyVaultAddresses.get(keyName)!.privateKey
                );
  ...
  describe("after initializing", () => {
    ...
    test("hasAccount should return false for keys that are not present", async () => {
      ...
      expect(
        await wallet.hasAccount("this is not a valid private key")
      ).toBeFalsy();
    });
          ...
          test("fails calling signTransaction", async () => {
            try {
              ...
              expect(e.message).toBe(
                `Could not find address ${unknownAddress}`
              );
            }
          });
