...
import { normalizeAddressWith0x, privateKeyToAddress } from '@planq-network/utils/lib/address'
import { verifySignature } from '@planq-network/utils/lib/signatureUtils'
import { recoverTransaction, verifyEIP712TypedDataSigner } from '@planq-network/wallet-base'
import net from 'net'
...
describe.skip('rpc-wallet', () => {
  it('should work against local geth ipc', async () => {
    const ipcUrl = '/Users/yorhodes/planq/blockchain/envs/alfajoresstaging/geth.ipc'
      ...
      describe('signing', () => {
        describe('using an unlocked address', () => {
          ...
          describe('when calling signTransaction', () => {
            ...
            test('succeeds', async () => {
              await expect(rpcWallet.signTransaction(planqTransaction)).resolves.not.toBeUndefined()
            })

            // TODO(yorke): enable once fixed: https://github.com/planq-network/planq-sdk/issues/4077
            test.skip('with same signer', async () => {
              const signedTx = await rpcWallet.signTransaction(planqTransaction)
            ...
            test.skip('signature with 0x00 prefix is canonicalized', async () => {
              ...
              const signedTx = await rpcWallet.signTransaction(planqTransactionZeroPrefix)
