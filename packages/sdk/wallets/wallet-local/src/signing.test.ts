...
async function verifyLocalSigning(
  ...
  const [signedPlanqTransaction, recoveredSigner] =
  ...
  expect(recoveredSigner.toLowerCase()).toEqual(
    planqTransaction.from!.toString().toLowerCase()
  );
  ...
  if (planqTransaction.nonce != null) {
    ...
    expect(signedPlanqTransaction.nonce).toEqual(
      parseInt(planqTransaction.nonce.toString(), 16)
    );
  }
  if (planqTransaction.gas != null) {
    ...
    expect(signedPlanqTransaction.gas).toEqual(
      parseInt(planqTransaction.gas.toString(), 16)
    );
  }
  ...
  if (planqTransaction.data != null) {
    debug(
      `Checking data actual ${signedPlanqTransaction.data} expected ${planqTransaction.data}`
    );
    expect(signedPlanqTransaction.data!.toLowerCase()).toEqual(
      planqTransaction.data.toLowerCase()
    );
  }
}
...
describe("Transaction Utils", () => {
  ...
  describe("Signer Testing with single local account and pay gas in PLQ", () => {
    it("Test1 should be able to sign and get the signer back with single local account", async () => {
      ...
      await verifyLocalSigningInAllPermutations(
        web3,
        ACCOUNT_ADDRESS1,
        ACCOUNT_ADDRESS2
      );
