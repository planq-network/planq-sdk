...
import {asn1FromPublicKey} from "@planq-network/wallet-hsm";
...
export const ACCOUNT_ADDRESS_NEVER = normalizeAddressWith0x(
...
const GCP_VERSION_NAME = USING_MOCK
  ? MOCK_VERSION_NAME
  : process.env.GCP_VERSION_NAME!;
...
describe("GcpHsmWallet class", () => {
  ...
  beforeEach(async () => {
    ...
    if (USING_MOCK) {
      jest
        .spyOn<any, any>(wallet, "generateKmsClient")
          ...
          return {
            getPublicKey: async ({name: versionName}: {name: string}) => {
              ...
              const temp = new BigNumber(
                ensureLeading0x(pubKey.toString("hex"))
              );
            ...
            asymmetricSign: async ({
              ...
              if (privateKey) {
                ...
                const signature = ec.sign(digest.sha256, pkBuffer, {
                  canonical: true,
                });
