import { ensureLeading0x, normalizeAddressWith0x, trimLeading0x } from '@planq-network/base/lib/address'
...
describe('LedgerWallet class', () => {
  ...
  describe('without initializing', () => {
    ...
    test('fails calling signTransaction', async () => {
      await expect(wallet.signTransaction(planqTransaction)).rejects.toThrowError()
    })
        ...
        describe('using a known address', () => {
          describe('when calling signTransaction', () => {
            ...
            test(
              'succeeds',
              async () => {
                await expect(wallet.signTransaction(planqTransaction)).resolves.not.toBeUndefined()
              },
              TEST_TIMEOUT_IN_MS
            )
            ...
            test(
              'with same signer',
              async () => {
                const signedTx: EncodedTransaction = await wallet.signTransaction(planqTransaction)
