...
const PLQ_APP_ACCEPTS_CONTRACT_DATA_FROM_VERSION = "1.0.2";
...
export class LedgerSigner implements Signer {
  ...
  async signTransaction(
    ...
    try {
      ...
      if (error instanceof TransportStatusError) {
        ...
        if (
          error.statusCode === 27264 &&
          error.statusText === "INCORRECT_DATA"
        ) {
  ...
  private validationRequired(): boolean {
    switch (this.ledgerAddressValidation) {
      ...
      default: {
        throw new Error(
          "ledger-signer@validationRequired: invalid ledgerValidation value"
        );
      }
    }
  }
  ...
  private async checkForKnownToken(rlpEncoded: RLPEncodedTx) {
    if (
      compareLedgerAppVersions(
        ...
        PLQ_APP_ACCEPTS_CONTRACT_DATA_FROM_VERSION
      ) >= 0
    ) {
