import { Future } from '@planq-network/base/lib/future'
import debugFactory from 'debug'
import { PlanqTxReceipt, PromiEvent } from '../types'
...
export class TransactionResult {
  ...
  private receiptFuture = new Future<PlanqTxReceipt>()
  ...
  constructor(pe: PromiEvent<any>) {
    pe.on('transactionHash', (hash: string) => {
      ...
      .on('receipt', (receipt: PlanqTxReceipt) => {
        debug('receipt: %O', receipt)
        this.receiptFuture.resolve(receipt)
      })

      .on('error', ((error: any, receipt: PlanqTxReceipt | false) => {
        if (!receipt) {
