import {Lock} from "@planq-network/base/lib/lock";
...
export function assertIsPlanqProvider(
  provider: any
): asserts provider is PlanqProvider {
  if (!(provider instanceof PlanqProvider)) {
    throw new Error(
      "A different Provider was manually added to the kit. The kit should have a PlanqProvider"
    );
  }
}

/*
 * PlanqProvider wraps a web3.js provider for use with Planq
 */
export class PlanqProvider implements Provider {
  ...
  isLocalAccount(address?: string): boolean {
    return (
      ...
      this.connection.wallet.hasAccount(address)
    );
  }
  ...
  send(payload: JsonRpcPayload, callback: Callback<JsonRpcResponse>): void {
    ...
    switch (payload.method) {
      ...
      case InterceptedMethods.personalSign: {
        ...
        address =
          payload.method === InterceptedMethods.sign
            ? payload.params[0]
            : payload.params[1];
        ...
        if (this.connection.isLocalAccount(address)) {
          rpcCallHandler(
            payload,
            this.handleSignPersonalMessage.bind(this),
            decoratedCallback
          );
        } else {
          this.forwardSend(payload, callback);
  ...
  private async handleSignPersonalMessage(
    ...
    const ecSignatureHex = this.connection.wallet!.signPersonalMessage(
      address,
      data
    );
  ...
  private async handleSignTransaction(
    ...
    const filledParams = await this.connection.paramsPopulator.populate(
      txParams
    );
    debugTxToSend("%O", filledParams);
    const signedTx = await this.connection.wallet!.signTransaction(
      filledParams
    );
  ...
  private async handleSendTransaction(payload: JsonRpcPayload): Promise<any> {
    ...
    try {
      ...
      const response = await this.connection.rpcCaller.call(
  ...
  private addProviderDelegatedFunctions(): void {
    if (
      hasProperty<{on: (type: string, callback: () => void) => void}>(
    ...
    if (
      ...
      }>(this.existingProvider, "removeListener")
    ) {
      // @ts-ignore
    ...
    if (
      hasProperty<{
        removeAllListener: (type: string, callback: () => void) => void;
      }>(this.existingProvider, "removeAllListener")
    ) {
      // @ts-ignore
