...
import {PlanqProvider} from "./planq-provider";
...
class MockWallet implements ReadOnlyWallet {
  ...
  signTransaction(_txParams: PlanqTx): Promise<EncodedTransaction> {
...
describe("PlanqProvider", () => {
  let mockCallback: any;
  let mockProvider: Provider;
  let planqProvider: PlanqProvider;
  const interceptedByPlanqProvider = [
    "eth_sendTransaction",
    "eth_signTransaction",
    "eth_sign",
    "personal_sign",
  ...
  beforeEach(() => {
    mockCallback = jest.fn(
      (payload: JsonRpcPayload, callback: Callback<JsonRpcResponse>): any => {
    ...
    planqProvider = connection.web3.currentProvider as any as PlanqProvider;
  });

  describe("when planq provider don't have any local account", () => {
    interceptedByPlanqProvider.forEach((method: string) => {
      test(`fowards the call to '${method}' to the original provider`, (done) => {
        ...
        planqProvider.send(payload, callback);
      });
    });
  });

  describe("when planq provider has a local account", () => {
    ...
    beforeEach(() => {
      planqProvider.addAccount(ACCOUNT_ADDRESS1);
    });
    ...
    describe("using that account", () => {
      test("call 'send' with 'eth_sendTransaction' signs and send a eth_sendRawTransaction to the original provider", (done) => {
        ...
        const callback: Callback<JsonRpcResponse> = (
          ...
          expect(mockCallback.mock.calls.length).toBe(1);
          expect(mockCallback.mock.calls[0][0].method).toBe(
      ...
      interceptedByPlanqProvider
        .filter(
          (x) =>
            x !== "eth_sendTransaction" && !x.startsWith("eth_signTypedData")
        )
