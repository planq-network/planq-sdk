import { NetworkConfig, timeTravel } from '@planq-network/dev-utils/lib/ganache-test'
...
export async function assumeOwnership(web3: Web3, to: string) {
  ...
  try {
    ...
    await lockedPlanq.lock().sendAndWaitForReceipt({ from: accounts[0], value: ONE_CGLD })
  } catch (error) {
  ...
  const tenMillionPLQ = web3.utils.toWei('10000000')

  await lockedPlanq.lock().sendAndWaitForReceipt({ value: tenMillionPLQ })
