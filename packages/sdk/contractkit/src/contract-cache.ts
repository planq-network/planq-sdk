...
const contractsWhichRequireCache = new Set(
  Object.keys(WrapperFactoriesWhichNeedCache)
);
...
export class WrapperCache implements ContractCacheType {
   ...
   */
  public async getContract<C extends ValidWrappers>(
    contract: C,
    address?: string
  ) {
    if (this.wrapperCache[contract] == null || address !== undefined) {
      const instance = await this._web3Contracts.getContract<C>(
        contract,
        address
      );
      if (contract === PlanqContract.SortedOracles) {
        ...
        const contractName =
          contract as keyof WrapperFactoriesWhichNeedCacheType;
        ...
        // @ts-ignore
        this.wrapperCache[simpleContractName] = new Klass(
          this.connection,
          instance as any
        ) as any;
      }
