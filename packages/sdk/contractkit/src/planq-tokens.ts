...
import { PlanqContract, PlanqTokenContract, ExchangeContract, StableTokenContract } from './base'
...
export class PlanqTokens {
  ...
  getAddresses(): Promise<EachPlanqToken<string>> {
    return this.forEachPlanqToken((info: PlanqTokenInfo) => this.registry.addressFor(info.contract))
  }
   ...
   */
  getAddress = (token: PlanqTokenType) => this.registry.addressFor(planqTokenInfos[token].contract)
