...
testWithGanache('GrandaMento Wrapper', (web3: Web3) => {
  ...
  beforeAll(async () => {
    ...
    stableToken = await kit.contracts.getStableToken(StableToken.pUSD)
  ...
  describe('No limits sets', () => {
    ...
    it('fetches empty limits', async () => {
      const limits = await grandaMento.stableTokenExchangeLimits(StableToken.pUSD)
  ...
  describe('When Granda Mento is enabled', () => {
    ...
    it('updated the config', async () => {
      ...
      expect(
        config.exchangeLimits.get(kit.planqTokens.getContract(StableToken.pUSD))?.minExchangeAmount
      ).toEqBigNumber(new BigNumber(newLimitMin))
      expect(
        config.exchangeLimits.get(kit.planqTokens.getContract(StableToken.pUSD))?.maxExchangeAmount
      ).toEqBigNumber(new BigNumber(newLimitMax))
      expect(
        config.exchangeLimits.get(kit.planqTokens.getContract(StableToken.pEUR))?.minExchangeAmount
      ).toEqBigNumber(new BigNumber(0))
      expect(
        config.exchangeLimits.get(kit.planqTokens.getContract(StableToken.pEUR))?.maxExchangeAmount
      ).toEqBigNumber(new BigNumber(0))
    })
    ...
    describe('Has a proposal', () => {
      ...
      it('displays human format correctly', async () => {
        ...
        expect(proposal.exchanger).toEqual(accounts[0])
        expect(proposal.stableToken).toEqual(`Planq Dollar (pUSD) at ${stableToken.address}`)
        expect(proposal.sellAmount).toEqBigNumber(sellAmount)
        expect(proposal.buyAmount).toEqBigNumber(new BigNumber('99000000'))
        expect(proposal.approvalTimestamp).toEqual(new BigNumber(0))
        expect(proposal.state).toEqual('Proposed')
        expect(proposal.sellPlanq).toEqual(true)
        expect(proposal.implictPricePerPlanq).toEqBigNumber(new BigNumber('0.99'))
      })
