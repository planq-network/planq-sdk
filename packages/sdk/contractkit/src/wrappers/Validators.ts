import { eqAddress, findAddressIndex, NULL_ADDRESS } from '@planq-network/base/lib/address'
import { concurrentMap } from '@planq-network/base/lib/async'
import { zeroRange, zip } from '@planq-network/base/lib/collections'
import { Address, PlanqTransactionObject, EventLog, toTransactionObject } from '@planq-network/connect'
...
export interface LockedPlanqRequirements {
  value: BigNumber
  duration: BigNumber
}

export interface ValidatorsConfig {
  groupLockedPlanqRequirements: LockedPlanqRequirements
...
export class ValidatorsWrapper extends BaseWrapperForGoverning<Validators> {
   ...
   */
  setNextCommissionUpdate: (commission: BigNumber.Value) => PlanqTransactionObject<void> = proxySend(
    this.connection,
   ...
   */
  updateCommission: () => PlanqTransactionObject<void> = proxySend(
    this.connection,
   ...
   * @returns The Locked Planq requirements for validators.
   */
  async getValidatorLockedPlanqRequirements(): Promise<LockedPlanqRequirements> {
    const res = await this.contract.methods.getValidatorLockedPlanqRequirements().call()
    return {
   ...
   * @returns The Locked Planq requirements for validator groups.
   */
  async getGroupLockedPlanqRequirements(): Promise<LockedPlanqRequirements> {
    const res = await this.contract.methods.getGroupLockedPlanqRequirements().call()
    return {
      value: valueToBigNumber(res[0]),
      duration: valueToBigNumber(res[1]),
    }
   ...
   */
  getAccountLockedPlanqRequirement = proxyCall(
  ...
  async getConfig(): Promise<ValidatorsConfig> {
    const res = await Promise.all([
      this.getValidatorLockedPlanqRequirements(),
      this.getGroupLockedPlanqRequirements(),
    ...
    return {
      validatorLockedPlanqRequirements: res[0],
      groupLockedPlanqRequirements: res[1],
  ...
  async getHumanReadableConfig() {
    ...
    const validatorLockedPlanqRequirements = {
      ...config.validatorLockedPlanqRequirements,
      duration: secondsToDurationString(config.validatorLockedPlanqRequirements.duration),
    }
    const groupLockedPlanqRequirements = {
      ...config.groupLockedPlanqRequirements,
      duration: secondsToDurationString(config.groupLockedPlanqRequirements.duration),
    }
    return {
      ...config,
   ...
   */
  updateBlsPublicKey: (blsPublicKey: string, blsPop: string) => PlanqTransactionObject<boolean> =
    proxySend(
      this.connection,
  ...
  meetsValidatorBalanceRequirements = async (address: Address) => {
    ...
    const total = await lockedPlanq.getAccountTotalLockedPlanq(address)
    const reqs = await this.getValidatorLockedPlanqRequirements()
  ...
  meetsValidatorGroupBalanceRequirements = async (address: Address) => {
    const lockedPlanq = await this.contracts.getLockedPlanq()
    const total = await lockedPlanq.getAccountTotalLockedPlanq(address)
    const reqs = await this.getGroupLockedPlanqRequirements()
    return reqs.value.lte(total)
  }
  ...
  registerValidator: (
    ...
    blsPublicKey: string,
    blsPop: string
  ) => PlanqTransactionObject<boolean> = proxySend(
    this.connection,
   ...
   */
  async registerValidatorGroup(commission: BigNumber): Promise<PlanqTransactionObject<boolean>> {
   ...
   */
  affiliate: (group: Address) => PlanqTransactionObject<boolean> = proxySend(
    this.connection,
   ...
   */
  async addMember(group: Address, validator: Address): Promise<PlanqTransactionObject<boolean>> {
