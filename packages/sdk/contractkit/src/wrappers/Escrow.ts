...
export class EscrowWrapper extends BaseWrapper<Escrow> {
  ...
  ) => PlanqTransactionObject<boolean> = proxySend(this.connection, this.contract.methods.transfer)
  ...
  withdraw: (
    ...
    r: string | number[],
    s: string | number[]
  ) => PlanqTransactionObject<boolean> = proxySend(this.connection, this.contract.methods.withdraw)
   ...
   */
  revoke: (paymentId: string) => PlanqTransactionObject<boolean> = proxySend(
    this.connection,
