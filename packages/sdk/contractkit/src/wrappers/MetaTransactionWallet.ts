import { Address, ensureLeading0x, trimLeading0x } from '@planq-network/base/lib/address'
import { Signature } from '@planq-network/base/lib/signatureUtils'
import { PlanqTransactionObject, PlanqTxObject, toTransactionObject } from '@planq-network/connect'
...
export type TransactionInput<T> = PlanqTxObject<T> | TransactionObjectWithValue<T> | RawTransaction
...
export class MetaTransactionWalletWrapper extends BaseWrapper<MetaTransactionWallet> {
   ...
   */
  public executeTransaction(tx: TransactionInput<any>): PlanqTransactionObject<string> {
  ...
  ): PlanqTransactionObject<{ 0: string; 1: string[] }> {
  ...
  public async signAndExecuteMetaTransaction(
    tx: TransactionInput<any>
  ): Promise<PlanqTransactionObject<string>> {
  ...
  transferOwnership: (newOwner: Address) => PlanqTransactionObject<void> = proxySend(
    this.connection,
    this.contract.methods.transferOwnership
  )

  setSigner: (newSigner: Address) => PlanqTransactionObject<void> = proxySend(
    this.connection,
    this.contract.methods.setSigner
  )

  setEip712DomainSeparator: () => PlanqTransactionObject<void> = proxySend(
    this.connection,
