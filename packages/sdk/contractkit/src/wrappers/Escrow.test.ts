...
testWithGanache("Escrow Wrapper", (web3) => {
  ...
  const TEN_AUSD = kit.web3.utils.toWei("10", "ether");
  ...
  it("transfer with trusted issuers should set TrustedIssuersPerPayment", async () => {
    ...
    await stableTokenContract
      .approve(escrow.address, TEN_AUSD)
      .sendAndWaitForReceipt();
    ...
    const trustedIssuersPerPayment = await escrow.getTrustedIssuersPerPayment(
      testPaymentId
    );
  ...
  it("withdraw should be successful after transferWithTrustedIssuers", async () => {
    ...
    expect(senderBalanceBefore.minus(+TEN_AUSD)).toEqual(senderBalanceAfter);
    expect(receiverBalanceBefore.plus(+TEN_AUSD)).toEqual(receiverBalanceAfter);
  });
