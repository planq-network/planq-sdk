...
import MTWContract from '@planq-network/protocol/build/contracts/MetaTransactionWallet.json'
import { generateTypedDataHash } from '@planq-network/utils/lib/sign-typed-data-utils'
...
testWithGanache('MetaTransactionWallet Wrapper', (web3) => {
  ...
  beforeAll(async () => {
    ...
    rando = accounts[2]
    planq = await kit.contracts.getPlanqToken()
  })
  ...
  beforeEach(async () => {
    ...
    await planq.transfer(wallet.address, new BigNumber(20e18).toFixed()).sendAndWaitForReceipt()
  ...
  describe('#executeTransaction', () => {
    describe('as a rando', () => {
      it('reverts', async () => {
        ...
        await expect(
          wallet
            .executeTransaction(planq.transfer(emptyAccounts[0], 10000).txo)
    ...
    describe('as the signer', () => {
      it('can call contracts', async () => {
        ...
        const result = await wallet
          .executeTransaction(planq.transfer(emptyAccounts[0], value.toFixed()).txo)
          .sendAndWaitForReceipt()
        expect(result.status).toBe(true)
        expect(await planq.balanceOf(emptyAccounts[0])).toEqual(value)
      })
    })
  })
  ...
  describe('#executeTransactions', () => {
    describe('as a rando', () => {
      it('reverts', async () => {
        ...
        await expect(
          wallet
            .executeTransactions([
              planq.transfer(emptyAccounts[0], 1000).txo,
              planq.transfer(emptyAccounts[1], 1000).txo,
            ])
            .sendAndWaitForReceipt({ from: rando })
        ).rejects.toThrow(/Invalid transaction sender/)
      })
    ...
    describe('as the signer', () => {
      it('can execute transactions', async () => {
        const walletBalanceBefore = await planq.balanceOf(wallet.address)
        const value = new BigNumber(1e18)
        const result = await wallet
          .executeTransactions([
            planq.transfer(emptyAccounts[0], value.toFixed()).txo,
            planq.transfer(emptyAccounts[1], value.toFixed()).txo,
          ])
          .sendAndWaitForReceipt()
        expect(result.status).toBe(true)

        expect(await planq.balanceOf(wallet.address)).toEqual(
          walletBalanceBefore.minus(value.times(2))
        )
        expect(await planq.balanceOf(emptyAccounts[0])).toEqual(value)
        expect(await planq.balanceOf(emptyAccounts[1])).toEqual(value)
      })
    })
  })
  ...
  describe('#getMetaTransactionDigest', () => {
    it('should match the digest created off-chain', async () => {
      const metaTransfer = planq.transfer(emptyAccounts[0], 1000).txo
  ...
  describe('#getMetaTransactionSigner', () => {
    it('should match what is signed off-chain', async () => {
      const metaTransfer = planq.transfer(emptyAccounts[0], 1000000).txo
  ...
  describe('#executeMetaTransaction', () => {
    describe('as a rando', () => {
      it('can execute transactions', async () => {
        ...
        const metaTransfer = planq.transfer(emptyAccounts[0], value.toFixed()).txo
        ...
        expect(await planq.balanceOf(emptyAccounts[0])).toEqual(value)
        expect(await planq.balanceOf(wallet.address)).toEqual(walletBalanceBefore.minus(value))
      })
      ...
      it('can batch transactions as a call to self', async () => {
        ...
        const metaBatch = wallet.executeTransactions([
          planq.transfer(emptyAccounts[0], value.toFixed()).txo,
          planq.transfer(emptyAccounts[1], value.toFixed()).txo,
          planq.transfer(emptyAccounts[2], value.toFixed()).txo,
        ]).txo
        ...
        for (let i = 0; i < 3; i++) {
          expect(await planq.balanceOf(emptyAccounts[i])).toEqual(value)
        }
      })
    })
