...
testWithGanache('Governance Wrapper', (web3: Web3) => {
  ...
  beforeAll(async () => {
    ...
    await concurrentMap(4, accounts.slice(0, 4), async (account) => {
      ...
      await lockedPlanq.lock().sendAndWaitForReceipt({ from: account, value: ONE_CGLD })
    })
  }, 5 * ONE_SEC)

  type Repoint = [PlanqContract, Address]
  ...
  describe('Proposals', () => {
    const repoints: Repoint[] = [
      [PlanqContract.Random, '0x0000000000000000000000000000000000000001'],
      [PlanqContract.Escrow, '0x0000000000000000000000000000000000000002'],
    ]
  ...
  // https://github.com/planq-network/planq-sdk/issues/1737

  // describe('Hotfixes', () => {
  //   const repoints: Repoint[] = [
  //     [PlanqContract.Random, '0x0000000000000000000000000000000000000003'],
  //     [PlanqContract.Escrow, '0x0000000000000000000000000000000000000004'],
  //   ]
