...
import { concurrentMap, concurrentValuesMap } from '@planq-network/base/lib/async'
...
export class ElectionWrapper extends BaseWrapperForGoverning<Election> {
   ...
   */
  async activate(account: Address): Promise<Array<PlanqTransactionObject<boolean>>> {
    const groups = await this.contract.methods.getGroupsVotedForByAccount(account).call()
    const isActivatable = await Promise.all(
  ...
  async revokePending(
    ...
    group: Address,
    value: BigNumber
  ): Promise<PlanqTransactionObject<boolean>> {
    const groups = await this.contract.methods.getGroupsVotedForByAccount(account).call()
  ...
  async revoke(
    ...
    group: Address,
    value: BigNumber
  ): Promise<Array<PlanqTransactionObject<boolean>>> {
   ...
   */
  async vote(validatorGroup: Address, value: BigNumber): Promise<PlanqTransactionObject<boolean>> {
