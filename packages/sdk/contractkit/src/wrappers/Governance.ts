...
import { Address, PlanqTxPending, toTransactionObject } from '@planq-network/connect'
...
export type ProposalTransaction = Pick<PlanqTxPending, 'to' | 'input' | 'value'>
...
export class GovernanceWrapper extends BaseWrapperForGoverning<Governance> {
  ...
  async getConstitution(proposal: Proposal): Promise<BigNumber> {
    ...
    // https://github.com/planq-network/planq-sdk/blob/3fffa158d67ffd6366e81ba7243eadede1974b1b/packages/protocol/contracts/governance/Governance.sol#L39
  ...
  async getSupport(proposalID: BigNumber.Value) {
    ...
    const networkWeight = await lockedPlanq.getTotalLockedPlanq()
  ...
  async getVoteWeight(voter: Address) {
    const lockedPlanqContract = await this.contracts.getLockedPlanq()
    return lockedPlanqContract.getAccountTotalLockedPlanq(voter)
  }
