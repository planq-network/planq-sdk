...
import { StableToken as StableTokenEnum, stableTokenInfos } from '../planq-tokens'
import { GrandaMento } from '../generated/astonic/GrandaMento'
...
export class GrandaMentoWrapper extends BaseWrapper<GrandaMento> {
  ...
  async createExchangeProposal(
    ...
    return createExchangeProposalInner(stableTokenRegistryId, sellAmount.toFixed(), sellPlanq)
  }
  ...
  async getHumanReadableExchangeProposal(
    ...
    return {
      ...
      implictPricePerPlanq: proposal.sellPlanq
  ...
  async getBuyAmount(
    planqStableTokenOracleRate: BigNumber, // Note this is intended to be a fixed point number
    sellAmount: BigNumber,
    sellPlanq: boolean
  ): Promise<BigNumber> {
    const result = await this.contract.methods
      .getBuyAmount(planqStableTokenOracleRate.toFixed(), sellAmount.toFixed(), sellPlanq)
      .call()
