...
import {Address, Connection, toTransactionObject} from "@planq-network/connect";
...
export class AttestationsWrapper extends BaseWrapper<Attestations> {
  ...
  async getVerifiedStatus(
    ...
    const fractionAttestation =
      stats.total < 1 ? 0 : stats.completed / stats.total;
    ...
    const isVerified =
      numAttestationsRemaining <= 0 &&
      fractionAttestation >= attestationThreshold;
  ...
  async getAttestationFeeRequired(attestationsRequested: number) {
    const contract = await this.contracts.getStableToken(StableToken.aUSD);
  ...
  async approveAttestationFee(attestationsRequested: number) {
    const tokenContract = await this.contracts.getStableToken(StableToken.aUSD);
   ...
   * @param tokens List of tokens used for attestation fees. use PlanqTokens.getAddresses() to get
  ...
  async revoke(identifer: string, account: Address) {
    ...
    return toTransactionObject(
      this.connection,
      this.contract.methods.revoke(identifer, idx)
    );
  }
}
