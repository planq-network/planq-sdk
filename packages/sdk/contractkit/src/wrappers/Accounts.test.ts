...
testWithGanache("Accounts Wrapper", (web3) => {
  ...
  const registerAccountWithLockedPlanq = async (account: string) => {
    ...
    await lockedPlanq
      .lock()
      .sendAndWaitForReceipt({from: account, value: minLockedPlanqValue});
  };

  const getParsedSignatureOfAddress = async (
    address: string,
    signer: string
  ) => {
  ...
  test("SBAT authorize attestation key", async () => {
    ...
    const attestationSigner = await accountsInstance.getAttestationSigner(
      account
    );
  ...
  test("SBAT remove attestation key authorization", async () => {
    ...
    let attestationSigner = await accountsInstance.getAttestationSigner(
      account
    );
    expect(attestationSigner).toEqual(signer);
  ...
  test("SBAT authorize validator key and change BLS key atomically", async () => {
    ...
    const sig = await getParsedSignatureOfAddress(account, signer);
    await (
      await accountsInstance.authorizeValidatorSignerAndBls(
        signer,
        sig,
        newBlsPublicKey,
        newBlsPoP
  ...
  test("SBAT set the wallet address to a different wallet address", async () => {
    ...
    await accountsInstance
      .createAccount()
      .sendAndWaitForReceipt({from: account});
    const signature = await accountsInstance.generateProofOfKeyPossession(
      account,
      wallet
    );
    await accountsInstance
  ...
  test("SNBAT fraction greater than 1", async () => {
    ...
    await expect(
      accountsInstance
        .setPaymentDelegation(beneficiary, fractionInvalid)
        .sendAndWaitForReceipt({})
    ).rejects.toEqual(
      new Error(
  ...
  test("SNBAT beneficiary and fraction", async () => {
    ...
    await accountsInstance
      ...
      .sendAndWaitForReceipt({from: account});
    await accountsInstance
      .setPaymentDelegation(beneficiary, fractionValid)
      .sendAndWaitForReceipt();
