...
import { addressToPublicKey, parseSignature } from '@planq-network/utils/lib/signatureUtils'
...
testWithGanache('Accounts Wrapper', (web3) => {
  ...
  const registerAccountWithLockedPlanq = async (account: string) => {
    ...
    await lockedPlanq.lock().sendAndWaitForReceipt({ from: account, value: minLockedPlanqValue })
  }
