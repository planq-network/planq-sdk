...
testWithGanache('Reserve Wrapper', (web3) => {
  ...
  test('can get sum of reserve unfrozen balance + other reserve address balances', async () => {
    const balanceWithOtherAddresses = await reserve.getUnfrozenReservePlanqBalance()
  ...
  test('two spenders required to confirm transfers planq', async () => {
    const tx = await reserve.transferPlanq(otherReserveAddress, 10)
    ...
    const tx2 = await reserve.transferPlanq(otherReserveAddress, 10)
  ...
  test('test does not transfer planq if not spender', async () => {
    const tx = await reserve.transferPlanq(otherReserveAddress, 10)
    const multisigTx = await reserveSpenderMultiSig.submitOrConfirmTransaction(
      reserve.address,
      tx.txo
