import {eqAddress} from "@planq-network/base/lib/address";
import {Err, Ok, Result} from "@planq-network/base/lib/result";
import {
  CircuitBreakerClient,
  CircuitBreakerKeyStatus,
  CircuitBreakerServiceContext,
  CircuitBreakerUnavailableError,
} from "@planq-network/identity/lib/odis/circuit-breaker";
...
import {
  ...
  HardeningConfig,
  PASSWORD_HARDENING_ATLAS_CONFIG,
  PASSWORD_HARDENING_MAINNET_CONFIG,
  PIN_HARDENING_ATLAS_CONFIG,
...
export async function createPinEncryptedBackup({
  ...
  if (environment === EnvironmentIdentifier.ATLAS) {
    hardening = PIN_HARDENING_ATLAS_CONFIG;
  } else if (
    environment === EnvironmentIdentifier.MAINNET ||
...
export async function createPasswordEncryptedBackup({
  ...
  let hardening: HardeningConfig | undefined;
  if (environment === EnvironmentIdentifier.ATLAS) {
    hardening = PASSWORD_HARDENING_ATLAS_CONFIG;
  } else if (
    environment === EnvironmentIdentifier.MAINNET ||
    environment === undefined
  ) {
...
export async function createBackup({
  ...
  debug(
    "creating a backup with the following information",
    hardening,
    metadata
  );
  ...
  if (hardening.odis === undefined && hardening.computational === undefined) {
    return Err(
      new UsageError(
        new Error("createBackup cannot be used with a empty hardening config")
      )
    );
  }
  ...
  const userSecretBuffer =
    typeof userSecret === "string"
      ? Buffer.from(userSecret, "utf8")
      : userSecret;
  const initialKey = deriveKey(KDFInfo.PASSWORD, [
    userSecretBuffer,
    passwordSalt,
  ]);
  ...
  if (hardening.circuitBreaker !== undefined) {
    debug(
      "generating a fuse key to enabled use of the circuit breaker service"
    );
    const circuitBreakerClient = new CircuitBreakerClient(
      hardening.circuitBreaker.environment
    );
  ...
  if (hardening.computational !== undefined) {
    debug(
      "hardening user key with computational function",
      hardening.computational
    );
...
export async function openBackup({
  ...
  if (backup.encryptedFuseKey !== undefined) {
    ...
    const unwrap = await circuitBreakerClient.unwrapKey(
      backup.encryptedFuseKey
    );
    if (!unwrap.ok) {
  ...
  if (backup.odisDomain !== undefined) {
    ...
    const authorizer = odisQueryAuthorizer(odisAuthKeySeed);
    if (
      domain.address.defined &&
      !eqAddress(authorizer.address, domain.address.value)
    ) {
      return Err(
        new InvalidBackupError(
          new Error(
    ...
    if (backup.environment?.odis === undefined) {
      return Err(
        new InvalidBackupError(
          new Error(
            "ODIS domain is provided by no ODIS environment information"
          )
        )
      );
    }
