import {getPhoneHash} from "@planq-network/base";
...
// this DEK has been registered to the above account on atlas
const dekPrivateKey =
...
const oldServiceContext = OdisUtilsOld.Query.getServiceContext("atlas");
const currentServiceContext = getServiceContext(OdisContextName.ATLAS);
...
describe("backwards compatibility of phone number identifiers", () => {
  ...
  it("should match when using EncryptionSigner", async () => {
    const oldRes =
      await OdisUtilsOld.PhoneNumberIdentifier.getPhoneNumberIdentifier(
        mockE164Number,
        mockAccount,
        authSigner,
        oldServiceContext
  ...
  it("blinded identifier should match", async () => {
    ...
    const oldRes =
      await OdisUtilsOld.PhoneNumberIdentifier.getBlindedPhoneNumber(
        mockE164Number,
        blsBlindingClient,
        seed
      );
  ...
  it("obfuscated identifier should match", async () => {
    ...
    const currRes = getIdentifierHash(
      mockE164Number,
      IdentifierPrefix.PHONE_NUMBER,
      expectedPepper
    );
