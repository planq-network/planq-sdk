import {MnemonicLanguages} from "@planq-network/base/lib/account";
  ...
  describe(".generateMnemonic()", () => {
    ...
    for (const language of getAllLanguages()) {
      ...
      it(`should generate a valid mnemonic in ${languageName}}`, async () => {
        ...
        expect(
          bip39.validateMnemonic(mnemonic, bip39.wordlists[languageName])
        ).toBeTruthy();
      });
    }
  });
  ...
  describe(".validateMnemonic()", () => {
    const testMnemonics = {
      ...
      [MnemonicLanguages.french]:
        "texte succès lexique frégate sévir oiseau lanaeur souvenir mythique onirique pélican opérer foulure enfouir maintien vexer relief aérer citerne ligoter arbitre gomme sénateur dénouer",
  ...
  describe(".generateKeys()", () => {
    it("should generate an expected private key for a mnemonic", async () => {
      ...
      const expectedPrivateKeys = [
        {
          derivation0: {
            ...
            privateKey:
              "9b20170cd294190efb2eb1d406a51e6705461cb540e777784565c1d8342016d7",
            publicKey:
              "0257780786b4ba7bf47b3be6082f65069f552012735a17c2080648de67cfb440c1",
          },
          derivation1: {
            address: "0x9a85EBC698647895a1e12544E9B1751Aed57f9F4",
            privateKey:
              "29c025cda952cb59d8504cca390bcdbc2cc3706ca5bfb65a5b6f5dd5dc5176dd",
            publicKey:
              "029c90f394bea3d46c46896b0bd1b36119b031a6da498b49a32aad77f10ce1f672",
          },
          password: {
            address: "0x1546D345F7A5Cf533290fd54833de1Ce0552A2d7",
            privateKey:
              "92956ef74d530224029380385ca556240be042b7e5592ffece5945286f1562c3",
            publicKey:
        ...
        },
        {
          derivation0: {
            ...
            privateKey:
              "eb0fea0b0aab13e4115939e2af62a5f136cdeefa1f0480c5550b93550339857d",
            publicKey:
              "0279548f9e2b8fbbeb91865068257de240fa307d479c94926a32e33f8707c70497",
          },
          derivation1: {
            address: "0x5bA0350E8a681b0fe3D939633B5514A9A6152f81",
            privateKey:
              "cd2aa97d2bf6ddac4f00513eb08de8d40dcff4106508d18ea995ffbff8166420",
            publicKey:
              "03e4784248f6c4b3bacf8090f67d002e58f4da448bb9b993432fe226abdcd5c83f",
          },
          password: {
            address: "0x9A78acd4C77c796C3d657f17F3D05cd46eFCC5bE",
            privateKey:
              "62c928058f72e5a04abcf5e46035d5f0933f996285ec25b3bc9d2b9fc907dc56",
            publicKey:
              "0240d5adc5c0ce46f3488401e3ea78a261de1cd8a8e6a1d9e55386c6c4881b70ec",
          },
        },
        {
          derivation0: {
            ...
            privateKey:
              "bfc514b7a895cade755f65196b4807a0635381ee16195b33e22b919ecaedf553",
            publicKey:
              "034ca1fb554b952b6794da020c8d101527a2a91884dbab671211ce77b2ec3f1a3e",
          },
          derivation1: {
            address: "0xff1ef005f5A11426343D3492d73e94bad169d900",
            privateKey:
              "074e6edfc31f8ccfd93427d204da5ada15124a25fde119b7f65b54ff283b6207",
            publicKey:
              "03606b5f63932b2a896c3fb3aa7f60f0f5aa9cd7ce8310199cae2c06514159f799",
          },
          password: {
            address: "0xb73C6AaDb67238323d811469A95E8e2B92cC0B4A",
            privateKey:
              "743594169177ae8ab3dd08a6e22842a2ac43dbe886a73eebc33cd21e73175661",
            publicKey:
              "03aa657da15ceb192b73a3aa3a36512a765d9c9751763dd7801585fba8d10f7467",
          },
        },
      ];
      ...
      for (let i = 0; i < mnemonics.length; ++i) {
        ...
        expect({derivation0, derivation1, password}).toEqual(
          expectedPrivateKeys[i]
        );
      }
    });
  });
  ...
  describe(".suggestMnemonicCorrections()", () => {
    it("should correct a single simple typo on the first suggestion", () => {
      const cases = [
        {
          mnemonic:
            "crush hollow differ mean easy ostrihc almost cherry route hurt inner bless",
          corrected:
            "crush hollow differ mean easy ostrich almost cherry route hurt inner bless",
        },
        {
          mnemonic:
            "monster note endless discover tilt glide girl wing spstial imitate mad ridge",
          corrected:
            "monster note endless discover tilt glide girl wing spatial imitate mad ridge",
        },
        {
          mnemonic:
            "mimo musgo efecto danza tariot gente gavilán visor sala imán madre potencia",
          corrected:
            "mimo musgo efecto danza tarot gente gavilán visor sala imán madre potencia",
        },
        {
          mnemonic:
      ...
      for (const {mnemonic, corrected} of cases) {
        expect(suggestMnemonicCorrections(mnemonic).next().value).toEqual(
          corrected
        );
      }
    });
    ...
    it("should quickly offer the corect suggestion for a phrase with a few typos", () => {
      ...
      for (const {mnemonic, corrected} of cases) {
        ...
        for (const suggestion of suggestMnemonicCorrections(mnemonic)) {
          ...
          if (attempts >= 25) {
            throw new Error(
              `Phrase '${mnemonic}' was not corrected within 100 attempts`
            );
          }
        }
      }
