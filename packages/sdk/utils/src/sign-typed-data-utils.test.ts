...
const TEST_TYPES: EIP712TestCase[] = [
    ...
    examples: [
      {
        data: {
          from: {
            wallet: "0x000000000000000000000000000000000000a1ce",
            name: "Alice",
          },
          to: {
            name: "Bob",
            wallet: "0x0000000000000000000000000000000000000b0b",
          },
          tx: {
            token: "0x9DabFe01de024C681320eb80FBc64EccEaa58ca2",
  ...
  },
  {
    ...
    typeEncoding:
      "GameBoard(Tile[][] grid)Tile(bool occupied,uint8 occupantId)",
    zero: {
      grid: [],
    },
    examples: [
              ...
              keccak256(
                Buffer.concat([
                  keccak256(
                    Buffer.concat([
                      keccak256(
                        utf8ToBytes("Tile(bool occupied,uint8 occupantId)")
                      ),
                      Buffer.from(
    ...
    examples: [
      {
        ...
        dataEncoding: Buffer.concat([
          ...
          Buffer.from(
            "0000000000000000000000001a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b",
            "hex"
          ),
          Buffer.from(
  ...
  for (const {primaryType, types, typeEncoding} of TEST_TYPES) {
    it(`should hash type ${primaryType} correctly`, () => {
      expect(typeHash(primaryType, types)).toEqual(
        keccak256(utf8ToBytes(typeEncoding))
      );
    });
  }
});
      ...
      it(`should hash data ${primaryType} correctly`, () => {
        for (const {data, dataEncoding} of examples) {
          const expected = keccak256(
            Buffer.concat([typeHash(primaryType, types), dataEncoding])
          );
