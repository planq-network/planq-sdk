...
testWithGanache("URI utils", (web3) => {
  ...
  beforeAll(async () => {
    const stableTokenAddr = await kit.registry.addressFor(
    ...
    const lockedPlanqAddr = await kit.registry.addressFor(
  ...
  describe("#parseUri", () => {
    ...
    it("should match aUSD transfer tx", () => {
  ...
  describe("#buildUri", () => {
    ...
    it("should match aUSD transfer URI", () => {
      const uri = buildUri(stableTokenTransferTx, "transfer", [
        "address",
        "uint256",
      ]);
