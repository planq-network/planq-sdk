import {trimLeading0x} from "@planq-network/base/lib/address";
import {zeroRange} from "@planq-network/base/lib/collections";
import {AbiCoder, PlanqTx} from "@planq-network/connect";
...
const ABI_TYPE_REGEX =
  "(u?int(8|16|32|64|128|256)|address|bool|bytes(4|32)?|string)(\\[\\])?";
...
const URI_REGEX_STR = `^planq:${ADDRESS_REGEX_STR}(@${CHAIN_ID_REGEX})?(/${FUNCTION_REGEX})?(\\?${QUERY_REGEX})?$`;

const uriRegexp = new RegExp(URI_REGEX_STR);

export function parseUri(uri: string): PlanqTx {
...
export function buildUri(
  ...
  if (!tx.to) {
    throw new Error("'to' address must be defined for planq URIs");
  }
  let uri = `planq:${tx.to!}`;
  ...
  if (tx.data !== undefined) {
    ...
    const functionSig = trimLeading0x(
      abi.encodeFunctionSignature(functionSelector)
    );
    ...
    if (functionSig !== funcEncoded) {
      throw new Error(
        "'functionName' and 'abiTypes' do not match first 4 bytes of 'tx.data'"
      );
    }
    ...
    if (txData.length > 8) {
      ...
      functionArgs = zeroRange(decoded.__length__).map((idx) =>
        decoded[idx].toLowerCase()
      );
    }
  }

  const {data, to, chainId, nonce, hardfork, common, chain, ...txQueryParams} =
    tx;
