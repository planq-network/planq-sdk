...
import { PlanqTxObject } from '@planq-network/connect'
...
import { privateKeyToAddress, privateKeyToPublicKey } from '@planq-network/utils/lib/address'
import { toFixed } from '@planq-network/utils/lib/fixidity'
import { signMessage } from '@planq-network/utils/lib/signatureUtils'
import { BigNumber } from 'bignumber.js'
import { AccountsInstance, ElectionInstance, LockedPlanqInstance, ValidatorsInstance } from 'types'
...
async function lockPlanq(
  ...
  // @ts-ignore
  const lockTx = lockedPlanq.contract.methods.lock()
...
async function registerValidatorGroup(
  ...
  const v = lockedPlanqValue.times(1.01).integerValue()
  ...
  console.info(`    - lock planq`)
  await lockPlanq(accounts, lockedPlanq, lockedPlanqValue, account.privateKey)
...
async function registerValidator(
  ...
  console.info(`    - lockPlanq ${valName}`)
  await lockPlanq(
    accounts,
    lockedPlanq,
...
module.exports = async (_deployer: any, networkName: string) => {
  ...
  // Calculate per validator locked planq for first group...
  const lockedPlanqPerValAtFirstGroup = new BigNumber(
    config.validators.groupLockedPlanqRequirements.value
  )
  // ...and the delta for each subsequent group
  const lockedPlanqPerValEachGroup = new BigNumber(
    config.validators.votesRatioOfLastVsFirstGroup - 1
  )
    .times(lockedPlanqPerValAtFirstGroup)
    .div(Math.max(valKeyGroups.length - 1, 1))
    .integerValue()

  const groups = valKeyGroups.map((keys, i) => {
    const lockedPlanqAmount = lockedPlanqPerValAtFirstGroup
      .plus(lockedPlanqPerValEachGroup.times(i))
      .times(keys.length)
    return {
      valKeys: keys,
      ...
      lockedPlanq: lockedPlanqAmount,
      voteAmount:
        i === 0 || i === valKeyGroups.length - 1
          ? lockedPlanqAmount
          : new BigNumber(config.validators.groupLockedPlanqRequirements.value),
      account: null,
    }
  })
  ...
  for (const [idx, group] of groups.entries()) {
    console.info(
      `  Registering validator group: ${group.name} with: ${group.lockedPlanq} CG locked...`
    )
