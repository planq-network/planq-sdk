import { ArtifactsSingleton } from '@planq-network/protocol/lib/artifactsSingleton';
import { hasEntryInRegistry, usesRegistry } from '@planq-network/protocol/lib/registry-utils';
...
export async function assumeOwnershipWithTruffle(contractsToOwn: string[], to: string, dequeuedIndex: number = 0, contractPackage?:ContractPackage) {
  ...
  const lockedPlanq: LockedPlanqInstance = await getDeployedProxiedContract('LockedPlanq', artifacts)
  ...
  const tenMillionPLQ = '10000000000000000000000000'
  // @ts-ignore
  await lockedPlanq.lock({ value: tenMillionPLQ })
