    ...
    ReentrancyGuard
{
   ...
   * @param frozenPlanq The amount of PLQ frozen.
   * @param frozenDays The number of days the frozen PLQ thaws over.
   */
    function setFrozenPlanq(uint256 frozenPlanq, uint256 frozenDays)
        public
        onlyOwner
    {
        require(
            frozenPlanq <= address(this).balance,
    ...
    /**
   * @notice Sets target allocations for PLQ and a diversified basket of non-Planq assets.
   ...
   */
    function setAssetAllocations(
        ...
        // NOTE: The PLQ asset launched as "Planq Planq" (cGLD), but was renamed to
        // just PLQ by the community.
        // TODO: Change "cGLD" to "PLQ" in this file, after ensuring that any
   ...
   */
        ...
        returns (bool)
    {
        ...
        address lastItem = otherReserveAddresses[otherReserveAddresses
            .length
            .sub(1)];
   ...
   * @return Returns true if the transaction succeeds.
   */
    function transferPlanq(address payable to, uint256 value)
        external
        returns (bool)
    {
        require(
   ...
   * @return Returns true if the transaction succeeds.
   */
    function _transferPlanq(address payable to, uint256 value)
        internal
        returns (bool)
    {
   ...
   * @return Returns true if the transaction succeeds.
   */
    function transferExchangePlanq(address payable to, uint256 value)
        external
   ...
   */
    ...
    /**
   * @notice Returns the amount of unfrozen PLQ in the reserve.
   * @return The total unfrozen PLQ in the reserve.
   */
    function getUnfrozenBalance() public view returns (uint256) {
        uint256 balance = address(this).balance;
        uint256 frozenReservePlanq = getFrozenReservePlanqBalance();
        return
            balance > frozenReservePlanq ? balance.sub(frozenReservePlanq) : 0;
    }

    /**
   * @notice Returns the amount of PLQ included in the reserve.
   * @return The PLQ amount included in the reserve.
   */
    function getReservePlanqBalance() public view returns (uint256) {
        return
            address(this).balance.add(getOtherReserveAddressesPlanqBalance());
    }

    /**
   * @notice Returns the amount of PLQ included in other reserve addresses.
   * @return The PLQ amount included in other reserve addresses.
   */
    function getOtherReserveAddressesPlanqBalance()
        public
        view
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < otherReserveAddresses.length; i = i.add(1)) {
            reservePlanqBalance = reservePlanqBalance.add(
   ...
   * @return The unfrozen PLQ amount included in the reserve.
   */
    ...
    /**
   * @notice Returns the amount of frozen PLQ in the reserve.
   * @return The total frozen PLQ in the reserve.
   */
    function getFrozenReservePlanqBalance() public view returns (uint256) {
        ...
        return
            frozenReservePlanqStartBalance.sub(
                frozenReservePlanqStartBalance.mul(frozenDays).div(
    ...
    function getReserveRatio() public view returns (uint256) {
        ...
        for (uint256 i = 0; i < _tokens.length; i = i.add(1)) {
            ...
            if (planqAmount != 0) {
                ...
                uint256 aStableTokenValueInPlanq = stableTokenSupply
                    .mul(planqAmount)
                    .div(stableAmount);
                stableTokensValueInPlanq = stableTokensValueInPlanq.add(
                    aStableTokenValueInPlanq
                );
            }
        }
        return
