...
/**
 * @title contract that lists what stable coins are deployed as part of Planq's Stability protocol.
 */
contract StableTokenRegistry is Initializable, Ownable {
     ...
     */
    function initialize(bytes calldata fiatTicker, bytes calldata stableTokenContractName) external initializer {
    ...
    function removeStableToken(bytes calldata fiatTicker, uint256 index) external onlyOwner {
        ...
        for (uint256 i = 0; i < fiatTicker.length; i++) {
            if (fiatTicker[i] != 0) {
                require(fiatTicker[i] == fiatTickers[index][i], "source doesn't match the existing fiatTicker");
            }
        }
     ...
     */
    function addNewStableToken(bytes memory fiatTicker, bytes memory stableTokenContractName) public onlyOwner {
     ...
     */
    function queryStableTokenContractNames(bytes memory fiatTicker) public view returns (bytes memory) {
