...
/**
 * @title Facilitates large exchanges between PLQ stable tokens.
 */
contract GrandaMento is IPlanqVersionedContract, Ownable, Initializable, UsingRegistry, ReentrancyGuard {
  ...
  struct ExchangeProposal {
    ...
    // Whether the exchanger is selling PLQ and buying stableToken.
    bool sellPlanq;
    ...
    uint256 buyAmount;
    // The price of PLQ quoted in stableToken at the time of the exchange proposal
  ...
  // The maximum allowed change in the PLQ/stable token price when an exchange proposal
   ...
   * @param _maxApprovalExchangeRateChange The maximum allowed change in PLQ price
   ...
   * @param sellPlanq Whether PLQ is being sold.
  ...
  function createExchangeProposal(
    ...
    // Gets the price of PLQ quoted in stableToken.
    ...
    uint256 buyAmount = getBuyAmount(celoStableTokenExchangeRate, sellAmount, sellPlanq);
    ...
    // Create new scope to prevent a stack too deep error.
    {
      ...
      uint256 stableTokenExchangeAmount = sellPlanq ? buyAmount : sellAmount;
      require(
    ...
    IERC20 sellToken = sellPlanq ? getPlanqToken() : IERC20(stableToken);
    ...
    uint256 storedSellAmount = sellPlanq ? sellAmount : IStableToken(stableToken).valueToUnits(sellAmount);
    ...
    emit ExchangeProposalCreated(exchangeProposalCount, msg.sender, stableTokenRegistryId, sellAmount, buyAmount, sellPlanq);
  ...
  function approveExchangeProposal(uint256 proposalId) external nonReentrant onlyApprover {
    ...
    // Ensure the change in the current price of PLQ quoted in the stable token
    ...
    require(rateChange.lte(maxApprovalExchangeRateChange), "PLQ exchange rate is too different from the proposed price");
  ...
  function executeExchangeProposal(uint256 proposalId) external nonReentrant {
    ...
    // If the exchange sells PLQ, the PLQ is sent to the Reserve from this contract
    // and stable token is minted to the exchanger.
    if (proposal.sellPlanq) {
      // Send the PLQ from this contract to the reserve.
      require(sellToken.transfer(address(getReserve()), sellAmount), "Transfer out of PLQ to Reserve failed");
      ...
      // this contract and PLQ is transferred from the Reserve to the exchanger.
      ...
      // Transfer the PLQ from the Reserve to the exchanger.
      require(
        getReserve().transferExchangePlanq(proposal.exchanger, proposal.buyAmount),
        "Transfer out of PLQ from Reserve failed"
      );
    }
  ...
  function getSellTokenAndSellAmount(ExchangeProposal memory proposal) private view returns (IERC20, uint256) {
    ...
    // This check should not be necessary for PLQ, but is done so regardless
    // for extra certainty that cancelling an exchange proposal can never fail
    // if for some reason the PLQ balance of this contract is less than the
   ...
   * @dev Stable token value amounts are used for the sellAmount, not unit amounts.
   * Assumes both PLQ and the stable token have 18 decimals.
   * @param celoStableTokenExchangeRate The unwrapped fraction exchange rate of PLQ
  ...
  /**
   * @notice Gets the oracle PLQ price quoted in the stable token.
   ...
   * @return The oracle PLQ price quoted in the stable token.
   */
  ...
  /**
   * @notice Sets the maximum allowed change in the PLQ/stable token price when
