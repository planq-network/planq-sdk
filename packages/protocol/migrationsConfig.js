const {toFixed} = require("@planq-network/utils/lib/fixidity");
const {
  PlanqContractName,
} = require("@planq-network/protocol/lib/registry-utils");
...
const DefaultConfig = {
  ...
  blockchainParameters: {
    gasForNonPlanqCurrencies: 50000,
  ...
  validators: {
    groupLockedPlanqRequirements: {
      value: "10000000000000000000000", // 10k PLQ per validator
      duration: 180 * DAY,
    },
    validatorLockedPlanqRequirements: {
      value: "10000000000000000000000", // 10k PLQ
    ...
    // MUST BE KEPT IN SYNC WITH VALIDATOR LOCKED PLANQ DURATION
...
const NetworkConfigs = {
  ...
  baklava: {
    ...
    stableToken: {
      // Don't set an initial planq price before oracles start to report.
      planqPrice: null,
      oracles: [
  ...
  atlasold: {
    ...
    stableToken: {
      oracles: [
        // Oracle addresses generated from atlas mnemonic
...
NetworkConfigs.atlasstaging = NetworkConfigs.atlas;
...
const migrationOverride = argv.migration_override
  ? JSON.parse(argv.migration_override)
  : {};
