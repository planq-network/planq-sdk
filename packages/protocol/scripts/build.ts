...
export const ImplContracts =
  OtherContracts.concat(ProxyContracts).concat(CoreContracts);
...
function compile(outdir: string) {
  ...
  for (const externalContractPackage of externalContractPackages) {
    console.log(
      `Building external contracts for ${externalContractPackage.name}`
    );
    exec(
...
function generateFilesForTruffle(outdir: string) {
  ...
  exec(
    `yarn run --silent typechain --target=truffle --outDir "${outdir}" "${globPattern}" `
  );
}
...
async function generateFilesForContractKit(outdir: string) {
  ...
  const globPattern = `${BUILD_DIR}/contracts/@(${contractKitContracts.join(
    "|"
  )}).json`;
