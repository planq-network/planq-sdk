import { planqRegistryAddress } from '@planq-network/protocol/lib/registry-utils'
import { _setInitialProxyImplementation, retryTx } from '@planq-network/protocol/lib/web3-utils'
import { Address, isValidAddress } from '@planq-network/utils/lib/address'
...
const ONE_PLQ = web3.utils.toWei('1', 'ether')
const TWO_PLQ = web3.utils.toWei('2', 'ether')
...
type ReleasePlanqTemplate = Partial<ReleasePlanqConfig>

async function handleGrant(config: ReleasePlanqConfig, currGrant: number) {
  ...
  if (totalValue.lt(startPlanq)) {
    console.info('Total value of grant less than PLQ for beneficiary addreess')
    return
  }

  const adjustedAmountPerPeriod = totalValue.minus(startPlanq).div(config.numReleasePeriods).dp(0)
  ...
  console.info('  Deploying ReleasePlanqMultiSigProxy...')
  const releasePlanqMultiSigProxy = await retryTx(ReleasePlanqMultiSigProxy.new, [
    { from: fromAddress },
  ])
  console.info('  Deploying ReleasePlanqMultiSig...')
  const releasePlanqMultiSigInstance = await retryTx(ReleasePlanqMultiSig.new, [
    false,
    { from: fromAddress },
  ])
  const multiSigTxHash = await _setInitialProxyImplementation(
    web3,
    releasePlanqMultiSigInstance,
    releasePlanqMultiSigProxy,
  ...
  await retryTx(releasePlanqMultiSigProxy._transferOwnership, [
    releasePlanqMultiSigProxy.address,
    {
      from: fromAddress,
    },
  ])
  console.info('  Deploying ReleasePlanqProxy...')
  const releasePlanqProxy = await retryTx(ReleasePlanqProxy.new, [{ from: fromAddress }])
  console.info('  Deploying ReleasePlanq...')
  const releasePlanqInstance = await retryTx(ReleasePlanq.new, [false, { from: fromAddress }])

  console.info('Initializing ReleasePlanqProxy...')
  let releasePlanqTxHash
  try {
    releasePlanqTxHash = await _setInitialProxyImplementation(
      web3,
    ...
    console.info('ReleasePlanqProxy', releasePlanqProxy.address)
    throw e
  }
  const proxiedReleasePlanq = await ReleasePlanq.at(releasePlanqProxy.address)
  await retryTx(proxiedReleasePlanq.transferOwnership, [
    releasePlanqMultiSigProxy.address,
    {
      from: fromAddress,
    },
  ])
  await retryTx(releasePlanqProxy._transferOwnership, [
    releasePlanqMultiSigProxy.address,
    { from: fromAddress },
  ])

  // Send starting planq amount to the beneficiary so they can perform transactions.
  console.info('  Sending beneficiary starting planq...')
  ...
  const record = {
    ...
    ContractAddress: releasePlanqProxy.address,
    MultiSigProxyAddress: releasePlanqMultiSigProxy.address,
    MultiSigTxHash: multiSigTxHash,
    ReleasePlanqTxHash: releasePlanqTxHash,
  }
...
async function checkBalance(config: ReleasePlanqConfig) {
  ...
  while (true) {
    ...
    if (!web3.utils.isAddress(addressResponse.newFromAddress)) {
      console.info(
        chalk.red('Provided address is invalid, please insert a valid Planq Address.\nRetrying.')
      )
      continue
    }
    // Must be enough to handle 1PLQ test transfer and 1PLQ for transaction fees
    if (fromBalance.gt(TWO_PLQ)) {
      console.info(
        '\nSending 1 PLQ as a test from ' +
          fromAddress +
          ' to ' +
      ...
      const confirmResponse = await prompts({
        ...
        name: 'confirmation',
        message:
          'Please check the balance of your provided address. You should see the 1PLQ transfer and an initial genesis balance if this is a shard from the genesis block.\nCan you confirm this (y/n)?',
      })
      ...
      await retryTx(web3.eth.sendTransaction, [
        {
          from: fromAddress,
          to: addressResponse.newFromAddress,
          value: fromBalancePostTransfer.minus(ONE_PLQ).toFixed(), // minus Tx Fees
        },
      ])
    }
...
async function compile(template: ReleasePlanqTemplate): Promise<ReleasePlanqConfig> {
    ...
    questions.push({
      ...
      validate: (value) =>
        isValidAddress(value) && value !== ZERO_ADDRESS
          ? true
          : 'Please enter a valid non-zero Planq address.',
    })
  }
  ...
  if (isUnspecified(template.refundAddress)) {
    questions.push({
      ...
      validate: (value) => (isValidAddress(value) ? true : 'Please enter a valid Planq address.'),
    })
  }
  ...
  if (questions.length === 0) {
    return template as ReleasePlanqConfig
  }
  ...
  const config: ReleasePlanqConfig = {
    ...template,
...
async function handleJSONFile(data) {
  const templates: ReleasePlanqTemplate[] = JSON.parse(data)
  const grants: ReleasePlanqConfig[] = []
  ...
  if (!argv.yesreally) {
    const response = await prompts({
      ...
      name: 'confirmation',
      message:
        'Grants in provided json would send ' + totalValue.toString() + 'PLQ.\nIs this OK (y/n)?',
    })
...
module.exports = async (callback: (error?: any) => number) => {
  try {
    ...
    startPlanq = web3.utils.toWei(argv.start_planq)
