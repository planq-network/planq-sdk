 ...
 *   --network atlas --build_artifacts_path ./build/contracts --proposal_path ./proposal.json
 *
...
async function main(buildTargets: BuildOptions) {
  const artifactBasePath =
    buildTargets.buildArtifactsPath || "./build/contracts";
  ...
  const artifacts = artifactPaths.map((a) =>
    require(path.join(process.cwd(), artifactBasePath, a))
  );
  ...
  for (const r of report) {
    ...
    formData.append(
      "files",
      fs.createReadStream(
        artifactBasePath + "/" + artifact.contractName + ".json"
      )
    );
