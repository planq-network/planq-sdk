...
#
# Flags:
# -b: Branch containing smart contracts that currently comprise the Planq protocol
...
# in release v8 and earlier, @planq-network/contractkit automatically uses set RELEASE_TAG
...
if [ -z "$GRANTS_FILE" ]; then
  ...
  yarn devchain generate-tar "$PWD/devchain.tar.gz" --release_planq_contracts $GRANTS_FILE >> $LOG_FILE
fi
