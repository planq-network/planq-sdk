...
# Updates the Planq/Stable token exchange rate to the provided value, or, if a CSV of
# (timestamp, stableValue, planqValue) tuples is provided, updates the exchange rate to the value
# associated with the most recent timestamp less than or equal to the current time stamp.
#
# Flags:
# -f: Filepath to csv of (timestamp, stableValue, planqValue) tuples
# -n: name of the network defined in truffle-config.js to set the exchange rate on
# -s: StableToken component of exchange rate
# -g: PlanqToken component of exchange rate
...
yarn run truffle exec ./scripts/truffle/set_exchange_rate.js \
  --network $NETWORK --stableValue $STABLE_VALUE --planqValue $PLANQ_VALUE \
