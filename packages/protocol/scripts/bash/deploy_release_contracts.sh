...
# -s: (Optional) Amount of planq for beneficiary to start with for transactions
...
#
# Example:
# `./scripts/bash/deploy_release_contracts.sh -n development -f 0x5409ED021D9299bf6814279A6A1411A7e866A631 -g scripts/truffle/releasePlanqExampleConfigs.json`
...
[ -z "$START_PLANQ" ] && echo "No starting planq provided via -s flag: defaulting to 1cGld" && START_PLANQ=1;
...
yarn run truffle exec ./scripts/truffle/deploy_release_contracts.js \
  --network $NETWORK --from $FROM --grants $GRANTS_FILE --start_planq $START_PLANQ --deployed_grants $DEPLOYED_GRANTS --output_file $OUTPUT_FILE $REALLY --build_directory $CONTRACT_ARTIFACTS_DIR \
