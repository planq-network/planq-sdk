    ...
    UsingPrecompiles
{
    ...
    function initialize(
        ...
        require(
            attestationRequestFeeTokens.length > 0 &&
                attestationRequestFeeTokens.length ==
                attestationRequestFeeValues.length,
            "attestationRequestFeeTokens specification was invalid"
        );
        for (
            uint256 i = 0;
            i < attestationRequestFeeTokens.length;
            i = i.add(1)
        ) {
            setAttestationRequestFee(
                attestationRequestFeeTokens[i],
                attestationRequestFeeValues[i]
            );
        }
    }
   ...
   */
    function revoke(bytes32 identifier, uint256 index) external {
        ...
        if (index != newNumAccounts) {
            identifiers[identifier].accounts[index] = identifiers[identifier]
                .accounts[newNumAccounts];
        }
        identifiers[identifier].accounts[newNumAccounts] = address(0x0);
        identifiers[identifier].accounts.length = identifiers[identifier]
            .accounts
            .length
            .sub(1);
    }
   ...
   */
        ...
        returns (uint32, uint32, address)
    {
        return (
            ...
            identifiers[identifier].unselectedRequests[account]
   ...
   */
    ...
    {
        require(
            identifiersToLookup.length > 0,
            "You have to pass at least one identifier"
        );
        ...
        for (uint256 i = 0; i < identifiersToLookup.length; i = i.add(1)) {
            address[] memory addrs = identifiers[identifiersToLookup[i]]
                .accounts;
            for (
                ...
                addresses[currentIndex] = getAccounts().getWalletAddress(
                    addrs[matchIndex]
                );
   ...
   */
    ...
    */
    ...
    {
        AttestedAddress storage state = identifiers[identifier]
            .attestations[account];
        ...
        for (uint256 i = 0; i < issuers.length; i = i.add(1)) {
            if (
                isAttestationCompletable(state.issuedAttestations[issuers[i]])
            ) {
                num = num.add(1);
            }
        }
        ...
        for (uint256 i = 0; i < issuers.length; i = i.add(1)) {
            if (
                ...
                blockNumbers[pointer] = state.issuedAttestations[issuers[i]]
                    .blockNumber;
        ...
        (stringLengths, stringData) = getAccounts().batchGetMetadataURL(
            completableIssuers
        );
   ...
   * @return The fee.
   */
    function getAttestationRequestFee(address token)
        external
        view
        returns (uint256)
    {
   ...
   */
    function setAttestationRequestFee(address token, uint256 fee)
        public
        onlyOwner
    {
   ...
   * an attestation and completing it.
   */
    function setAttestationExpiryBlocks(uint256 _attestationExpiryBlocks)
        public
        onlyOwner
    {
        require(
            _attestationExpiryBlocks > 0,
            "attestationExpiryBlocks has to be greater than 0"
        );
   ...
   *                                 requests.
   */
    function setSelectIssuersWaitBlocks(uint256 _selectIssuersWaitBlocks)
        public
        onlyOwner
    {
        require(
            _selectIssuersWaitBlocks > 0,
            "selectIssuersWaitBlocks has to be greater than 0"
        );
   ...
   */
    function setMaxAttestations(uint256 _maxAttestations) public onlyOwner {
        require(
            _maxAttestations > 0,
            "maxAttestations has to be greater than 0"
        );
   ...
   */
    function requireNAttestationsRequested(
        ...
        uint256 requested = identifiers[identifier].attestations[account]
            .requested;
        require(
            requested == expected,
            "requested attestations does not match expected"
        );
    }
   ...
   */
    ...
    function isAttestationExpired(uint32 attestationRequestBlock)
        internal
        view
        returns (bool)
    {
        return
            block.number >=
            uint256(attestationRequestBlock).add(attestationExpiryBlocks);
    }

    function isAttestationCompletable(Attestation storage attestation)
        internal
        view
        returns (bool)
    {
        ...
        returns (bool)
    {
        return
            block.number <
            attestationRequestBlock.add(
                getRandom().randomnessBlockRetentionWindow()
            );
    }
}
