    ...
    CalledByVm
{
  ...
  */
   ...
   */
    function initialize(uint256 _randomnessBlockRetentionWindow)
        external
        initializer
    {
   ...
   */
    function _revealAndCommit(
        ...
        require(
            newCommitment != computeCommitment(0),
        ...
        if (commitments[proposer] != 0) {
            require(
                randomness != 0,
                "randomness cannot be zero if there is a previous commitment"
            );
            ...
            require(
                randomness == 0,
                "randomness should be zero if there is no previous commitment"
            );
        }
   ...
   */
    function computeCommitment(bytes32 randomness)
        public
        pure
        returns (bytes32)
    {
   ...
   */
    function getBlockRandomness(uint256 blockNumber)
        external
        view
        returns (bytes32)
    {
   ...
   * @return The associated randomness value.
   */
    function _getBlockRandomness(uint256 blockNumber, uint256 cur)
        internal
        view
        returns (bytes32)
    {
