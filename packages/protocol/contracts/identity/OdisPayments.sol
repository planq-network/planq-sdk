    ...
    UsingRegistryV2
{
    ...
    event PaymentMade(address indexed account, uint256 valueInAUSD);

    // Store amount sent (all time) from account to this contract.
    // Values in totalPaidAUSD should only ever be incremented, since ODIS relies
    // on all-time paid balance to compute every quota.
    mapping(address => uint256) public totalPaidAUSD;
   ...
   */
    function payInAUSD(address account, uint256 value) external nonReentrant {
        IERC20(registryContract.getAddressForOrDie(STABLE_TOKEN_REGISTRY_ID))
            .safeTransferFrom(msg.sender, address(this), value);
        totalPaidAUSD[account] = totalPaidAUSD[account].add(value);
