    ...
    UsingRegistryV2BackwardsCompatible
{
   ...
   */
    function addDefaultTrustedIssuer(address trustedIssuer) external onlyOwner {
        ...
        require(
            defaultTrustedIssuers.length.add(1) <=
                MAX_TRUSTED_ISSUERS_PER_PAYMENT,
   ...
   */
    function removeDefaultTrustedIssuer(address trustedIssuer, uint256 index)
        external
        onlyOwner
    {
        ...
        if (index != numDefaultTrustedIssuers - 1) {
            ...
            defaultTrustedIssuers[index] = defaultTrustedIssuers[numDefaultTrustedIssuers -
                1];
        }
  ...
  */
        ...
        returns (bool)
    {
        ...
        require(
            payment.token != address(0) && payment.value > 0,
        ...
        if (payment.minAttestations > 0) {
            ...
            address attestationsAddress = registryContract.getAddressForOrDie(
                ATTESTATIONS_REGISTRY_ID
            );
  ...
  */
    function getReceivedPaymentIds(bytes32 identifier)
        external
        view
  ...
  * specified sender.
  */
    function getSentPaymentIds(address sender)
        external
        view
        returns (address[] memory)
    {
  ...
  */
    function getTrustedIssuersPerPayment(address paymentId)
        external
        view
        returns (address[] memory)
    {
  ...
  */
    function hasCompletedV2Attestations(
        ...
        (, address[] memory accounts, , , ) = federatedAttestations
            .lookupAttestations(identifier, trustedIssuers);
  ...
  * @dev msg.sender needs to have already approved this contract to transfer.
   */
    function _transfer(
        ...
        require(
            token != address(0) && value > 0 && expirySeconds > 0,
        ...
        uint256 receivedIndex = receivedPaymentIds[identifier]
            .push(paymentId)
            .sub(1);
  ...
  */
    function deletePayment(address paymentId) private {
        ...
        address[] storage received = receivedPaymentIds[payment
            .recipientIdentifier];
        address[] storage sent = sentPaymentIds[payment.sender];

        escrowedPayments[received[received.length - 1]].receivedIndex = payment
            .receivedIndex;
