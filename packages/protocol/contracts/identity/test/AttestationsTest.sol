...
contract AttestationsTest is Attestations(true) {
    ...
    function request(
        ...
        require(
            IERC20(attestationRequestFeeToken).transferFrom(
                msg.sender,
                address(this),
                attestationRequestFees[attestationRequestFeeToken].mul(
        ...
        require(
            ...
            "You have to request at least 1 attestation"
        );
        require(
            attestationsRequested <= maxAttestations,
            "Too many attestations requested"
        );
        ...
        require(
            state.unselectedRequests[msg.sender].blockNumber == 0 ||
                ...
                !isAttestationRequestSelectable(
                    state.unselectedRequests[msg.sender].blockNumber
                ),
            "There exists an unexpired, unselected attestation request"
        );

        state.unselectedRequests[msg.sender].blockNumber = block
            .number
            .toUint32();
        state.unselectedRequests[msg.sender]
            .attestationsRequested = attestationsRequested.toUint32();
        state.unselectedRequests[msg.sender]
            .attestationRequestFeeToken = attestationRequestFeeToken;
   ...
   */
    function complete(bytes32 identifier, uint8 v, bytes32 r, bytes32 s)
        external
    {
        ...
        AttestedAddress storage attestedAddress = identifiers[identifier]
            .attestations[msg.sender];
        require(
        ...
        pendingWithdrawals[token][issuer] = pendingWithdrawals[token][issuer]
            .add(attestationRequestFees[token]);
   ...
   */
    function addIncompleteAttestations(bytes32 identifier) internal {
        AttestedAddress storage state = identifiers[identifier].attestations[msg
            .sender];
        UnselectedRequest storage unselectedRequest = identifiers[identifier]
        ...
        require(
            unselectedRequest.attestationsRequested <= issuersLength,
            "not enough issuers"
        );
        ...
        while (currentIndex < unselectedRequest.attestationsRequested) {
            ...
            if (
                ...
                attestation.attestationRequestFeeToken = unselectedRequest
                    .attestationRequestFeeToken;
