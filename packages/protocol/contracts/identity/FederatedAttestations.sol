    ...
    UsingRegistryV2
{
   ...
   */
        ...
        )
    {
        ...
        for (uint256 i = 0; i < trustedIssuers.length; i = i.add(1)) {
            ...
            for (
                uint256 j = 0;
                j < attestationsPerIssuer.length;
                j = j.add(1)
            ) {
                accounts[totalAttestations] = attestationsPerIssuer[j].account;
                signers[totalAttestations] = attestationsPerIssuer[j].signer;
                issuedOns[totalAttestations] = attestationsPerIssuer[j]
                    .issuedOn;
                publishedOns[totalAttestations] = attestationsPerIssuer[j]
                    .publishedOn;
   ...
   */
        ...
        returns (uint256[] memory countsPerIssuer, bytes32[] memory identifiers)
    {
        ...
        (totalIdentifiers, countsPerIssuer) = getNumIdentifiers(
            account,
            trustedIssuers
        );
   ...
   */
    function validateAttestationSig(
        ...
        bytes32 structHash = getUniqueAttestationHash(
            identifier,
            issuer,
            account,
            signer,
    ...
    /**
   * @notice Sets the EIP712 domain separator for the Planq FederatedAttestations abstraction.
   */
   ...
   */
        ...
        returns (uint256 totalAttestations, uint256[] memory countsPerIssuer)
    {
        ...
        for (uint256 i = 0; i < trustedIssuers.length; i = i.add(1)) {
            numAttestationsForIssuer = identifierToAttestations[identifier][trustedIssuers[i]]
                .length;
   ...
   */
        ...
        returns (uint256 totalIdentifiers, uint256[] memory countsPerIssuer)
    {
        ...
        for (uint256 i = 0; i < trustedIssuers.length; i = i.add(1)) {
            numIdentifiersForIssuer = addressToIdentifiers[account][trustedIssuers[i]]
                .length;
   ...
   */
    function _registerAttestation(
        ...
        require(
            !revokedAttestations[getUniqueAttestationHash(
                identifier,
                issuer,
                account,
                signer,
        ...
        uint256 numExistingAttestations = identifierToAttestations[identifier][issuer]
            .length;
        require(
        ...
        require(
            addressToIdentifiers[account][issuer].length.add(1) <=
        ...
        for (uint256 i = 0; i < numExistingAttestations; i = i.add(1)) {
            ...
            require(
                identifierToAttestations[identifier][issuer][i].account !=
                    account,
