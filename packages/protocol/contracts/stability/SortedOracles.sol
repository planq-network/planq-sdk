...
/**
 * @title Maintains a sorted list of oracle exchange rates between PLQ and other currencies.
 */
    ...
    Initializable
{
   ...
   */
    function setReportExpiry(uint256 _reportExpirySeconds) public onlyOwner {
        ...
        require(
            _reportExpirySeconds != reportExpirySeconds,
            "reportExpirySeconds hasn't changed"
        );
   ...
   * @param token The address of the token for which the PLQ exchange rate is being reported.
   ...
   */
    function removeExpiredReports(address token, uint256 n) external {
        ...
        for (uint256 i = 0; i < n; i = i.add(1)) {
            (bool isExpired, address oldestAddress) = isOldestReportExpired(
                token
            );
            if (isExpired) {
   ...
   * @param token The address of the token for which the PLQ exchange rate is being reported.
   * @param value The amount of `token` equal to one PLQ, expressed as a fixidity value.
   ...
   */
        ...
        returns (uint256, uint256)
    {
        return (
            ...
            numRates(token) == 0 ? 0 : FIXED1_UINT
        );
    }
   ...
   */
    function reportExists(address token, address oracle)
        internal
        view
        returns (bool)
    {
        return
            rates[token].contains(oracle) && timestamps[token].contains(oracle);
    }
   ...
   */
    function getOracles(address token)
        external
        view
   ...
   */
    function getTokenReportExpirySeconds(address token)
        public
        view
        returns (uint256)
    {
