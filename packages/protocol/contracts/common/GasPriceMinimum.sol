    ...
    CalledByVm
{
   ...
   */
    function setAdjustmentSpeed(uint256 _adjustmentSpeed) public onlyOwner {
        ...
        require(
            adjustmentSpeed.lt(FixidityLib.fixed1()),
   ...
   */
    function setTargetDensity(uint256 _targetDensity) public onlyOwner {
        ...
        require(
            targetDensity.lt(FixidityLib.fixed1()),
   ...
   * @param tokenAddress The currency the gas price should be in (defaults to planq).
   * @return current gas price minimum in the requested currency
   */
    function getGasPriceMinimum(address tokenAddress)
        external
        view
        returns (uint256)
    {
        if (
            ...
            tokenAddress == registry.getAddressForOrDie(PLANQ_TOKEN_REGISTRY_ID)
        ) {
            ...
            (rateNumerator, rateDenominator) = sortedOracles.medianRate(
                tokenAddress
            );
   ...
   */
    function updateGasPriceMinimum(uint256 blockGasTotal, uint256 blockGasLimit)
        ...
        returns (uint256)
    {
        gasPriceMinimum = getUpdatedGasPriceMinimum(
            blockGasTotal,
            blockGasLimit
        );
    ...
    function getUpdatedGasPriceMinimum(
        ...
        FixidityLib.Fraction memory adjustment = densityGreaterThanTarget
            ...
            : FixidityLib.fixed1().subtract(
                adjustmentSpeed.multiply(densityDelta)
            );
        ...
        return
            newGasPriceMinimum >= gasPriceMinimumFloor
                ? newGasPriceMinimum
