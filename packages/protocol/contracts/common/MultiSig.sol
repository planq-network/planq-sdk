...
contract MultiSig is Initializable {
   ...
   */
    ...
    modifier notExecuted(uint256 transactionId) {
        require(
            ...
            "transaction was executed already"
        );
        _;
    }
   ...
   */
    ...
    function removeOwner(address owner) external onlyWallet ownerExists(owner) {
        ...
        if (internalRequired > owners.length)
            changeInternalRequirement(owners.length);
    ...
    function isConfirmed(uint256 transactionId) public view returns (bool) {
        ...
        for (uint256 i = 0; i < owners.length; i = i.add(1)) {
            ...
            bool isInternal = transactions[transactionId].destination ==
                address(this);
            if (
                (isInternal && count == internalRequired) ||
                (!isInternal && count == required)
            ) return true;
        }
        return false;
    }
