...
import "../governance/interfaces/ILockedPlanq.sol";
...
import "../common/interfaces/IExchange.sol";
...
contract UsingRegistryV2 {
    ...
    bytes32 internal constant ATTESTATIONS_REGISTRY_ID = keccak256(
    ...
    bytes32 internal constant ELECTION_REGISTRY_ID = keccak256(
        abi.encodePacked("Election")
    );
    bytes32 internal constant EXCHANGE_REGISTRY_ID = keccak256(
        abi.encodePacked("Exchange")
    );
    bytes32 internal constant EXCHANGE_EURO_REGISTRY_ID = keccak256(
        abi.encodePacked("ExchangeEUR")
    );
    bytes32 internal constant EXCHANGE_REAL_REGISTRY_ID = keccak256(
        abi.encodePacked("ExchangeBRL")
    );
    ...
    bytes32 internal constant FREEZER_REGISTRY_ID = keccak256(
        abi.encodePacked("Freezer")
    );
    bytes32 internal constant PLANQ_TOKEN_REGISTRY_ID = keccak256(
        abi.encodePacked("PlanqToken")
    );
    bytes32 internal constant GOVERNANCE_REGISTRY_ID = keccak256(
        abi.encodePacked("Governance")
    );
    bytes32 internal constant GOVERNANCE_SLASHER_REGISTRY_ID = keccak256(
        abi.encodePacked("GovernanceSlasher")
    );
    bytes32 internal constant LOCKED_PLANQ_REGISTRY_ID = keccak256(
        abi.encodePacked("LockedPlanq")
    );
    bytes32 internal constant RESERVE_REGISTRY_ID = keccak256(
        abi.encodePacked("Reserve")
    );
    bytes32 internal constant RANDOM_REGISTRY_ID = keccak256(
        abi.encodePacked("Random")
    );
    bytes32 internal constant SORTED_ORACLES_REGISTRY_ID = keccak256(
        abi.encodePacked("SortedOracles")
    );
    bytes32 internal constant STABLE_TOKEN_REGISTRY_ID = keccak256(
    ...
    bytes32 internal constant VALIDATORS_REGISTRY_ID = keccak256(
    ...
    modifier onlyRegisteredContracts(bytes32[] memory identifierHashes) {
        require(
            registryContract.isOneOf(identifierHashes, msg.sender),
    ...
    function getAccounts() internal view returns (IAccounts) {
        return
            IAccounts(
                registryContract.getAddressForOrDie(ACCOUNTS_REGISTRY_ID)
            );
    }
    ...
    function getAttestations() internal view returns (IAttestations) {
        return
            IAttestations(
                registryContract.getAddressForOrDie(ATTESTATIONS_REGISTRY_ID)
            );
    }
    ...
    function getElection() internal view returns (IElection) {
        return
            IElection(
                registryContract.getAddressForOrDie(ELECTION_REGISTRY_ID)
            );
    }
    ...
    function getExchange() internal view returns (IExchange) {
        return
            IExchange(
                registryContract.getAddressForOrDie(EXCHANGE_REGISTRY_ID)
            );
    }
    ...
    function getExchangeEuro() internal view returns (IExchange) {
        return
            IExchange(
                registryContract.getAddressForOrDie(EXCHANGE_EURO_REGISTRY_ID)
            );
    }
    ...
    function getExchangeREAL() internal view returns (IExchange) {
        return
            IExchange(
                registryContract.getAddressForOrDie(EXCHANGE_REAL_REGISTRY_ID)
            );
    }
        ...
        return
            IFeeCurrencyWhitelist(
                registryContract.getAddressForOrDie(
                    FEE_CURRENCY_WHITELIST_REGISTRY_ID
                )
            );
    }

    function getFederatedAttestations()
        internal
        view
        returns (IFederatedAttestations)
    {
        return
            IFederatedAttestations(
                registryContract.getAddressForOrDie(
                    FEDERATED_ATTESTATIONS_REGISTRY_ID
                )
            );
    }
    ...
    function getFreezer() internal view returns (IFreezer) {
        return
            IFreezer(registryContract.getAddressForOrDie(FREEZER_REGISTRY_ID));
    }

    function getPlanqToken() internal view returns (IERC20) {
        return
            IERC20(
                registryContract.getAddressForOrDie(PLANQ_TOKEN_REGISTRY_ID)
            );
    }
    ...
    function getGovernance() internal view returns (IGovernance) {
        return
            IGovernance(
                registryContract.getAddressForOrDie(GOVERNANCE_REGISTRY_ID)
            );
    }

    function getLockedPlanq() internal view returns (ILockedPlanq) {
        return
            ILockedPlanq(
                registryContract.getAddressForOrDie(LOCKED_PLANQ_REGISTRY_ID)
            );
    }
    ...
    function getReserve() internal view returns (IReserve) {
        return
            IReserve(registryContract.getAddressForOrDie(RESERVE_REGISTRY_ID));
    }

    function getSortedOracles() internal view returns (ISortedOracles) {
        return
            ISortedOracles(
                registryContract.getAddressForOrDie(SORTED_ORACLES_REGISTRY_ID)
            );
    }
    ...
    function getValidators() internal view returns (IValidators) {
        return
            IValidators(
                registryContract.getAddressForOrDie(VALIDATORS_REGISTRY_ID)
            );
    }
}
