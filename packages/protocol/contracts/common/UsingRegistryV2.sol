...
contract UsingRegistryV2 {
  ...
  bytes32 internal constant PLANQ_TOKEN_REGISTRY_ID = keccak256(abi.encodePacked("PlanqToken"));
  ...
  bytes32 internal constant LOCKED_PLANQ_REGISTRY_ID = keccak256(abi.encodePacked("LockedPlanq"));
  ...
  function getPlanqToken() internal view returns (IERC20) {
    return IERC20(registryContract.getAddressForOrDie(PLANQ_TOKEN_REGISTRY_ID));
  }
  ...
  function getLockedPlanq() internal view returns (ILockedPlanq) {
    return ILockedPlanq(registryContract.getAddressForOrDie(LOCKED_PLANQ_REGISTRY_ID));
  }
