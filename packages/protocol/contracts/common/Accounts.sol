    ...
    UsingRegistry
{
    ...
    bytes32 constant ValidatorSigner = keccak256(
        abi.encodePacked("planq.network/core/validator")
    );
    bytes32 constant AttestationSigner = keccak256(
        abi.encodePacked("planq.network/core/attestation")
    );
    bytes32 constant VoteSigner = keccak256(
        abi.encodePacked("planq.network/core/vote")
    );
    ...
    event SignerAuthorizationStarted(
        address indexed account,
        address signer,
        bytes32 indexed role
    );
    event SignerAuthorizationCompleted(
        address indexed account,
        address signer,
        bytes32 indexed role
    );
    ...
    event IndexedSignerSet(
        address indexed account,
        address signer,
        bytes32 role
    );
    event IndexedSignerRemoved(
        address indexed account,
        address oldSigner,
        bytes32 role
    );
    event DefaultSignerSet(
        address indexed account,
        address signer,
        bytes32 role
    );
    event DefaultSignerRemoved(
        address indexed account,
        address oldSigner,
        bytes32 role
    );
    event LegacySignerSet(
        address indexed account,
        address signer,
        bytes32 role
    );
    event LegacySignerRemoved(
        address indexed account,
        address oldSigner,
        bytes32 role
    );
   ...
   */
    ...
    /**
   * @notice Sets the EIP712 domain separator for the Planq Accounts abstraction.
   */
    function setEip712DomainSeparator() public {
        ...
        eip712DomainSeparator = keccak256(
            abi.encode(
                ...
                keccak256(bytes("Planq Core Contracts")),
                keccak256("1.0"),
                chainId,
                address(this)
            )
   ...
   */
    function removeStorageRoot(uint256 index) external {
        require(isAccount(msg.sender), "Unknown account");
        require(
            index < offchainStorageRoots[msg.sender].length,
        ...
        offchainStorageRoots[msg.sender][index] = offchainStorageRoots[msg
            .sender][lastIndex];
   ...
   */
        ...
        returns (bytes memory, uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < numberRoots; i++) {
            totalLength = totalLength.add(
                offchainStorageRoots[account][i].length
            );
        }
   ...
   */
    function setPaymentDelegation(address beneficiary, uint256 fraction)
        public
    {
        require(
            isAccount(msg.sender),
   ...
   */
    function deletePaymentDelegation() public {
        ...
        paymentDelegations[msg.sender] = PaymentDelegation(
            address(0x0),
            FixidityLib.wrap(0)
        );
   ...
   */
    function getPaymentDelegation(address account)
        external
        view
   ...
   */
    function setIndexedSigner(address signer, bytes32 role) public {
        ...
        require(
            isSigner(msg.sender, signer, role),
   ...
   */
    function authorizeValidatorSigner(
        ...
        require(
            !getValidators().isValidator(msg.sender),
            "Cannot authorize validator signer"
        );
   ...
   */
    function authorizeValidatorSignerWithPublicKey(
        ...
        require(
            getValidators().updateEcdsaPublicKey(
                msg.sender,
                signer,
                ecdsaPublicKey
            ),
   ...
   */
    function authorizeValidatorSignerWithKeys(
        ...
        require(
            getValidators().updatePublicKeys(
                msg.sender,
                signer,
                ecdsaPublicKey,
                blsPublicKey,
   ...
   */
    function authorizeSigner(address signer, bytes32 role) public {
        require(
            isAccount(msg.sender),
            "Must first register sender with Account.createAccount"
        );
        require(
            isNotAccount(signer) &&
                isNotAuthorizedSignerForAnotherAccount(msg.sender, signer),
    ...
    function completeSignerAuthorization(address account, bytes32 role) public {
        ...
        require(
            isNotAccount(msg.sender) &&
                isNotAuthorizedSignerForAnotherAccount(account, msg.sender),
            "Cannot re-authorize address signer"
        );
        require(
   ...
   */
        ...
        returns (bool)
    {
        ...
        if (role == ValidatorSigner && account.signers.validator == signer) {
            ...
            role == AttestationSigner && account.signers.attestation == signer
        ) {
            return true;
   ...
   * @param role The role that has been authorized.
   */
    function isSigner(address account, address signer, bytes32 role)
        public
        view
        returns (bool)
    {
   ...
   */
    ...
    function signerToAccountWithRole(address signer, bytes32 role)
        internal
        view
        returns (address)
    {
   ...
   * @return The associated account.
   */
    function voteSignerToAccount(address signer)
        external
        view
        returns (address)
    {
   ...
   */
    function isLegacyRole(bytes32 role) public pure returns (bool) {
        return
            role == VoteSigner ||
            role == ValidatorSigner ||
            role == AttestationSigner;
    }
   ...
   * @param role The role of the signer.
   */
    function getLegacySigner(address _account, bytes32 role)
        public
        view
        returns (address)
    {
   ...
   * @param role The role of the signer.
   */
    function getDefaultSigner(address account, bytes32 role)
        public
        view
        returns (address)
    {
   ...
   * @param role The role of the signer.
   */
    function getIndexedSigner(address account, bytes32 role)
        public
        view
        returns (address)
    {
        return
            isLegacyRole(role)
                ? getLegacySigner(account, role)
                : getDefaultSigner(account, role);
    }
   ...
   */
    function hasLegacySigner(address account, bytes32 role)
        public
        view
        returns (bool)
    {
   ...
   */
    function hasDefaultSigner(address account, bytes32 role)
        public
        view
        returns (bool)
    {
   ...
   */
    function hasIndexedSigner(address account, bytes32 role)
        public
        view
        returns (bool)
    {
        return
            isLegacyRole(role)
                ? hasLegacySigner(account, role)
                : hasDefaultSigner(account, role);
    }
   ...
   */
    function hasAuthorizedSigner(address account, string calldata role)
        external
        view
        returns (bool)
    {
   ...
   */
    function hasAuthorizedVoteSigner(address account)
        external
        view
        returns (bool)
    {
   ...
   */
    function hasAuthorizedValidatorSigner(address account)
        external
        view
        returns (bool)
    {
   ...
   */
    function hasAuthorizedAttestationSigner(address account)
        external
        view
        returns (bool)
    {
   ...
   */
    function getMetadataURL(address account)
        external
        view
        ...
        returns (uint256[] memory, bytes memory)
    {
        ...
        for (uint256 i = 0; i < accountsToQuery.length; i = i.add(1)) {
            for (uint256 j = 0; j < sizes[i]; j = j.add(1)) {
                data[pointer] = bytes(
                    accounts[accountsToQuery[i]].metadataURL
                )[j];
   ...
   */
    function getDataEncryptionKey(address account)
        external
        view
   ...
   */
    function isNotAuthorizedSigner(address signer)
        internal
        view
        returns (bool)
    {
   ...
   */
    function authorizeAddress(address authorized, uint8 v, bytes32 r, bytes32 s)
        private
    {
        address signer = Signatures.getSignerOfAddress(msg.sender, v, r, s);
   ...
   */
    function getRoleAuthorizationSigner(
        ...
        return
            Signatures.getSignerOfTypedDataHash(
   ...
   */
    function authorizeAddressWithRole(
        ...
        address signer = getRoleAuthorizationSigner(
            msg.sender,
            authorized,
            role,
            v,
   ...
   */
    function authorize(address authorized) private {
        ...
        require(
            isNotAccount(authorized) &&
                isNotAuthorizedSignerForAnotherAccount(msg.sender, authorized),
