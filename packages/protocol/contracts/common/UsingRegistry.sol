...
contract UsingRegistry is Ownable {
    ...
    bytes32 constant PLANQ_TOKEN_REGISTRY_ID = keccak256(
    ...
    bytes32 constant LOCKED_PLANQ_REGISTRY_ID = keccak256(
    ...
    modifier onlyRegisteredContract(bytes32 identifierHash) {
        require(
            registry.getAddressForOrDie(identifierHash) == msg.sender,
    ...
    modifier onlyRegisteredContracts(bytes32[] memory identifierHashes) {
        require(
            registry.isOneOf(identifierHashes, msg.sender),
   ...
   */
    ...
    function getLockedPlanq() internal view returns (ILockedPlanq) {
        return
            ILockedPlanq(registry.getAddressForOrDie(LOCKED_PLANQ_REGISTRY_ID));
    }
