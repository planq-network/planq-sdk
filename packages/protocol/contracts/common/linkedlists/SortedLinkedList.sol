...
library SortedLinkedList {
   ...
   */
    function insert(
        ...
        require(
            (lesserKey != bytes32(0) || greaterKey != bytes32(0)) ||
                list.list.numElements == 0,
            "greater and lesser key zero"
        );
        require(
            contains(list, lesserKey) || lesserKey == bytes32(0),
            "invalid lesser key"
        );
        require(
            contains(list, greaterKey) || greaterKey == bytes32(0),
            "invalid greater key"
        );
   ...
   */
    function popN(List storage list, uint256 n)
        internal
   ...
   */
    function contains(List storage list, bytes32 key)
        internal
        view
        returns (bool)
    {
   ...
   */
    function getValue(List storage list, bytes32 key)
        internal
        view
        returns (uint256)
    {
   ...
   */
    function getKeys(List storage list)
        internal
        view
        returns (bytes32[] memory)
    {
   ...
   */
    function headN(List storage list, uint256 n)
        internal
        view
        returns (bytes32[] memory)
    {
   ...
   */
    function getLesserAndGreater(
        ...
        if (
            lesserKey == bytes32(0) &&
            isValueBetween(list, value, lesserKey, list.list.tail)
        ) {
            return (lesserKey, list.list.tail);
        } else if (
            greaterKey == bytes32(0) &&
            isValueBetween(list, value, list.list.head, greaterKey)
        ) {
            ...
            isValueBetween(
                ...
                value,
                lesserKey,
                list.list.elements[lesserKey].nextKey
            )
        ) {
            ...
            isValueBetween(
                list,
                value,
                list.list.elements[greaterKey].previousKey,
                greaterKey
            )
        ) {
   ...
   */
    function isValueBetween(
        ...
        bool isLesser = lesserKey == bytes32(0) ||
            list.values[lesserKey] <= value;
        bool isGreater = greaterKey == bytes32(0) ||
            list.values[greaterKey] >= value;
