...
/**
 * @title A library operating on Planq Governance proposals.
 */
library Proposals {
   ...
   */
    ...
    function setDescriptionUrl(
        ...
        require(
            bytes(descriptionUrl).length != 0,
            "Description url must have non-zero length"
        );
   ...
   */
    function updateVote(
        ...
        proposal.votes.abstain = proposal.votes.abstain.sub(
            previousAbstainVotes
        );
   ...
   */
    ...
    {
        uint256 totalVotes = proposal.votes.yes.add(proposal.votes.no).add(
   ...
   */
    ...
    {
        require(
            index < proposal.transactions.length,
            "getTransaction: bad index"
        );
   ...
   */
    function isApproved(Proposal storage proposal)
        internal
        view
        returns (bool)
    {
   ...
   * @param value The value of PLQ to be sent with the function call.
   ...
   */
    function externalCall(
        ...
        if (dataLength > 0)
            require(
                Address.isContract(destination),
