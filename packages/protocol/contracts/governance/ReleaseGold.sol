...
contract ReleasePlanq is UsingRegistry, ReentrancyGuard, IReleasePlanq, Initializable {
  ...
  event ReleasePlanqInstanceCreated(address indexed beneficiary, address indexed atAddress);
  event ReleaseScheduleRevoked(uint256 revokeTimestamp, uint256 releasedBalanceAtRevoke);
  event ReleasePlanqInstanceDestroyed(address indexed beneficiary, address indexed atAddress);
  ...
  modifier onlyCanVote() {
    require(canVote, "Release Planq contract does not have permission to vote");
    _;
  }

  modifier onlyCanValidate() {
    require(canValidate, "Release Planq contract does not have permission to validate");
    _;
  }
  ...
  function setMaxDistribution(uint256 distributionRatio) external onlyReleaseOwner {
    ...
    if (distributionRatio == 1000) {
      ...
      require(totalBalance > 0, "Do not set max distribution before factory sends the planq");
  ...
  function refundAndFinalize() external nonReentrant onlyReleaseOwnerAndRevoked {
    require(getRemainingLockedBalance() == 0, "Total planq balance must be unlocked");
  ...
  function getRemainingLockedBalance() public view returns (uint256) {
    if (getAccounts().isAccount(address(this))) {
      ...
      uint256 pendingWithdrawalSum = lockedPlanq.getTotalPendingWithdrawals(address(this));
      return lockedPlanq.getAccountTotalLockedPlanq(address(this)).add(pendingWithdrawalSum);
    }
    return 0;
  }
   ...
   */
  function lockPlanq(uint256 value) external nonReentrant onlyBeneficiaryAndNotRevoked {
   ...
   */
  function unlockPlanq(uint256 value) external nonReentrant onlyWhenInProperState {
   ...
   */
  function withdrawLockedPlanq(uint256 index) external nonReentrant onlyWhenInProperState {
  ...
  function fundSigner(address payable signer) private {
    ...
    require(address(this).balance >= value, "no available PLQ to fund signer");
