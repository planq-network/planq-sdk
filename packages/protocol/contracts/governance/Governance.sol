    ...
    UsingPrecompiles
{
   ...
   * @param _minDeposit The minimum PLQ deposit needed to make a proposal.
   ...
   */
    function setConcurrentProposals(uint256 _concurrentProposals)
        public
        onlyOwner
    {
        require(
        ...
        require(
            _concurrentProposals != concurrentProposals,
   ...
   */
    function setDequeueFrequency(uint256 _dequeueFrequency) public onlyOwner {
        ...
        require(
            _dequeueFrequency != dequeueFrequency,
   ...
   */
    function setReferendumStageDuration(uint256 referendumStageDuration)
        public
        onlyOwner
    {
        require(referendumStageDuration > 0, "Duration must be larger than 0");
        require(
            referendumStageDuration != stageDurations.referendum,
            "Duration unchanged"
        );
   ...
   */
    function setExecutionStageDuration(uint256 executionStageDuration)
        public
        onlyOwner
    {
        require(executionStageDuration > 0, "Duration must be larger than 0");
        require(
            executionStageDuration != stageDurations.execution,
            "Duration unchanged"
        );
   ...
   */
    function setParticipationBaseline(uint256 participationBaseline)
        public
        onlyOwner
    {
        FixidityLib.Fraction memory participationBaselineFrac = FixidityLib
            .wrap(participationBaseline);
        require(
   ...
   */
    function setParticipationFloor(uint256 participationFloor)
        public
        onlyOwner
    {
        FixidityLib.Fraction memory participationFloorFrac = FixidityLib.wrap(
            participationFloor
        );
        require(
   ...
   */
    function setBaselineUpdateFactor(uint256 baselineUpdateFactor)
        public
        onlyOwner
    {
        FixidityLib.Fraction memory baselineUpdateFactorFrac = FixidityLib.wrap(
        ...
        require(
            !baselineUpdateFactorFrac.equals(
   ...
   */
    function setBaselineQuorumFactor(uint256 baselineQuorumFactor)
        public
        onlyOwner
    {
        FixidityLib.Fraction memory baselineQuorumFactorFrac = FixidityLib.wrap(
        ...
        require(
            !baselineQuorumFactorFrac.equals(
   ...
   */
    function setConstitution(
        ...
        require(
            threshold > FIXED_HALF &&
                threshold <= FixidityLib.fixed1().unwrap(),
            "Threshold has to be greater than majority and not greater than unanimity"
        );
        if (functionId == 0) {
            constitution[destination].defaultThreshold = FixidityLib.wrap(
                threshold
            );
        } else {
            constitution[destination]
                .functionThresholds[functionId] = FixidityLib.wrap(threshold);
        }
   ...
   * @param values The values of PLQ to be sent in the proposed transactions.
   ...
   */
    function removeIfQueuedAndExpired(uint256 proposalId)
        private
        returns (bool)
    {
   ...
   */
        ...
        returns (Proposals.Proposal storage, Proposals.Stage)
    {
        ...
        require(
            _isDequeuedProposal(proposal, proposalId, index),
   ...
   */
        ...
        returns (bool)
    {
        ...
        uint256 weight = getLockedPlanq().getAccountTotalLockedPlanq(account);
        require(weight > 0, "cannot upvote without locking planq");
        require(
            voter.upvote.proposalId == 0 ||
   ...
   * @return proposal stage
   */
    function getProposalStage(uint256 proposalId)
        external
        view
        returns (Proposals.Stage)
    {
        ...
        if (isQueued(proposalId)) {
            return
                _isQueuedProposalExpired(proposal)
                    ? Proposals.Stage.Expiration
                    : Proposals.Stage.Queued;
        } else {
            Proposals.Stage stage = getProposalDequeuedStage(proposal);
            return
                _isDequeuedProposalExpired(proposal, stage)
                    ? Proposals.Stage.Expiration
                    : stage;
        }
    }
   ...
   */
    function approve(uint256 proposalId, uint256 index)
        external
        onlyApprover
        returns (bool)
    {
        ...
        proposal.networkWeight = getLockedPlanq().getTotalLockedPlanq();
   ...
   */
    ...
    function votePartially(
        ...
        address account = getAccounts().voteSignerToAccount(msg.sender);
        uint256 totalLockedPlanq = getLockedPlanq().getAccountTotalLockedPlanq(
            account
        );

        require(
            totalLockedPlanq >= yesVotes.add(noVotes).add(abstainVotes),
            "Voter doesn't have enough locked Planq"
        );
        _vote(
            proposal,
            proposalId,
   ...
   */
    function _vote(
        ...
        if (previousVoteRecord.proposalId != proposalId) {
            ...
            proposal.updateVote(
                ...
                previousVoteRecord.deprecated_value == Proposals.VoteValue.No
                    ? previousVoteRecord.deprecated_weight
                    : 0,
                previousVoteRecord.deprecated_value ==
                    Proposals.VoteValue.Abstain
                    ? previousVoteRecord.deprecated_weight
                    : 0,
                yesVotes,
                noVotes,
        ...
        if (
            proposal.timestamp >
            proposals[voter.mostRecentReferendumProposal].timestamp
        ) {
            voter.mostRecentReferendumProposal = proposalId;
        }

        emit ProposalVotedV2(
            proposalId,
            account,
            yesVotes,
            noVotes,
          ...
          requireDequeuedAndDeleteExpired(voteRecord.proposalId, dequeueIndex); // prettier-ignore
                ...
                if (stage == Proposals.Stage.Referendum) {
                    if (voteRecord.deprecated_weight != 0) {
                        // backward compatibility for transition period - this should be deleted later on
                        uint256 previousYes = voteRecord.deprecated_value ==
                            Proposals.VoteValue.Yes
                            ? voteRecord.deprecated_weight
                            : 0;
                        uint256 previousNo = voteRecord.deprecated_value ==
                            Proposals.VoteValue.No
                            ? voteRecord.deprecated_weight
                            : 0;
                        uint256 previousAbstain = voteRecord.deprecated_value ==
                            Proposals.VoteValue.Abstain
                            ? voteRecord.deprecated_weight
                            : 0;
                        proposal.updateVote(
                        ...
                        proposal.networkWeight = getLockedPlanq()
                            .getTotalLockedPlanq();
   ...
   */
    function execute(uint256 proposalId, uint256 index)
        external
        nonReentrant
        returns (bool)
    {
   ...
   */
    function approveHotfix(bytes32 hash)
        external
   ...
   */
    function isHotfixWhitelistedBy(bytes32 hash, address whitelister)
        public
        view
        returns (bool)
    {
   ...
   */
    function executeHotfix(
        ...
        bytes32 hash = keccak256(
            abi.encode(values, destinations, data, dataLengths, salt)
        );

        (bool approved, bool executed, uint256 preparedEpoch) = getHotfixRecord(
            hash
        );
        require(!executed, "hotfix already executed");
        require(approved, "hotfix not approved");
        require(
            preparedEpoch == getEpochNumber(),
            "hotfix must be prepared for this epoch"
        );

        Proposals
            .makeMem(values, destinations, data, dataLengths, msg.sender, 0)
            .executeMem();
    ...
    /**
   * @notice Withdraws refunded PLQ deposits.
   ...
   */
    function isVoting(address account) external view returns (bool) {
        ...
        Proposals.Proposal storage proposal = proposals[voter
            .mostRecentReferendumProposal];
        bool isVotingReferendum = (getProposalDequeuedStage(proposal) ==
   ...
   */
    function getUpvoteRecord(address account)
        external
        view
   ...
   */
    function getMostRecentReferendumProposal(address account)
        external
        view
        returns (uint256)
    {
   ...
   */
    function hotfixWhitelistValidatorTally(bytes32 hash)
        public
        view
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < n; i = i.add(1)) {
            ...
            address validatorAccount = accounts.signerToAccount(
                validatorSigner
            );
            if (
   ...
   */
    function isHotfixPassing(bytes32 hash) public view returns (bool) {
        return
            hotfixWhitelistValidatorTally(hash) >= minQuorumSizeInCurrentSet();
    }
   ...
   */
    function getHotfixRecord(bytes32 hash)
        public
        view
   ...
   */
    function dequeueProposalsIfReady() public {
        ...
        if (now >= lastDequeue.add(dequeueFrequency)) {
            uint256 numProposalsToDequeue = Math.min(
                concurrentProposals,
                queue.list.numElements
            );
            ...
            for (uint256 i = 0; i < numProposalsToDequeue; i = i.add(1)) {
                ...
                refundedDeposits[proposal.proposer] = refundedDeposits[proposal
                    .proposer]
                    .add(proposal.deposit);
                // solhint-disable-next-line not-rely-on-time
   ...
   * @dev If any of the top proposals have expired, they are deleted.
   */
    function dequeueProposalIfReady(uint256 proposalId)
        public
        returns (bool isProposalDequeued)
    {
   ...
   * @return Whether or not the proposal is passing.
   */
    function isProposalPassing(uint256 proposalId)
        external
        view
        returns (bool)
    {
   ...
   * @return Whether or not the proposal is passing.
   */
    function _isProposalPassing(Proposals.Proposal storage proposal)
        private
        view
        returns (bool)
    {
        FixidityLib.Fraction memory support = proposal
            .getSupportWithQuorumPadding(
            participationParameters.baseline.multiply(
                participationParameters.baselineQuorumFactor
            )
        );
        ...
        for (uint256 i = 0; i < proposal.transactions.length; i = i.add(1)) {
            bytes4 functionId = ExtractFunctionSignature
                .extractFunctionSignature(proposal.transactions[i].data);
            FixidityLib.Fraction memory threshold = _getConstitution(
   ...
   */
    function isDequeuedProposal(uint256 proposalId, uint256 index)
        external
        view
        returns (bool)
    {
   ...
   */
    function isDequeuedProposalExpired(uint256 proposalId)
        external
        view
        returns (bool)
    {
        ...
        return
            _isDequeuedProposalExpired(
                proposal,
                getProposalDequeuedStage(proposal)
            );
    }
   ...
   * @return Whether or not the dequeued proposal has expired.
   */
    function isQueuedProposalExpired(uint256 proposalId)
        public
        view
        returns (bool)
    {
   ...
   * @param proposal The proposal struct.
   */
    function updateParticipationBaseline(Proposals.Proposal storage proposal)
        private
    {
        FixidityLib.Fraction memory participation = proposal.getParticipation();
        FixidityLib.Fraction memory participationComponent = participation
            .multiply(participationParameters.baselineUpdateFactor);
        FixidityLib.Fraction memory baselineComponent = participationParameters
            .baseline
            .multiply(
        ...
        participationParameters.baseline = participationComponent.add(
            baselineComponent
        );
        if (
            participationParameters.baseline.lt(
                participationParameters.baselineFloor
            )
        ) {
            participationParameters.baseline = participationParameters
                .baselineFloor;
        }
        emit ParticipationBaselineUpdated(
            participationParameters.baseline.unwrap()
        );
    }
   ...
   */
    function getConstitution(address destination, bytes4 functionId)
        external
        view
        returns (uint256)
    {
        ...
        returns (FixidityLib.Fraction memory)
    {
        ...
        if (
            constitution[destination].functionThresholds[functionId].unwrap() !=
            0
        ) {
            threshold = constitution[destination]
                .functionThresholds[functionId];
   ...
   */
        ...
        returns (uint256)
    {
        ...
        if (isVotingQueue) {
            uint256 weight = getLockedPlanq().getAccountTotalLockedPlanq(
                account
            );
            return weight;
        }
        ...
        for (uint256 index = 0; index < dequeued.length; index = index.add(1)) {
            ...
            uint256 votesCast = voteRecord.yesVotes.add(voteRecord.noVotes).add(
                voteRecord.abstainVotes
            );
            maxUsed = Math.max(
                maxUsed,
