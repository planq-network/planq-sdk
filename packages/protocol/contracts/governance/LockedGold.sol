  ...
  UsingRegistry
{
  ...
  event PlanqUnlocked(address indexed account, uint256 value, uint256 available);
  ...
  */
  ...
  function unlock(uint256 value) external nonReentrant {
    ...
    uint256 remainingLockedPlanq = getAccountTotalLockedPlanq(msg.sender).sub(value);

    uint256 totalReferendumVotes = getGovernance().getAmountOfPlanqUsedForVoting(msg.sender);
    require(
      remainingLockedPlanq >= totalReferendumVotes,
      "Not enough unlockable planq. Planq is locked in voting."
    );

    uint256 balanceRequirement = getValidators().getAccountLockedPlanqRequirement(msg.sender);
    require(
      balanceRequirement == 0 || balanceRequirement <= remainingLockedPlanq,
   ...
   */
  function getAccountTotalLockedPlanq(address account) public view returns (uint256) {
   ...
   */
  function getAccountNonvotingLockedPlanq(address account) external view returns (uint256) {
  ...
  function slash(
    ...
    uint256 maxSlash = Math.min(penalty, getAccountTotalLockedPlanq(account));
