...
contract GovernanceSlasher is Ownable, Initializable, UsingRegistry {
   ...
   */
    function approveSlashing(address account, uint256 penalty)
        external
        onlyOwner
    {
   ...
   */
    function getApprovedSlashing(address account)
        external
        view
        returns (uint256)
    {
    ...
    /**
   * @notice Calls `LockedPlanq.slash` on `account` if `account` has an entry in `slashed`.
   * @param account Account to slash
   * @param electionLessers Lesser pointers for slashing locked election planq.
   * @param electionGreaters Greater pointers for slashing locked election planq.
