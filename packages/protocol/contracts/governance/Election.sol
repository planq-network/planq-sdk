    ...
    CalledByVm
{
    ...
    // A maximum of 1,000,000,000 PLQ (1e27) yields a maximum of 1e47 units, whose product is at
    // most 1e74, which is less than 2^256.
    uint256 private constant UNIT_PRECISION_FACTOR = 100000000000000000000;

    struct PendingVote {
        // The value of the vote, in planq.
        uint256 value;
   ...
   */
    ...
    {
        require(
            _maxNumGroupsVotedFor != maxNumGroupsVotedFor,
   ...
   * @param value The amount of planq to use to vote.
   ...
   */
        ...
        returns (bool)
    {
        ...
        if (!alreadyVotedForGroup) {
            require(
                allowedToVoteOverMaxNumberOfGroups[msg.sender] ||
                    groups.length < maxNumGroupsVotedFor,
        ...
        getLockedPlanq().decrementNonvotingAccountBalance(account, value);
   ...
   */
    ...
    function _activate(address group, address account) internal returns (bool) {
        PendingVote storage pendingVote = votes.pending.forGroup[group]
            .byAccount[account];
        require(
   ...
   */
    function revokePending(
        ...
        getLockedPlanq().incrementNonvotingAccountBalance(account, value);
   ...
   */
    function _decrementVotes(
        ...
        if (pendingVotes > 0) {
            ...
            emit ValidatorGroupPendingVoteRevoked(
                account,
                group,
                decrementValue
            );
        ...
        if (activeVotes > 0 && remainingValue > 0) {
            ...
            uint256 units = decrementActiveVotes(
                group,
                account,
                decrementValue
            );
   ...
   */
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < groups.length; i = i.add(1)) {
            total = total.add(
                getTotalVotesForGroupByAccount(groups[i], account)
            );
        }
        return total;
    }
   ...
   */
    function updateTotalVotesByAccountForGroup(address account, address group)
        public
    {
        cachedVotesByAccount[account]
            .totalVotes -= cachedVotesByAccount[account]
            .cachedVotesPerGroup[group];
        uint256 newTotalVotesForGroupByAccount = getTotalVotesForGroupByAccount(
            group,
            account
        );
        cachedVotesByAccount[account]
            .cachedVotesPerGroup[group] = newTotalVotesForGroupByAccount;
        cachedVotesByAccount[account]
            .totalVotes += newTotalVotesForGroupByAccount;
    }
   ...
   */
        ...
        returns (uint256)
    {
        return
            unitsToVotes(
                group,
                votes.active.forGroup[group].unitsByAccount[account]
            );
    }
   ...
   */
    function getActiveVoteUnitsForGroup(address group)
        external
        view
        returns (uint256)
    {
   ...
   */
    ...
    {
        return
            votes.pending.forGroup[group].total.add(
   ...
   */
    function getPendingVotesForGroup(address group)
        public
        view
        returns (uint256)
    {
   ...
   */
    function getGroupEpochRewards(
        ...
        if (
            !validators.meetsAccountLockedPlanqRequirements(group) ||
            votes.active.total <= 0
        ) {
            return 0;
        }
   ...
   */
    function _distributeEpochRewards(
        ...
        if (votes.total.eligible.contains(group)) {
            uint256 newVoteTotal = votes.total.eligible.getValue(group).add(
                value
            );
            votes.total.eligible.update(group, newVoteTotal, lesser, greater);
        }

        votes.active.forGroup[group].total = votes.active.forGroup[group]
            .total
            .add(value);
   ...
   */
    function decrementTotalVotes(
        ...
        if (votes.total.eligible.contains(group)) {
            uint256 newVoteTotal = votes.total.eligible.getValue(group).sub(
                value
            );
            votes.total.eligible.update(group, newVoteTotal, lesser, greater);
        }
   ...
   */
    function incrementPendingVotes(
        address group,
        address account,
        uint256 value
    ) private {
   ...
   * @param value The number of votes.
   */
    function decrementPendingVotes(
        address group,
        address account,
        uint256 value
    ) private {
   ...
   */
        ...
        returns (uint256)
    {
        ...
        groupActive.unitsByAccount[account] = groupActive
            .unitsByAccount[account]
            .add(units);
        return units;
    }
   ...
   */
    function votesToUnits(address group, uint256 value)
        private
        view
        returns (uint256)
    {
        if (votes.active.forGroup[group].totalUnits == 0) {
            ...
            return
                value.mul(votes.active.forGroup[group].totalUnits).div(
                    votes.active.forGroup[group].total
                );
        }
    }
   ...
   */
    function unitsToVotes(address group, uint256 value)
        private
        view
        returns (uint256)
    {
        if (votes.active.forGroup[group].totalUnits == 0) {
            ...
            return
                value.mul(votes.active.forGroup[group].total).div(
   ...
   * @dev Votes are not allowed to be cast that would increase a group's proportion of locked planq
   ...
   *   returns false.
   */
    function canReceiveVotes(address group, uint256 value)
        public
        view
        returns (bool)
    {
        ...
        uint256 right = getValidators().getGroupNumMembers(group).add(1).mul(
            getLockedPlanq().getTotalLockedPlanq()
        );
        return left <= right;
    }
   ...
   */
    function getNumVotesReceivable(address group)
        external
        view
        returns (uint256)
    {
        uint256 numerator = getValidators()
            .getGroupNumMembers(group)
            .add(1)
            .mul(getLockedPlanq().getTotalLockedPlanq());
   ...
   */
    function electNValidatorSigners(
        ...
        uint256[] memory numMembersElected = new uint256[](
        ...
        while (
            ...
            if (numMembers[groupIndex] <= numMembersElected[groupIndex]) {
                ...
                numMembersElected[groupIndex] = numMembersElected[groupIndex]
                    .add(1);
        ...
        for (uint256 i = 0; i < electionGroups.length; i = i.add(1)) {
            ...
            for (
                uint256 j = 0;
                j < electedGroupValidators.length;
                j = j.add(1)
            ) {
   ...
   */
    ...
    /**
   * @notice Reduces the total amount of `account`'s voting planq by `value` by
   ...
   */
    function forceDecrementVotes(
        ...
        external
        nonReentrant
        onlyRegisteredContract(LOCKED_PLANQ_REGISTRY_ID)
        returns (uint256)
    {
        require(value > 0, "Decrement value must be greater than 0.");
        DecrementVotesInfo memory info = DecrementVotesInfo(
            votes.groupsVotedFor[account],
            value
        );
        require(
