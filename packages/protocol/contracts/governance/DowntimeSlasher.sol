...
contract DowntimeSlasher is IPlanqVersionedContract, SlasherUtil {
   ...
   */
        ...
        returns (bytes32)
    {
        ...
        require(
            epochNumberOfBlock(startBlock, epochSize) ==
                epochNumberOfBlock(endBlock, epochSize),
   ...
   * @dev startBlock and endBlock must be in the same epoch.
   */
    function setBitmapForInterval(uint256 startBlock, uint256 endBlock)
        public
        returns (bytes32)
    {
        require(
   ...
   */
    function wasDownForInterval(
        ...
        require(
            signerIndex < numberValidatorsInSet(startBlock),
        ...
        return
            (bitmaps[msg.sender][startBlock][endBlock] &
                bytes32(1 << signerIndex)) ==
            0;
    }
   ...
   */
    function isBitmapSetForInterval(uint256 startBlock, uint256 endBlock)
        public
        view
        returns (bool)
    {
   ...
   */
    function slash(
        ...
        address validator = getValidatorAccountFromSignerIndex(
            signerIndices[0],
            startBlock
        );
        require(
        ...
        require(
            wasDownForIntervals(startBlocks, endBlocks, signerIndices),
            "not down"
        );
