...
import "./interfaces/IReleasePlanq.sol";
    ...
    Initializable
{
    ...
    struct RevocationInfo {
        ...
        bool canExpire;
        // Released planq instance balance at time of revocation.
    ...
    // Duration (in seconds) after planq is fully released
    // when planq should be switched back to control of releaseOwner.
    // 2 years
    uint256 public constant EXPIRATION_TIME = 63072000;

    // Beneficiary of the PLQ released in this contract.
    ...
    // Address that receives refunded planq if contract is revoked.
    address payable public refundAddress;

    // Indicates how much of the released amount has been withdrawn so far.
    uint256 public totalWithdrawn;

    // Indicates the maximum planq currently available for distribution, regardless of schedule.
    ...
    // Indicates if this schedule's unreleased planq can be used for validating.
    bool public canValidate;

    // Indicates if this schedule's unreleased planq can be used for voting.
    bool public canVote;

    // Public struct housing params pertaining to releasing planq.
    ...
    event ReleasePlanqInstanceCreated(
        address indexed beneficiary,
        address indexed atAddress
    );
    event ReleaseScheduleRevoked(
        uint256 revokeTimestamp,
        uint256 releasedBalanceAtRevoke
    );
    event ReleasePlanqInstanceDestroyed(
        address indexed beneficiary,
        address indexed atAddress
    );
    event DistributionLimitSet(
        address indexed beneficiary,
    ...
    modifier onlyBeneficiary() {
        require(
            ...
            "Sender must be the registered beneficiary address"
        );
        _;
    }
    ...
    modifier onlyRevoked() {
        require(
            isRevoked(),
            "Release schedule instance must have already been revoked"
        );
        _;
    }
    ...
    modifier onlyCanVote() {
        require(
            canVote,
            "Release Planq contract does not have permission to vote"
        );
        _;
    }
    ...
    modifier onlyCanValidate() {
        require(
            canValidate,
            "Release Planq contract does not have permission to validate"
        );
        _;
    }
   ...
   */
    ...
    function isFunded() public view returns (bool) {
        ...
        return
            ...
            releaseSchedule.amountReleasedPerPeriod.mul(
   ...
   */
    ...
    {
        IERC20(registry.getAddressForOrDie(STABLE_TOKEN_REGISTRY_ID)).transfer(
            to,
            value
        );
    }
   ...
   * @dev Protects against planq balance changes.
   */
    ...
    {
        require(
            erc20 != registry.getAddressForOrDie(PLANQ_TOKEN_REGISTRY_ID),
   ...
   * @param releaseCliffTime Duration (in seconds) after `releaseStartTime` of the planqs' cliff.
   * @param numReleasePeriods Number of releasing periods.
   * @param releasePeriod Duration (in seconds) of each release period.
   * @param amountReleasedPerPeriod The released planq amount per period.
   ...
   * @param _canValidate If this schedule's planq can be used for validating.
   * @param _canVote If this schedule's planq can be used for voting.
   ...
   */
    function initialize(
        ...
        // PLQ in the ReleasePlanq contract even after it becomes withdrawable.
        revocationInfo.canExpire = !canValidate;
        require(
            releaseSchedule.numReleasePeriods >= 1,
        ...
        require(
            !(revocable && _canValidate),
            "Revocable contracts cannot validate"
        );
        require(
            initialDistributionRatio <= 1000,
            "Initial distribution ratio out of bounds"
        );
        require(
            (revocable && _refundAddress != address(0)) ||
                (!revocable && _refundAddress == address(0)),
        ...
        if (initialDistributionRatio < 1000) {
            // Cannot use `getTotalBalance()` here because the factory has not yet sent the planq.
            uint256 totalGrant = releaseSchedule.amountReleasedPerPeriod.mul(
                releaseSchedule.numReleasePeriods
            );
            // Initial ratio is expressed to 3 significant figures: [0, 1000].
            maxDistribution = totalGrant.mul(initialDistributionRatio).div(
                1000
            );
        } else {
        ...
        canVote = _canVote;
        emit ReleasePlanqInstanceCreated(beneficiary, address(this));
    }
   ...
   */
    ...
    /**
   * @notice Controls if the liquidity provision has been met, allowing planq to be withdrawn.
   */
    function setLiquidityProvision() external onlyReleaseOwner {
        require(
            !liquidityProvisionMet,
            "Liquidity provision has already been set"
        );
   ...
   *         and sets this value as the maximum allowed planq to be currently withdrawn.
   * @param distributionRatio Amount in range [0, 1000] (3 significant figures)
   *                          indicating % of total balance available for distribution.
   */
    function setMaxDistribution(uint256 distributionRatio)
        external
        onlyReleaseOwner
    {
        require(
            distributionRatio <= 1000,
            "Max distribution ratio must be within bounds"
        );
        require(
            maxDistribution != MAX_UINT,
        ...
        if (distributionRatio == 1000) {
            ...
            require(
                totalBalance > 0,
                "Do not set max distribution before factory sends the planq"
            );
   ...
   */
    function _setBeneficiary(address payable newBeneficiary) private {
        require(
            newBeneficiary != address(0x0),
            "Can't set the beneficiary to the zero address"
        );
    ...
    /**
   * @notice Transfers planq from this release schedule instance to the beneficiary.
   * @param amount The requested planq amount.
   */
    function withdraw(uint256 amount) external nonReentrant onlyBeneficiary {
        ...
        if (getRemainingTotalBalance() == 0) {
            emit ReleasePlanqInstanceDestroyed(beneficiary, address(this));
   ...
   */
    function refundAndFinalize()
        external
        nonReentrant
        onlyReleaseOwnerAndRevoked
    {
        require(
            getRemainingLockedBalance() == 0,
            "Total planq balance must be unlocked"
        );
        uint256 beneficiaryAmount = revocationInfo.releasedBalanceAtRevoke.sub(
            totalWithdrawn
        );
        require(
            address(this).balance >= beneficiaryAmount,
   ...
   */
    ...
    /**
   * @notice Mark the contract as expired, freeing all remaining planq for refund to `refundAddress`
   * @dev Only callable `EXPIRATION_TIME` after the final planq release.
   */
   ...
   * @return The total released instance planq balance.
   * @dev The returned amount may vary over time due to locked planq rewards.
   */
    function getTotalBalance() public view returns (uint256) {
        return
            getRemainingUnlockedBalance().add(getRemainingLockedBalance()).add(
                totalWithdrawn
            );
    }

    /**
   * @notice Calculates the sum of locked and unlocked planq in the release schedule instance.
   * @return The remaining total released instance planq balance.
   * @dev The returned amount may vary over time due to locked planq rewards.
   */
    ...
    /**
   * @notice Calculates remaining unlocked planq balance in the release schedule instance.
   * @return The available unlocked release schedule instance planq balance.
   */
    ...
    /**
   * @notice Calculates remaining locked planq balance in the release schedule instance.
   ...
   * @return The remaining locked planq of the release schedule instance.
   * @dev The returned amount may vary over time due to locked planq rewards.
   */
    function getRemainingLockedBalance() public view returns (uint256) {
        if (getAccounts().isAccount(address(this))) {
            ILockedPlanq lockedPlanq = getLockedPlanq();
            uint256 pendingWithdrawalSum = lockedPlanq
                .getTotalPendingWithdrawals(address(this));
            return
                lockedPlanq.getAccountTotalLockedPlanq(address(this)).add(
   ...
   */
    function getCurrentReleasedTotalAmount() public view returns (uint256) {
        if (
            block.timestamp < releaseSchedule.releaseCliff ||
            !liquidityProvisionMet
        ) {
            return 0;
        }
        ...
        if (
            ...
            releaseSchedule.releaseStartTime.add(
                releaseSchedule.numReleasePeriods.mul(
                    releaseSchedule.releasePeriod
                )
            )
        ) {
            return totalBalance;
        ...
        uint256 periodsSinceStart = timeSinceStart.div(
            releaseSchedule.releasePeriod
        );
        return
            totalBalance.mul(periodsSinceStart).div(
                releaseSchedule.numReleasePeriods
            );
    }

    /**
   * @notice A wrapper function for the lock planq method.
   * @param value The value of planq to be locked.
   */
    function lockPlanq(uint256 value)
        external
        nonReentrant
        onlyBeneficiaryAndNotRevoked
    {
        getLockedPlanq().lock.gas(gasleft()).value(value)();
    }

    /**
   * @notice A wrapper function for the unlock planq method function.
   * @param value The value of planq to be unlocked for the release schedule instance.
   */
    function unlockPlanq(uint256 value)
        external
        nonReentrant
        onlyWhenInProperState
    {
        getLockedPlanq().unlock(value);
    }

    /**
   * @notice A wrapper function for the relock locked planq method function.
   * @param index The index of the pending locked planq withdrawal.
   * @param value The value of planq to be relocked for the release schedule instance.
   */
    function relockPlanq(uint256 index, uint256 value)
        external
        nonReentrant
        onlyBeneficiaryAndNotRevoked
    {
        getLockedPlanq().relock(index, value);
    }

    /**
   * @notice A wrapper function for the withdraw locked planq method function.
   * @param index The index of the pending locked planq withdrawal.
   * @dev The amount shall be withdrawn back to the release schedule instance.
   */
    function withdrawLockedPlanq(uint256 index)
        external
        nonReentrant
        onlyWhenInProperState
    {
        getLockedPlanq().withdraw(index);
    }

    /**
   * @notice Funds a signer address so that transaction fees can be paid.
   * @param signer The signer address to fund.
   * @dev Note that this effectively decreases the total balance by 1 PLQ.
   */
    function fundSigner(address payable signer) private {
        // Fund signer account with 1 PLQ.
        uint256 value = 1 ether;
        require(
            address(this).balance >= value,
            "no available PLQ to fund signer"
        );
   ...
   *      key, with the ReleasePlanq contract address as the message.
   */
   ...
   * @param name A string to set as the name of the account.
   */
    function setAccountName(string calldata name)
        external
        onlyBeneficiaryAndNotRevoked
    {
  ...
  */
    function getWithdrawableAmount() public view returns (uint256) {
        return
            Math.min(
                Math.min(maxDistribution, getCurrentReleasedTotalAmount()).sub(
                    totalWithdrawn
                ),
