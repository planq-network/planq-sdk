    ...
    CalledByVm
{
    ...
    // group, after which no restrictions on Locked Planq will apply to the account.
    //
    ...
    // which no restrictions on Locked Planq will apply to the account.
    struct LockedPlanqRequirements {
        uint256 value;
        // In seconds.
        uint256 duration;
    }
    ...
    LockedPlanqRequirements public validatorLockedPlanqRequirements;
    LockedPlanqRequirements public groupLockedPlanqRequirements;
    ...
    event GroupLockedPlanqRequirementsSet(uint256 value, uint256 duration);
    event ValidatorLockedPlanqRequirementsSet(uint256 value, uint256 duration);
    ...
    modifier onlySlasher() {
        require(
            getLockedPlanq().isSlasher(msg.sender),
   ...
   * @param groupRequirementValue The Locked Planq requirement amount for groups.
   * @param groupRequirementDuration The Locked Planq requirement duration for groups.
   * @param validatorRequirementValue The Locked Planq requirement amount for validators.
   * @param validatorRequirementDuration The Locked Planq requirement duration for validators.
   ...
   */
    function initialize(
        ...
        setGroupLockedPlanqRequirements(
            groupRequirementValue,
            groupRequirementDuration
        );
        setValidatorLockedPlanqRequirements(
   ...
   */
    function setCommissionUpdateDelay(uint256 delay) public onlyOwner {
        require(
            delay != commissionUpdateDelay,
            "commission update delay not changed"
        );
   ...
   * @return True upon success.
   */
    function setMembershipHistoryLength(uint256 length)
        public
        onlyOwner
        returns (bool)
    {
        ...
        require(
            length != membershipHistoryLength,
    ...
    function setValidatorScoreParameters(
        ...
        require(
            exponent != validatorScoreParameters.exponent ||
                !FixidityLib.wrap(adjustmentSpeed).equals(
   ...
   */
    ...
    /**
   * @notice Updates the Locked Planq requirements for Validator Groups.
   * @param value The per-member amount of Locked Planq required.
   * @param duration The time (in seconds) that these requirements persist for.
   * @return True upon success.
   */
    function setGroupLockedPlanqRequirements(uint256 value, uint256 duration)
        public
        onlyOwner
        returns (bool)
    {
        LockedPlanqRequirements storage requirements = groupLockedPlanqRequirements;
        require(
            value != requirements.value || duration != requirements.duration,
            "Group requirements not changed"
        );
        groupLockedPlanqRequirements = LockedPlanqRequirements(value, duration);
        emit GroupLockedPlanqRequirementsSet(value, duration);
        return true;
    }

    /**
   * @notice Updates the Locked Planq requirements for Validators.
   * @param value The amount of Locked Planq required.
   * @param duration The time (in seconds) that these requirements persist for.
   * @return True upon success.
   */
    function setValidatorLockedPlanqRequirements(
        uint256 value,
        uint256 duration
    ) public onlyOwner returns (bool) {
        LockedPlanqRequirements storage requirements = validatorLockedPlanqRequirements;
        require(
            value != requirements.value || duration != requirements.duration,
            "Validator requirements not changed"
        );
        validatorLockedPlanqRequirements = LockedPlanqRequirements(
            value,
            duration
        );
        emit ValidatorLockedPlanqRequirementsSet(value, duration);
        return true;
    }
   ...
   * @dev Fails if the account does not have sufficient Locked Planq.
   */
    function registerValidator(
        ...
        require(
            ...
            // amounts of PLQ, causing the slash function to always run out of gas.
        ...
        uint256 lockedPlanqBalance = getLockedPlanq()
            .getAccountTotalLockedPlanq(account);
        require(
            lockedPlanqBalance >= validatorLockedPlanqRequirements.value,
            "Deposit too small"
        );
   ...
   */
    ...
    {
        MembershipHistory storage history = validators[account]
            .membershipHistory;
        ...
        return (
            epochs,
            membershipGroups,
            history.lastRemovedFromGroupTimestamp,
            history.tail
        );
    }
   ...
   */
    function calculateEpochScore(uint256 uptime) public view returns (uint256) {
        require(
            uptime <= FixidityLib.fixed1().unwrap(),
            "Uptime cannot be larger than one"
        );
        uint256 numerator;
        uint256 denominator;
        uptime = Math.min(
            uptime.add(downtimeGracePeriod),
   ...
   */
        ...
        returns (uint256)
    {
        ...
        require(
            uptimes.length <= maxGroupSize,
            "Uptime array larger than maximum group size"
        );
   ...
   * @return True upon success.
   */
    function updateValidatorScoreFromSigner(address signer, uint256 uptime)
        external
        onlyVm()
    {
   ...
   * @return True upon success.
   */
    function _updateValidatorScoreFromSigner(address signer, uint256 uptime)
        internal
    {
        address account = getAccounts().signerToAccount(signer);
        require(isValidator(account), "Not a validator");

        FixidityLib.Fraction memory epochScore = FixidityLib.wrap(
            calculateEpochScore(uptime)
        );
        FixidityLib.Fraction memory newComponent = validatorScoreParameters
            .adjustmentSpeed
            .multiply(epochScore);

        FixidityLib.Fraction memory currentComponent = FixidityLib
            .fixed1()
            .subtract(validatorScoreParameters.adjustmentSpeed);
        ...
        emit ValidatorScoreUpdated(
            account,
            validators[account].score.unwrap(),
            epochScore.unwrap()
        );
    }
   ...
   */
    function _distributeEpochPaymentsFromSigner(
        ...
        // Both the validator and the group must maintain the minimum locked planq balance in order to
        // receive epoch payments.
        if (
            meetsAccountLockedPlanqRequirements(account) &&
            meetsAccountLockedPlanqRequirements(group)
        ) {
            ...
            uint256 groupPayment = totalPayment
                .multiply(groups[group].commission)
                .fromFixed();
            FixidityLib.Fraction memory remainingPayment = FixidityLib.newFixed(
                totalPayment.fromFixed().sub(groupPayment)
            );
            (address beneficiary, uint256 fraction) = getAccounts()
                .getPaymentDelegation(account);
            uint256 delegatedPayment = remainingPayment
                .multiply(FixidityLib.wrap(fraction))
                .fromFixed();
            uint256 validatorPayment = remainingPayment.fromFixed().sub(
                delegatedPayment
            );
            IStableToken stableToken = getStableToken();
            require(
                stableToken.mint(group, groupPayment),
                "mint failed to validator group"
            );
            require(
                stableToken.mint(account, validatorPayment),
                "mint failed to validator account"
            );
            if (fraction != 0) {
                require(
                    stableToken.mint(beneficiary, delegatedPayment),
                    "mint failed to delegatee"
                );
            }
   ...
   */
    function deregisterValidator(uint256 index)
        external
        nonReentrant
        returns (bool)
    {
        ...
        // `validatorLockedPlanqRequirements.duration` seconds.
        Validator storage validator = validators[account];
        ...
        uint256 requirementEndTime = validator
            .membershipHistory
            .lastRemovedFromGroupTimestamp
            .add(validatorLockedPlanqRequirements.duration);
   ...
   */
    function _updateBlsPublicKey(
        ...
        require(
            checkProofOfPossession(account, blsPublicKey, blsPop),
            "Invalid BLS PoP"
        );
   ...
   */
    function registerValidatorGroup(uint256 commission)
        external
        nonReentrant
        returns (bool)
    {
        require(
            ...
            "Commission can't be greater than 100%"
        );
        address account = getAccounts().validatorSignerToAccount(msg.sender);
        require(
            !getElection().allowedToVoteOverMaxNumberOfGroups(account),
        ...
        uint256 lockedPlanqBalance = getLockedPlanq()
            .getAccountTotalLockedPlanq(account);
        require(
            lockedPlanqBalance >= groupLockedPlanqRequirements.value,
            "Not enough locked planq"
        );
   ...
   */
    function deregisterValidatorGroup(uint256 index)
        external
        nonReentrant
        returns (bool)
    {
        ...
        // `groupLockedPlanqRequirements.duration` seconds can be deregistered.
        ...
        if (sizeHistory.length > 1) {
            require(
                sizeHistory[1].add(groupLockedPlanqRequirements.duration) < now,
   ...
   */
    function _addMember(
        ...
        require(
            isValidatorGroup(group) && isValidator(validator),
            "Not validator and group"
        );
        ValidatorGroup storage _group = groups[group];
        require(
            _group.members.numElements < maxGroupSize,
            "group would exceed maximum size"
        );
        require(
            validators[validator].affiliation == group,
        ...
        require(
            meetsAccountLockedPlanqRequirements(validator),
   ...
   */
    function removeMember(address validator)
        external
        nonReentrant
        returns (bool)
    {
   ...
   */
    function setNextCommissionUpdate(uint256 commission) external {
        ...
        require(
            commission != group.commission.unwrap(),
            "Commission must be different"
        );
   ...
   */
    function updateCommission() external {
        ...
        require(
            group.nextCommissionBlock <= block.number,
        ...
        emit ValidatorGroupCommissionUpdated(
            account,
    ...
    /**
   * @notice Returns the current locked planq balance requirement for the supplied account.
   * @param account The account that may have to meet locked planq balance requirements.
   * @return The current locked planq balance requirement for the supplied account.
   */
    function getAccountLockedPlanqRequirement(address account)
        public
        view
        returns (uint256)
    {
        if (isValidator(account)) {
            return validatorLockedPlanqRequirements.value;
        } else if (isValidatorGroup(account)) {
            uint256 multiplier = Math.max(
                1,
                groups[account].members.numElements
            );
            uint256[] storage sizeHistory = groups[account].sizeHistory;
            if (sizeHistory.length > 0) {
                for (
                    uint256 i = sizeHistory.length.sub(1);
                    i > 0;
                    i = i.sub(1)
                ) {
                    if (
                        sizeHistory[i].add(
                            groupLockedPlanqRequirements.duration
                        ) >=
                        now
                    ) {
            ...
            return groupLockedPlanqRequirements.value.mul(multiplier);
        }
        return 0;
    }

    /**
   * @notice Returns whether or not an account meets its Locked Planq requirements.
   * @param account The address of the account.
   * @return Whether or not an account meets its Locked Planq requirements.
   */
    function meetsAccountLockedPlanqRequirements(address account)
        public
        view
        returns (bool)
    {
        uint256 balance = getLockedPlanq().getAccountTotalLockedPlanq(account);
        ...
        return balance.add(10) >= getAccountLockedPlanqRequirement(account);
    }
   ...
   */
    ...
    /**
   * @notice Returns the Locked Planq requirements for validators.
   * @return The Locked Planq value.
   * @return The Locked Planq duration.
   */
    function getValidatorLockedPlanqRequirements()
        external
        view
        returns (uint256, uint256)
    {
        return (
            validatorLockedPlanqRequirements.value,
            validatorLockedPlanqRequirements.duration
        );
    }

    /**
   * @notice Returns the Locked Planq requirements for validator groups.
   * @return The Locked Planq value.
   * @return The Locked Planq duration.
   */
    function getGroupLockedPlanqRequirements()
        external
        view
        returns (uint256, uint256)
    {
        return (
            groupLockedPlanqRequirements.value,
            groupLockedPlanqRequirements.duration
        );
    }
   ...
   */
    function _removeMember(address group, address validator)
        private
        returns (bool)
    {
        ValidatorGroup storage _group = groups[group];
        ...
        require(
            _group.members.contains(validator),
   ...
   */
    function updateMembershipHistory(address account, address group)
        private
        returns (bool)
    {
        MembershipHistory storage history = validators[account]
        ...
        uint256 head = history.numEntries == 0
            ? 0
            : history.tail.add(history.numEntries.sub(1));
        ...
        if (
            ...
            history.entries[head].epochNumber == epochNumber
        ) {
   ...
   * @dev Used to determine how much planq an account needs to keep locked.
   */
   ...
   */
    function getMembershipInLastEpochFromSigner(address signer)
        external
        view
        returns (address)
    {
   ...
   * @return The group that `account` was a member of at the end of the last epoch.
   */
    function getMembershipInLastEpoch(address account)
        public
        view
        returns (address)
    {
   ...
   * @param validatorAccount The LockedPlanq account of the validator.
   * @return True upon success.
   */
   ...
   */
    function setDowntimeGracePeriod(uint256 value)
        public
        nonReentrant
        onlyOwner
    {
   ...
   */
    function resetSlashingMultiplier() external nonReentrant {
        ...
        require(
            now >=
                group.slashInfo.lastSlashed.add(slashingMultiplierResetPeriod),
   ...
   */
        ...
        onlySlasher
    {
        require(isValidatorGroup(account), "Not a validator group");
        ValidatorGroup storage group = groups[account];
        group.slashInfo.multiplier = FixidityLib.wrap(
            group.slashInfo.multiplier.unwrap().div(2)
        );
   ...
   */
    function groupMembershipInEpoch(
        ...
        require(
            index >= history.tail && history.numEntries > 0,
            "index out of bounds"
        );
