  ...
  CalledByVm
{
  ...
  modifier onlySlasher() {
    require(getLockedPlanq().isSlasher(msg.sender), "Only registered slasher can call");
    _;
  }
  ...
  function initialize(
    ...
    setGroupLockedPlanqRequirements(groupRequirementValue, groupRequirementDuration);
    setValidatorLockedPlanqRequirements(validatorRequirementValue, validatorRequirementDuration);
   ...
   * @return True upon success.
   */
  function setValidatorLockedPlanqRequirements(uint256 value, uint256 duration)
    public
    onlyOwner
    returns (bool)
  {
    ...
    validatorLockedPlanqRequirements = LockedPlanqRequirements(value, duration);
  ...
  function registerValidator(
    ...
    uint256 lockedPlanqBalance = getLockedPlanq().getAccountTotalLockedPlanq(account);
    require(lockedPlanqBalance >= validatorLockedPlanqRequirements.value, "Deposit too small");
    ...
    returns (uint256)
  {
    ...
    if (meetsAccountLockedPlanqRequirements(account) && meetsAccountLockedPlanqRequirements(group)) {
  ...
  function affiliate(address group) external nonReentrant returns (bool) {
    ...
    require(meetsAccountLockedPlanqRequirements(account), "Validator doesn't meet requirements");
    require(meetsAccountLockedPlanqRequirements(group), "Group doesn't meet requirements");
    Validator storage validator = validators[account];
    if (validator.affiliation != address(0)) {
  ...
  function registerValidatorGroup(uint256 commission) external nonReentrant returns (bool) {
    ...
    uint256 lockedPlanqBalance = getLockedPlanq().getAccountTotalLockedPlanq(account);
    require(lockedPlanqBalance >= groupLockedPlanqRequirements.value, "Not enough locked planq");
    ...
    returns (bool)
  {
    ...
    require(meetsAccountLockedPlanqRequirements(group), "Group requirements not met");
    require(meetsAccountLockedPlanqRequirements(validator), "Validator requirements not met");
    if (numMembers == 1) {
   ...
   */
  function getAccountLockedPlanqRequirement(address account) public view returns (uint256) {
    if (isValidator(account)) {
      ...
      if (sizeHistory.length > 0) {
        for (uint256 i = sizeHistory.length.sub(1); i > 0; i = i.sub(1)) {
          if (sizeHistory[i].add(groupLockedPlanqRequirements.duration) >= now) {
   ...
   */
  function meetsAccountLockedPlanqRequirements(address account) public view returns (bool) {
   ...
   */
  function getValidatorLockedPlanqRequirements() external view returns (uint256, uint256) {
    return (validatorLockedPlanqRequirements.value, validatorLockedPlanqRequirements.duration);
  }
   ...
   * @return The Locked Planq duration.
   */
  function getGroupLockedPlanqRequirements() external view returns (uint256, uint256) {
    return (groupLockedPlanqRequirements.value, groupLockedPlanqRequirements.duration);
  }
