...
contract BlockchainParameters is Ownable, Initializable, UsingPrecompiles {
   ...
   * @param _gasForNonPlanqCurrencies Intrinsic gas for non-planq gas currencies.
   * @param gasLimit Block gas limit.
   * @param lookbackWindow Lookback window for measuring validator uptime.
   */
    function initialize(
        uint256 _gasForNonPlanqCurrencies,
        uint256 gasLimit,
        uint256 lookbackWindow
    ) external initializer {
        _transferOwnership(msg.sender);
        setBlockGasLimit(gasLimit);
        setIntrinsicGasForAlternativeFeeCurrency(_gasForNonPlanqCurrencies);
   ...
   */
    ...
    /**
   * @notice Sets the intrinsic gas for non-planq gas currencies.
   * @param gas Intrinsic gas for non-planq gas currencies.
   */
    function setIntrinsicGasForAlternativeFeeCurrency(uint256 gas)
        public
        onlyOwner
    {
   ...
   */
    function setUptimeLookbackWindow(uint256 window) public onlyOwner {
        ...
        emit UptimeLookbackWindowSet(
            window,
            uptimeLookbackWindow.nextValueActivationEpoch
        );
    }
   ...
   */
    function getUptimeLookbackWindow()
        public
        view
        returns (uint256 lookbackWindow)
    {
   ...
   * @notice Gets the uptime lookback window.
   */
    function _getUptimeLookbackWindow()
        internal
        view
        returns (uint256 lookbackWindow)
    {
