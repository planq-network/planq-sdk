...
contract MockLockedPlanq is ILockedPlanq {
  ...
  function setAccountTotalLockedPlanq(address account, uint256 value) external {
    accountTotalLockedPlanq[account] = value;
  }

  function getAccountTotalLockedPlanq(address account) external view returns (uint256) {
  ...
  function lock() external payable {
    accountTotalLockedPlanq[msg.sender] = accountTotalLockedPlanq[msg.sender].add(msg.value);
  }

  function unlock(uint256 value) external {
    accountTotalLockedPlanq[msg.sender] = accountTotalLockedPlanq[msg.sender].sub(value);
  }
  ...
  function slash(
    ...
    accountTotalLockedPlanq[account] = accountTotalLockedPlanq[account].sub(penalty);
  }
