...
contract MockUsingPrecompiles {
    ...
    {
        return
            epochSigner[keccak256(
                abi.encodePacked(calcEpoch(blockNumber), index)
            )];
    }

    function setEpochSigner(uint256 epoch, uint256 index, address signer)
        public
    {
    ...
    function setBlockNumber(bytes memory header, uint256 number)
        public
        returns (uint256)
    {
