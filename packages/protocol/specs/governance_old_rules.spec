...
/* The sum of yes votes for a proposal cannot exceed total weight - requires linked LockedPlanq */
// this rule requires linkage of LockedPlanq
rule sum_of_votes_cannot_exceed_total_weight(method f, uint256 p) filtered { f -> !f.isView } {
	uint256 _totalWeight = getTotalLockedPlanq();
	...
	uint256 totalWeight_ = getTotalLockedPlanq();
