...
/**
 * Example Usage:
 * `certoraRun.py locked_planq.conf  --path $PWD/contracts/  --cache planqlockedloop3unwind --settings -patient=0,-b=3,-assumeUnwindCond`
 */

methods {
	init_state()
	getNonvotingLockedPlanq() returns uint256 envfree
	getAccountNonvotingLockedPlanq(address)  returns uint256 envfree
	...
	getPlanqTokenExt() returns address envfree
}

/**
 * Account's nonvoting planq cannot exceed total locked nonvoting planq.
 */
rule totalNonVotingGEAccountNonVoting(address a, method f) {
	require(sinvoke getNonvotingLockedPlanq()  >= sinvoke getAccountNonvotingLockedPlanq(a));
	require(
	...
	calldataarg arg;
	sinvoke f(eF,arg);
	assert(sinvoke getNonvotingLockedPlanq() >= sinvoke getAccountNonvotingLockedPlanq(a));
}

/**
 * Total here refers to the sum of address balance and locked planq (voting and nonvoting).
 */
rule totalPreserved(address account, method f) {
	...
	uint256 _accountNonVoting = sinvoke getAccountNonvotingLockedPlanq(account);
	...
	uint256 accountNonVoting_ = sinvoke getAccountNonvotingLockedPlanq(account);
...
/**
 * An address' total planq cannot be changed by any other contract's function calls.
 */
rule noChangeByOther(address a, address b, method f) {
	...
	require(
    a != currentContract &&
    (a == sinvoke getPlanqTokenExt() => f.selector != withdraw(uint256).selector));
	uint256 _ercBalance = sinvoke ercBalanceOf(a);
	uint256 _accountNonVoting = sinvoke getAccountNonvotingLockedPlanq(a);
	...
	uint256 accountNonVoting_ = sinvoke getAccountNonvotingLockedPlanq(a);
...
/**
 * This rule verifies the Certora Prover is correctly modeling the behavior of PlanqToken._transfer.
 */
...
rule noWithdrawBeforeUnlocking(address account, uint256 value, method f) {
    ...
    sinvoke getAccountNonvotingLockedPlanq(account) + totalPendingWithdrawals_ >= value,
