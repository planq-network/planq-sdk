...
import { fixed1, fromFixed, toFixed } from '@planq-network/utils/lib/fixidity'
...
contract('Validators', (accounts: string[]) => {
  ...
  const validatorLockedPlanqRequirements = {
    value: new BigNumber(1000),
    duration: new BigNumber(60 * DAY),
  }
  const groupLockedPlanqRequirements = {
    value: new BigNumber(1000),
  ...
  const registerValidator = async (validator: string) => {
    await mockLockedPlanq.setAccountTotalLockedPlanq(validator, validatorLockedPlanqRequirements.value)
  ...
  const registerValidatorGroup = async (group: string, numMembers: number = 1) => {
    await mockLockedPlanq.setAccountTotalLockedPlanq(
      group,
      groupLockedPlanqRequirements.value.times(numMembers)
    )
  ...
  describe('#initialize()', () => {
    ...
    it('should have set the group locked planq requirements', async () => {
      const [value, duration] = await validators.getGroupLockedPlanqRequirements()
      assertEqualBN(value, groupLockedPlanqRequirements.value)
      assertEqualBN(duration, groupLockedPlanqRequirements.duration)
    })

    it('should have set the validator locked planq requirements', async () => {
      const [value, duration] = await validators.getValidatorLockedPlanqRequirements()
      assertEqualBN(value, validatorLockedPlanqRequirements.value)
      assertEqualBN(duration, validatorLockedPlanqRequirements.duration)
    })
  ...
  describe('#setGroupLockedPlanqRequirements()', () => {
    describe('when the requirements are different', () => {
      const newRequirements = {
        value: groupLockedPlanqRequirements.value.plus(1),
        duration: groupLockedPlanqRequirements.duration.plus(1),
      }
      ...
      describe('when called by the owner', () => {
        ...
        beforeEach(async () => {
          resp = await validators.setGroupLockedPlanqRequirements(
        ...
        it('should emit the GroupLockedPlanqRequirementsSet event', async () => {
          assert.equal(resp.logs.length, 1)
          const log = resp.logs[0]
          assertContainSubset(log, {
            event: 'GroupLockedPlanqRequirementsSet',
            args: {
        ...
        describe('when called by a non-owner', () => {
          it('should revert', async () => {
            await assertTransactionRevertWithReason(
              validators.setGroupLockedPlanqRequirements(
                newRequirements.value,
                newRequirements.duration,
                { from: nonOwner }
              ),
  ...
  describe('#setValidatorLockedPlanqRequirements()', () => {
    describe('when the requirements are different', () => {
      const newRequirements = {
        value: validatorLockedPlanqRequirements.value.plus(1),
        duration: validatorLockedPlanqRequirements.duration.plus(1),
      }
      ...
      describe('when called by the owner', () => {
        ...
        beforeEach(async () => {
          resp = await validators.setValidatorLockedPlanqRequirements(
            newRequirements.value,
            newRequirements.duration
          )
        })
        ...
        it('should emit the ValidatorLockedPlanqRequirementsSet event', async () => {
          assert.equal(resp.logs.length, 1)
          const log = resp.logs[0]
          assertContainSubset(log, {
            event: 'ValidatorLockedPlanqRequirementsSet',
            args: {
              value: newRequirements.value,
              duration: newRequirements.duration,
            },
        ...
        describe('when called by a non-owner', () => {
          it('should revert', async () => {
            await assertTransactionRevertWithReason(
              validators.setValidatorLockedPlanqRequirements(
                newRequirements.value,
                newRequirements.duration,
                { from: nonOwner }
              ),
  ...
  describe('#registerValidator', () => {
    ...
    describe('when the account is not a registered validator', () => {
      ...
      describe('when the account has authorized a validator signer', () => {
        ...
        it('should set account locked planq requirements', async () => {
          const requirement = await validators.getAccountLockedPlanqRequirement(validator)
          assertEqualBN(requirement, validatorLockedPlanqRequirements.value)
        })
    ...
    describe('when the account is already a registered validator group', () => {
      beforeEach(async () => {
        await mockLockedPlanq.setAccountTotalLockedPlanq(validator, groupLockedPlanqRequirements.value)
    ...
    describe('when the account does not meet the locked planq requirements', () => {
      beforeEach(async () => {
        await mockLockedPlanq.setAccountTotalLockedPlanq(
          validator,
          validatorLockedPlanqRequirements.value.minus(11)
        )
      })
    ...
    describe('when the account is a registered validator', () => {
      ...
      describe('when the validator has been a member of a validator group', () => {
        ...
        describe('when the validator is no longer a member of a validator group', () => {
          ...
          describe('when it has been more than `validatorLockedPlanqRequirements.duration` since the validator was removed from the group', () => {
            beforeEach(async () => {
              await timeTravel(validatorLockedPlanqRequirements.duration.plus(1).toNumber(), web3)
              resp = await validators.deregisterValidator(index)
            })
          ...
          describe('when it has been less than `validatorLockedPlanqRequirements.duration` since the validator was removed from the group', () => {
            beforeEach(async () => {
              await timeTravel(validatorLockedPlanqRequirements.duration.minus(1).toNumber(), web3)
            })
  ...
  describe('#affiliate', () => {
    ...
    describe('when the account has a registered validator', () => {
      ...
      describe('when affiliating with a registered validator group', () => {
        ...
        describe('when the validator meets the locked planq requirements', () => {
          describe('when the group meets the locked planq requirements', () => {
            beforeEach(async () => {
          ...
          describe('when the group does not meet the locked planq requirements', () => {
            beforeEach(async () => {
              await mockLockedPlanq.setAccountTotalLockedPlanq(
                group,
                groupLockedPlanqRequirements.value.minus(11)
              )
            })
        ...
        describe('when the validator does not meet the locked planq requirements', () => {
          beforeEach(async () => {
            await mockLockedPlanq.setAccountTotalLockedPlanq(
              validator,
              validatorLockedPlanqRequirements.value.minus(11)
  ...
  describe('#updateEcdsaPublicKey()', () => {
    describe('when called by a registered validator', () => {
      ...
      describe('when called by the registered `Accounts` contract', () => {
        beforeEach(async () => {
          await registry.setAddressFor(PlanqContractName.Accounts, accounts[0])
        })
  ...
  describe('#registerValidatorGroup', () => {
    ...
    describe('when the account is not a registered validator group', () => {
      ...
      describe('when the account meets the locked planq requirements', () => {
        beforeEach(async () => {
          await mockLockedPlanq.setAccountTotalLockedPlanq(group, groupLockedPlanqRequirements.value)
        ...
        it('should set account locked planq requirements', async () => {
          const requirement = await validators.getAccountLockedPlanqRequirement(group)
          assertEqualBN(requirement, groupLockedPlanqRequirements.value)
        })
    ...
    describe('when the account has a registered validator group', () => {
      ...
      describe('when the group has had members', () => {
        ...
        describe('when the group no longer has members', () => {
          ...
          describe('when it has been more than `groupLockedPlanqRequirements.duration` since the validator was removed from the group', () => {
            beforeEach(async () => {
              await timeTravel(groupLockedPlanqRequirements.duration.plus(1).toNumber(), web3)
              resp = await validators.deregisterValidatorGroup(index)
            })
          ...
          describe('when it has been less than `groupLockedPlanqRequirements.duration` since the validator was removed from the group', () => {
            beforeEach(async () => {
              await timeTravel(groupLockedPlanqRequirements.duration.minus(1).toNumber(), web3)
            })
            ...
            describe('when adding many validators affiliated with the group', () => {
              it("should update the groups's size history and balance requirements", async () => {
                ...
                for (let i = 2; i < maxGroupSize.toNumber() + 1; i++) {
                  ...
                  const requirement = await validators.getAccountLockedPlanqRequirement(group)
                  assertEqualBN(requirement, groupLockedPlanqRequirements.value.times(numMembers))
                }
              })
            })
          })
        ...
        describe('when the group does not meet the locked planq requirements', () => {
          ...
          describe('when the group already has a member', () => {
            ...
            beforeEach(async () => {
              await mockLockedPlanq.setAccountTotalLockedPlanq(
                group,
                groupLockedPlanqRequirements.value.times(2).minus(11)
              )
              await validators.addFirstMember(validator, NULL_ADDRESS, NULL_ADDRESS)
  ...
  describe('#getAccountLockedPlanqRequirement', () => {
    describe('when a validator group has added members', () => {
      ...
      beforeEach(async () => {
        ...
        for (let i = 1; i < numMembers + 1; i++) {
          ...
          await mockLockedPlanq.setAccountTotalLockedPlanq(
            group,
            groupLockedPlanqRequirements.value.times(i)
          )
          if (i === 1) {
            await validators.addFirstMember(validator, NULL_ADDRESS, NULL_ADDRESS)
          } else {
          ...
          actualRequirements.push(await validators.getAccountLockedPlanqRequirement(group))
        }
      })

      it('should increase the requirement with each added member', async () => {
        for (let i = 0; i < numMembers; i++) {
          assertEqualBN(actualRequirements[i], groupLockedPlanqRequirements.value.times(i + 1))
        }
      })
      ...
      describe('when a validator group is removing members', () => {
        ...
        it('should decrease the requirement `duration`+1 seconds after removal', async () => {
          for (let i = 0; i < numMembers; i++) {
            assertEqualBN(
              await validators.getAccountLockedPlanqRequirement(group),
              groupLockedPlanqRequirements.value.times(numMembers - i)
            )
            const removalTimestamp = removalTimestamps[i]
            const requirementExpiry = groupLockedPlanqRequirements.duration.plus(removalTimestamp)
  ...
  describe('#distributeEpochPaymentsFromSigner', () => {
    ...
    beforeEach(async () => {
      ...
      await mineToNextEpoch(web3)
      await mockLockedPlanq.addSlasher(accounts[2])
