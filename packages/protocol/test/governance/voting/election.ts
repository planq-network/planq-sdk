...
import { normalizeAddressWith0x } from '@planq-network/utils/lib/address'
...
// @ts-ignoree
MockLockedPlanq.numberFormat = 'BigNumber'
...
contract('Election', (accounts: string[]) => {
  ...
  describe('#vote', () => {
    ...
    describe('when the group is eligible', () => {
      ...
      describe('when the group can receive votes', () => {
        beforeEach(async () => {
          await mockLockedPlanq.setTotalLockedPlanq(value)
        ...
        describe('when the voter can vote for an additional group', () => {
          describe('when the voter has sufficient non-voting balance', () => {
            beforeEach(async () => {
              await mockLockedPlanq.incrementNonvotingAccountBalance(voter, value)
            })
            ...
            describe('when the voter has not already voted for this group', () => {
              ...
              it("should decrement the account's nonvoting locked planq balance", async () => {
                assertEqualBN(await mockLockedPlanq.nonvotingAccountBalance(voter), 0)
              })
          ...
          describe('when the voter does not have sufficient non-voting balance', () => {
            beforeEach(async () => {
              await mockLockedPlanq.incrementNonvotingAccountBalance(voter, value.minus(1))
            })
      ...
      describe('when the group cannot receive votes', () => {
        beforeEach(async () => {
          await mockLockedPlanq.setTotalLockedPlanq(value.div(2).minus(1))
          await mockValidators.setMembers(group, [accounts[9]])
          await mockValidators.setNumRegisteredValidators(1)
    ...
    describe('when the voter has pending votes', () => {
      ...
      describe('when an epoch boundary has passed since the pending votes were made', () => {
        ...
        describe('when another voter activates votes', () => {
          ...
          const value2 = 573
          beforeEach(async () => {
            await mockLockedPlanq.incrementNonvotingAccountBalance(voter2, value2)
  ...
  describe('#revokePending', () => {
    ...
    describe('when the voter has pending votes', () => {
      ...
      describe('when the validator group has votes but is ineligible', () => {
        ...
        it("should increment the account's nonvoting locked planq balance", async () => {
          assertEqualBN(await mockLockedPlanq.nonvotingAccountBalance(voter), revokedValue)
        })
  ...
  describe('#revokeActive', () => {
    ...
    describe('when the voter has active votes', () => {
      ...
      beforeEach(async () => {
        ...
        await election.markGroupEligible(group, NULL_ADDRESS, NULL_ADDRESS)
        await registry.setAddressFor(PlanqContractName.Validators, mockValidators.address)
        await mockLockedPlanq.setTotalLockedPlanq(voteValue0 + voteValue1)
        await mockValidators.setNumRegisteredValidators(1)
        await mockLockedPlanq.incrementNonvotingAccountBalance(voter0, voteValue0)
        await mockLockedPlanq.incrementNonvotingAccountBalance(voter1, voteValue1)
      ...
      describe('when the validator group has votes but is ineligible', () => {
        ...
        it("should increment the account's nonvoting locked planq balance", async () => {
          assertEqualBN(await mockLockedPlanq.nonvotingAccountBalance(voter0), revokedValue)
        })
  ...
  describe('#electValidatorSigners', () => {
    ...
    totalLockedPlanq = voter1.weight + voter2.weight + voter3.weight
    ...
    beforeEach(async () => {
      ...
      await registry.setAddressFor(PlanqContractName.Random, random.address)
    })

    describe('when there is a large number of groups', () => {
      const numbers: any = {}
      beforeEach(async () => {
        await mockLockedPlanq.setTotalLockedPlanq(new BigNumber(1e25))
        await mockValidators.setNumRegisteredValidators(400)
        await mockLockedPlanq.incrementNonvotingAccountBalance(voter1.address, new BigNumber(1e25))
    ...
    describe('when there are some groups', () => {
      beforeEach(async () => {
        ...
        for (const voter of [voter1, voter2, voter3]) {
          await mockLockedPlanq.incrementNonvotingAccountBalance(voter.address, voter.weight)
        }
        await mockLockedPlanq.setTotalLockedPlanq(totalLockedPlanq)
      ...
      describe('when a group receives enough votes for > n seats but only has n members', () => {
        beforeEach(async () => {
          ...
          const increment = 80
          const votes = 80
          await mockLockedPlanq.incrementNonvotingAccountBalance(voter3.address, increment)
          await mockLockedPlanq.setTotalLockedPlanq(totalLockedPlanq + increment)
      ...
      describe('when a group does not receive `electabilityThresholdVotes', () => {
        beforeEach(async () => {
          const thresholdExcludingGroup3 = (voter3.weight + 1) / totalLockedPlanq
  ...
  describe('#getGroupEpochRewards', () => {
    ...
    beforeEach(async () => {
      ...
      await election.markGroupEligible(group2, NULL_ADDRESS, group1)
      await registry.setAddressFor(PlanqContractName.Validators, mockValidators.address)
      await mockLockedPlanq.setTotalLockedPlanq(voteValue1.plus(voteValue2))
      ...
      await mockLockedPlanq.incrementNonvotingAccountBalance(voter, voteValue1.plus(voteValue2))
    ...
    describe('when one group has active votes', () => {
      ...
      describe('when the group does not meet the locked planq requirements ', () => {
        beforeEach(async () => {
          await mockValidators.setDoesNotMeetAccountLockedPlanqRequirements(group1)
        })
    ...
    describe('when two groups have active votes', () => {
      ...
      describe('when one group does not meet the locked planq requirements ', () => {
        beforeEach(async () => {
          await mockValidators.setDoesNotMeetAccountLockedPlanqRequirements(group2)
        })
  ...
  describe('#distributeEpochRewards', () => {
    ...
    beforeEach(async () => {
      ...
      await election.markGroupEligible(group, NULL_ADDRESS, NULL_ADDRESS)
      await registry.setAddressFor(PlanqContractName.Validators, mockValidators.address)
      await mockLockedPlanq.setTotalLockedPlanq(voteValue)
      await mockValidators.setMembers(group, [accounts[9]])
      await mockValidators.setNumRegisteredValidators(1)
      await mockLockedPlanq.incrementNonvotingAccountBalance(voter, voteValue)
    ...
    describe('when there are two groups with active votes', () => {
      ...
      beforeEach(async () => {
        ...
        await registry.setAddressFor(PlanqContractName.Validators, mockValidators.address)
        await mockLockedPlanq.setTotalLockedPlanq(voteValue.plus(voteValue2))
        await mockValidators.setNumRegisteredValidators(2)
        await mockLockedPlanq.incrementNonvotingAccountBalance(voter2, voteValue2)
  ...
  describe('#forceDecrementVotes', () => {
    ...
    describe('when the account has voted for one group', () => {
      beforeEach(async () => {
        ...
        await mockLockedPlanq.incrementNonvotingAccountBalance(voter, value)
        await election.vote(group, value, NULL_ADDRESS, NULL_ADDRESS)
        await registry.setAddressFor(PlanqContractName.LockedPlanq, accounts[2])
      })

      describe('when the account only has pending votes', () => {
        describe('when the account is slashed for the total pending voted planq', () => {
          const index = 0
      ...
      describe('when the account only has active votes', () => {
        ...
        describe('when the account is slashed for the total active voting planq', () => {
          const index = 0
          const slashedValue = value
          const remaining = value - slashedValue
          beforeEach(async () => {
          ...
          it('should decrement active voted planq to zero', async () => {
            assertEqualBN(await election.getActiveVotesForGroupByAccount(group, voter), remaining)
          })

          it('should decrement total voted planq to zero', async () => {
            assertEqualBN(await election.getTotalVotesForGroupByAccount(group, voter), remaining)
            assertEqualBN(await election.getTotalVotesByAccount(voter), remaining)
            assertEqualBN(await election.getTotalVotesForGroup(group), remaining)
            assertEqualBN(await election.getTotalVotes(), remaining)
    ...
    describe('when the account has voted for more than one group equally', () => {
      ...
      describe('when the accounts only have pending votes', () => {
        describe('when both accounts are slashed for the total pending voted planq', () => {
          const slashedValue = value
          const remaining = value - slashedValue
    ...
    describe('when the account has voted for more than one group inequally', () => {
      ...
      beforeEach(async () => {
        ...
        await registry.setAddressFor(PlanqContractName.Validators, mockValidators.address)
        await mockLockedPlanq.setTotalLockedPlanq(value + value2)
        await mockValidators.setNumRegisteredValidators(2)
        await mockLockedPlanq.incrementNonvotingAccountBalance(voter, value + value2)
      ...
      describe('when `forceDecrementVotes` is called with malformed inputs', () => {
        ...
        describe('when called from an address other than the locked planq contract', () => {
          it('should revert', async () => {
            await assertTransactionRevertWithReason(
              election.forceDecrementVotes(
                voter,
  ...
  describe('#consistencyChecks', () => {
      ...
      if (debug) {
        ...
        debugLog(
          `Actual ${account.address}:\n\tnonvoting: ${(
            await mockLockedPlanq.nonvotingAccountBalance(account.address)
          ).toFixed()}\n\tpending: ${(
    ...
    const revokeAllAndCheckInvariants = async (delta: number = 0) => {
      const vAccounts = await Promise.all(
        voters.map(async (v) => {
          return {
            ...
            nonvoting: await mockLockedPlanq.nonvotingAccountBalance(v),
          }
        })
      )
      ...
      for (const account of vAccounts) {
        ...
        assertEqualBN(await mockLockedPlanq.nonvotingAccountBalance(address), account.nonvoting)
      }
    }

    let voterAccounts: Account[]
    beforeEach(async () => {
      // 50M gives us 450M total locked planq
      const voterStartBalance = new BigNumber(web3.utils.toWei('50000000'))
      await mockValidators.setMembers(group, [accounts[9]])
      await registry.setAddressFor(PlanqContractName.Validators, accounts[0])
      await election.markGroupEligible(group, NULL_ADDRESS, NULL_ADDRESS)
      await registry.setAddressFor(PlanqContractName.Validators, mockValidators.address)
      await mockLockedPlanq.setTotalLockedPlanq(voterStartBalance.times(voters.length))
      await mockValidators.setNumRegisteredValidators(1)
      await Promise.all(
        voters.map((voter) =>
          mockLockedPlanq.incrementNonvotingAccountBalance(voter, voterStartBalance)
        )
      )
