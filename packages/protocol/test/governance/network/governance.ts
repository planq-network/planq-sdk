import { NULL_ADDRESS, trimLeading0x } from '@planq-network/base/lib/address'
...
import { fixed1, multiply, toFixed } from '@planq-network/utils/lib/fixidity'
...
contract('Governance', (accounts: string[]) => {
  ...
  beforeEach(async () => {
    ...
    await mockLockedPlanq.setAccountTotalLockedPlanq(account, yesVotes)
    await mockLockedPlanq.setTotalLockedPlanq(yesVotes)
    transactionSuccess1 = {
      value: 0,
  ...
  describe('#upvote()', () => {
    ...
    describe('when the upvoted proposal is expired', () => {
      ...
      beforeEach(async () => {
        ...
        await mockLockedPlanq.setAccountTotalLockedPlanq(otherAccount1, yesVotes)
    ...
    for (let numVoted = 0; numVoted < 3; numVoted++) {
      describe(`when account has partially voted on ${numVoted} proposals`, () => {
        ...
        it('should return false on `isVoting`', async () => {
          await governance.revokeVotes()
          const voting = await governance.isVoting(accounts[0])
          const totalVotesByAccount = await governance.getAmountOfPlanqUsedForVoting(accounts[0])
          assert.isFalse(voting)
  ...
  describe('#vote()', () => {
    ...
    describe('when proposal is approved', () => {
      ...
      it('should revert when the account weight is 0', async () => {
        await mockLockedPlanq.setAccountTotalLockedPlanq(account, 0)
        await assertTransactionRevertWithReason(
    ...
    describe('When voting on different proposal with same index', () => {
      ...
      it('should ignore votes from previous proposal', async () => {
        ...
        const otherAccount1 = accounts[1]
        await accountsInstance.createAccount({ from: otherAccount1 })
        await mockLockedPlanq.setAccountTotalLockedPlanq(otherAccount1, otherAccountWeight)
  ...
  describe('#votePartially()', () => {
    ...
    describe('when proposal is approved', () => {
      ...
      it('should revert when the account does not have enough planq', async () => {
        await assertTransactionRevertWithReason(
      ...
      it('should revert when the account does not have enough planq when voting partially', async () => {
  ...
  describe('#hotfixWhitelistValidatorTally', () => {
    const newHotfixHash = bufferToHex(toBuffer(keccak256(utf8ToBytes('planq bug fix'))))
  ...
  describe('#isProposalPassing()', () => {
    ...
    describe('when the adjusted support is greater than threshold', () => {
      beforeEach(async () => {
        await mockLockedPlanq.setAccountTotalLockedPlanq(account, (yesVotes * 51) / 100)
        await mockLockedPlanq.setAccountTotalLockedPlanq(otherAccount, (yesVotes * 49) / 100)
        await governance.vote(proposalId, index, VoteValue.Yes)
    ...
    describe('when the adjusted support is less than or equal to threshold', () => {
      beforeEach(async () => {
        await mockLockedPlanq.setAccountTotalLockedPlanq(account, (yesVotes * 50) / 100)
        await mockLockedPlanq.setAccountTotalLockedPlanq(otherAccount, (yesVotes * 50) / 100)
        await governance.vote(proposalId, index, VoteValue.Yes)
        await governance.vote(proposalId, index, VoteValue.No, { from: otherAccount })
      })
  ...
  describe('#getAmountOfPlanqUsedForVoting()', () => {
