...
contract('BlockchainParameters', (accounts: string[]) => {
  ...
  const gasForNonPlanqCurrencies = 50000
  ...
  describe('#setIntrinsicGasForAlternativeFeeCurrency()', () => {
    it('should set the variable', async () => {
      await blockchainParameters.setIntrinsicGasForAlternativeFeeCurrency(gasForNonPlanqCurrencies)
      assertEqualBN(
        gasForNonPlanqCurrencies,
    ...
    it('should emit the corresponding event', async () => {
      ...
      assertContainSubset(log, {
        ...
        args: {
          gas: new BigNumber(gasForNonPlanqCurrencies),
        },
      })
    })
    it('only owner should be able to set', async () => {
  ...
  describe('#initialize()', () => {
    ...
    it('should set the variables', async () => {
      await blockchainParameters.initialize(gasForNonPlanqCurrencies, gasLimit, lookbackWindow)
      assert.equal((await blockchainParameters.blockGasLimit()).toNumber(), gasLimit)
