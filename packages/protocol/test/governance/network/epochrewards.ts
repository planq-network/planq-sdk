...
import { makeTruffleContractForMigration } from '@planq-network/protocol/lib/web3-utils'
...
contract('EpochRewards', (accounts: string[]) => {
  ...
  const targetVotingPlanqFraction = toFixed(new BigNumber(2 / 3))
  ...
  beforeEach(async () => {
    ...
    await registry.setAddressFor(PlanqContractName.SortedOracles, mockSortedOracles.address)
  ...
  describe('#setTargetVotingPlanqFraction()', () => {
    describe('when the fraction is different', () => {
      const newFraction = targetVotingPlanqFraction.plus(1)

      describe('when called by the owner', () => {
        it('should set the target voting planq fraction', async () => {
          await epochRewards.setTargetVotingPlanqFraction(newFraction)
          assertEqualBN(await epochRewards.getTargetVotingPlanqFraction(), newFraction)
        })

        it('should emit the TargetVotingPlanqFractionSet event', async () => {
          const resp = await epochRewards.setTargetVotingPlanqFraction(newFraction)
          assert.equal(resp.logs.length, 1)
          const log = resp.logs[0]
          assertContainSubset(log, {
            event: 'TargetVotingPlanqFractionSet',
            args: {
        ...
        describe('when called by a non-owner', () => {
          it('should revert', async () => {
            await assertTransactionRevertWithReason(
              epochRewards.setTargetVotingPlanqFraction(newFraction, {
                from: nonOwner,
              }),
      ...
      describe('when the fraction is the same', () => {
        it('should revert', async () => {
          await assertTransactionRevertWithReason(
            epochRewards.setTargetVotingPlanqFraction(targetVotingPlanqFraction),
  ...
  describe('#getTargetPlanqTotalSupply()', () => {
    describe('when it has been fewer than 15 years since genesis', () => {
      ...
      it('should return 600MM + 200MM * t / 15', async () => {
        assertEqualDpBN(
          await epochRewards.getTargetPlanqTotalSupply(),
  ...
  describe('#getTargetTotalEpochPaymentsInPlanq()', () => {
    describe('when a StableToken exchange rate is set', () => {
      ...
      it('should return the number of validators times the max payment divided by the exchange rate', async () => {
        const expected = targetValidatorEpochPayment
          ...
          .div(exchangeRate)
          .integerValue(BigNumber.ROUND_FLOOR)
        assertEqualBN(await epochRewards.getTargetTotalEpochPaymentsInPlanq(), expected)
      })
    })
  })
  ...
  describe('#getRewardsMultiplier()', () => {
    ...
    beforeEach(async () => {
      ...
      targetEpochReward = targetEpochReward.plus(
        await epochRewards.getTargetTotalEpochPaymentsInPlanq()
      )
    })

    describe('when the target supply is equal to the actual supply after rewards', () => {
      beforeEach(async () => {
        await mockPlanqToken.setTotalSupply(expectedTargetTotalSupply.minus(targetEpochReward))
        await timeTravelToDelta(timeDelta)
      })
    ...
    describe('when the actual remaining supply is 10% more than the target remaining supply after rewards', () => {
      beforeEach(async () => {
        ...
        const totalSupply = SUPPLY_CAP.minus(actualRemainingSupply)
          ...
          .integerValue(BigNumber.ROUND_FLOOR)
        await mockPlanqToken.setTotalSupply(totalSupply)
        await timeTravelToDelta(timeDelta)
      })
  ...
  describe('#updateTargetVotingYield()', () => {
    ...
    beforeEach(async () => {
      ...
      await registry.setAddressFor(PlanqContractName.Reserve, reserve.address)
    ...
    describe('when the percentage of voting planq is equal to the target', () => {
      beforeEach(async () => {
        const totalVotes = floatingSupply
          .times(fromFixed(targetVotingPlanqFraction))
          .integerValue(BigNumber.ROUND_FLOOR)
    ...
    describe('when the percentage of voting planq is 10% less than the target', () => {
      beforeEach(async () => {
        const totalVotes = floatingSupply
          .times(fromFixed(targetVotingPlanqFraction).minus(0.1))
          .integerValue(BigNumber.ROUND_FLOOR)
        await mockElection.setTotalVotes(totalVotes)
        await epochRewards.updateTargetVotingYield()
      })
    ...
    describe('when the percentage of voting planq is 10% more than the target', () => {
      beforeEach(async () => {
        const totalVotes = floatingSupply
          .times(fromFixed(targetVotingPlanqFraction).plus(0.1))
          .integerValue(BigNumber.ROUND_FLOOR)
        await mockElection.setTotalVotes(totalVotes)
        await epochRewards.updateTargetVotingYield()
      })
    ...
    describe('when the percentage of voting planq is 0%', () => {
      beforeEach(async () => {
      ...
      it('should increase the target voting yield by the target voting planq percentage times adjustment factor', async () => {
        const expected = fromFixed(
          targetVotingYieldParams.initial.plus(
            targetVotingYieldParams.adjustmentFactor.times(fromFixed(targetVotingPlanqFraction))
          )
        )
        const actual = fromFixed((await epochRewards.getTargetVotingYieldParameters())[0])
        // Assert equal to 9 decimal places due to fixidity imprecision.
    ...
    describe('when the percentage of voting planq is 30%', () => {
      beforeEach(async () => {
      ...
      it('should decrease the target voting yield by |votingFraction-target voting planq percentage| times adjustment factor', async () => {
        const votingFraction = 0.3
        const expected = fromFixed(
          targetVotingYieldParams.initial.plus(
            targetVotingYieldParams.adjustmentFactor.times(
              fromFixed(targetVotingPlanqFraction.minus(toFixed(votingFraction)))
            )
          )
        )
        const actual = fromFixed((await epochRewards.getTargetVotingYieldParameters())[0])
    ...
    describe('when the percentage of voting planq is 90%', () => {
      beforeEach(async () => {
    ...
    describe('when the percentage of voting planq is 100%', () => {
      beforeEach(async () => {
      ...
      it('should decrease the target voting yield by (100 - target voting planq percentage) times adjustment factor', async () => {
        const votingFraction = 1.0
        const expected = fromFixed(
          targetVotingYieldParams.initial.plus(
            targetVotingYieldParams.adjustmentFactor.times(
    ...
    describe('When voting fraction remains below target 5 epochs in a row', () => {
      ...
      it('target voting yield should be increased 5 times as expected', async () => {
        const expected = fromFixed(
          targetVotingYieldParams.initial.plus(
            targetVotingYieldParams.adjustmentFactor
              .times(fromFixed(targetVotingPlanqFraction).minus(0.3))
      ...
      it('target voting yield should be decreased 5 times as expected', async () => {
        ...
        const expected = fromFixed(
          targetVotingYieldParams.initial.minus(
            targetVotingYieldParams.adjustmentFactor
              .times(fromFixed(votingFraction).minus(fromFixed(targetVotingPlanqFraction)))
              .times(fromFixed(toFixed(5)))
          )
        )
        const actual = fromFixed((await epochRewards.getTargetVotingYieldParameters())[0])
      ...
      it('target voting yield should be adjusted as expected', async () => {
        ...
        for (const votingFractionElement of votingFractionArray) {
          expected = expected.plus(
            targetVotingYieldParams.adjustmentFactor.times(
              fromFixed(targetVotingPlanqFraction.minus(toFixed(votingFractionElement)))
            )
          )
        }
  ...
  describe('#calculateTargetEpochRewards()', () => {
    describe('when there are active votes, a stable token exchange rate is set and the actual remaining supply is 10% more than the target remaining supply after rewards', () => {
      ...
      let expectedTargetPlanqSupplyIncrease: BigNumber
      beforeEach(async () => {
        await epochRewards.setNumberValidatorsInCurrentSet(numberValidators)
        await mockElection.setActiveVotes(activeVotes)
        const expectedTargetTotalEpochPaymentsInPlanq = targetValidatorEpochPayment
          .times(numberValidators)
          .div(exchangeRate)
          .integerValue(BigNumber.ROUND_FLOOR)
        ...
        expectedTargetPlanqSupplyIncrease = expectedTargetEpochRewards
          .plus(expectedTargetTotalEpochPaymentsInPlanq)
          .div(
            new BigNumber(1)
        ...
        const actualRemainingSupply = expectedTargetRemainingSupply.times(1.1)
        const totalSupply = SUPPLY_CAP.minus(actualRemainingSupply)
          .minus(expectedTargetPlanqSupplyIncrease)
          .integerValue(BigNumber.ROUND_FLOOR)
        await mockPlanqToken.setTotalSupply(totalSupply)
  ...
  describe('#isReserveLow', () => {
    ...
    describe('reserve ratio of 0.5', () => {
      beforeEach(async () => {
        ...
        const planqBalance = stableBalance.div(exchangeRate).div(2).times(0.5).integerValue()
    ...
    describe('reserve ratio of 1.5', () => {
      beforeEach(async () => {
        const stableBalance = new BigNumber(2397846127684712867321)
        const planqBalance = stableBalance.div(exchangeRate).div(2).times(1.5).integerValue()
        await mockStableToken.setTotalSupply(stableBalance)
        await web3.eth.sendTransaction({
          from: accounts[9],
          to: reserve.address,
    ...
    describe('reserve ratio of 2.5', () => {
      beforeEach(async () => {
        const stableBalance = new BigNumber(2397846127684712867321)
        const planqBalance = stableBalance.div(exchangeRate).div(2).times(2.5).integerValue()
        await mockStableToken.setTotalSupply(stableBalance)
        await web3.eth.sendTransaction({
          from: accounts[9],
          to: reserve.address,
