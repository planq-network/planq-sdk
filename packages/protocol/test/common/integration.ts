...
async function findLessersAndGreaters(
  ...
  const changed = await slashingOfGroups(account, penalty, lockedPlanq, election)
...
contract('Integration: Governance slashing', (accounts: string[]) => {
  ...
  describe('When performing slashing', () => {
    before(async () => {
      await timeTravel(config.governance.referendumStageDuration, web3)
      valueOfSlashed = await lockedPlanq.getAccountTotalLockedPlanq(slashedAccount)
...
Array.from([
  ...
  ['ExchangeBRL', 'StableTokenBRL'], // BRL (pREAL)
  ...
  contract(`Integration: ${exchangeId} ${stableTokenId}`, (accounts: string[]) => {
    ...
    describe('Buying', () => {
      ...
      // Note that this test relies on having purchased pUSD in a previous test
    ...
    describe('When transferring planq', () => {
      ...
      beforeEach(async () => {
        originalReserve = await planqToken.balanceOf(reserve.address)
        originalOtherAccount = await planqToken.balanceOf(otherReserveAddress)
      })
...
contract('Integration: Adding StableToken', (accounts: string[]) => {
  ...
  before(async () => {
    ...
    await assumeOwnershipWithTruffle(['Reserve'], accounts[0], 0, ASTONIC_PACKAGE)
  })
  ...
  describe('When the contracts have been deployed and initialized', () => {
    before(async () => {
      ...
      await exchangeAbc.initialize(
        registry.address,
        'StableTokenABC',
        '5000000000000000000000', // spread, matches mainnet for pUSD and pEUR
        '1300000000000000000000', // reserveFraction, matches mainnet for pEUR
        '300', // updateFrequency, matches mainnet for pUSD and pEUR
