import { recoverFunds } from '@planq-network/protocol/lib/recover-funds'
...
contract('Proxy', (accounts: string[]) => {
  ...
  it('recovers funds from an incorrectly intialized implementation', async () => {
    ...
    const planqToken = await PlanqToken.new(true)
    ...
    const initialBalance = new BigNumber(await planqToken.balanceOf(owner))
    await planqToken.transfer(proxy.address, amount)

    await proxy._setImplementation(getSet.address)

    assert((await planqToken.balanceOf(owner)).eq(initialBalance.minus(amount)))
    ...
    assert((await planqToken.balanceOf(owner)).eq(initialBalance))
  })
})
