...
import { fundAccountWithpUSD, getKey, ONE, TestAccounts } from '../scaffold'

export function runOracleTest(context: EnvTestContext) {
  describe('Oracle Test', () => {
    const logger = context.logger.child({ test: 'exchange' })
    beforeAll(async () => {
      await fundAccountWithpUSD(context, TestAccounts.Exchange, ONE.times(2))
    })
    ...
    test('report a rate', async () => {
      ...
      const isOracle = await oracles.isOracle(PlanqContract.StableToken, from.address)
      ...
      if (!ourRate) {
        const currentMedianRate = await oracles.medianRate(PlanqContract.StableToken)
      ...
      const reportTx = await oracles.report(PlanqContract.StableToken, rateToReport, from.address)
      ...
      const newOracleRates = await oracles.getReports(PlanqContract.StableToken)
