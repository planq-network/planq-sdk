...
export function runReserveTest(context: EnvTestContext) {
  describe("Reserve Test", () => {
    ...
    beforeAll(async () => {
      await fundAccountWithaUSD(
        context,
        TestAccounts.ReserveSpender,
        ONE.times(2)
      );
      await fundAccountWithaUSD(
        context,
    ...
    test("move funds from the Reserve to a custodian and back", async () => {
      const spender = await getKey(
        context.mnemonic,
        TestAccounts.ReserveSpender
      );
      const custodian = await getKey(
        context.mnemonic,
        TestAccounts.ReserveCustodian
      );
      ...
      if (!spenderMultiSigAddress) {
        ...
        context.logger.debug(
          {spenderMultiSigAddress},
          "got reserve spender address"
        );
      }
      ...
      const spenderMultiSig = await context.kit.contracts.getMultiSig(
        spenderMultiSigAddress
      );
      const isOwner = await spenderMultiSig.isowner(spender.address);
      expect(isOwner).toBeTruthy();

      const reserveValue = await reserve.getReservePlanqBalance();
      ...
      const spenderTx = reserve.transferPlanq(
        custodian.address,
      ...
      logger.debug(
        {receipt: multiSigTxReceipt},
        "funds moved to custodian via spender"
      );

      const returnTx = planqToken.transfer(
        reserve.address,
        valueToTransfer.toString()
      );
      const returnTxReceipt = await returnTx.sendAndWaitForReceipt({
