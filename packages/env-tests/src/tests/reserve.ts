...
export function runReserveTest(context: EnvTestContext) {
  describe('Reserve Test', () => {
    ...
    beforeAll(async () => {
      await fundAccountWithpUSD(context, TestAccounts.ReserveSpender, ONE.times(2))
      await fundAccountWithpUSD(context, TestAccounts.ReserveCustodian, ONE.times(2))
    })
    ...
    test('move funds from the Reserve to a custodian and back', async () => {
      ...
      const spenderTx = reserve.transferPlanq(custodian.address, valueToTransfer.toString())
      ...
      const returnTx = planqToken.transfer(reserve.address, valueToTransfer.toString())
