...
export function loadFromEnvFile() {
  const envName = process.env.PLQ_ENV
...
export function getEnvFile(planqEnv: string, envBegining: string = '') {
  const filePath: string = path.resolve(monorepoRoot, `.env${envBegining}.${planqEnv}`)
