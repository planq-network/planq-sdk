...
export async function fundAccountWithaUSD(
  context: EnvTestContext,
  account: TestAccounts,
  value: BigNumber
) {
  await fundAccountWithStableToken(context, account, value, StableToken.aUSD);
}
...
async function fundAccount(
  ...
  if (rootBalance.lte(value)) {
    logger.error(
      ...
      "Error funding test account"
    );
    throw new Error(
      `Root account ${
        root.address
      }'s ${token} balance (${rootBalance.toPrecision(
        4
...
export async function getValidatorKey(
  mnemonic: string,
  index: number
): Promise<KeyInfo> {
...
export async function clearAllFundsToRoot(
  ...
  context.logger.debug(
    {root: root.address},
    "Clearing funds of test accounts back to root"
  );
  ...
  await concurrentMap(5, accounts, async (_val, index) => {
    ...
    for (const stableToken of stableTokensToClear) {
      const stableTokenInstance = await context.kit.planqTokens.getWrapper(
        stableToken
      );
      ...
      if (balance.gt(maxBalanceBeforeCollecting)) {
        ...
        const balanceAfter = await stableTokenInstance.balanceOf(
          account.address
        );
        context.logger.debug(
          {
