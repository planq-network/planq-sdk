...
describe('BinanceAdapter', () => {
  ...
  describe('parseTicker', () => {
    ...
    it('handles a response that matches the documentation', () => {
      expect(binanceAdapter.parseTicker(tickerJson)).toEqual({
        source: Exchange.BINANCE,
        symbol: binanceAdapter.standardPairSymbol,
    ...
    for (const field of Object.keys(tickerJson)) {
      ...
      if (requiredFields.includes(field)) {
        it(`throws an error if ${field} is missing`, () => {
          expect(() => {
            binanceAdapter.parseTicker(incompleteTickerJson)
          }).toThrowError()
        })
      } else {
  ...
  describe('isOrderbookLive', () => {
    ...
    it('returns false if the symbol is not found', async () => {
      jest.spyOn(binanceAdapter, 'fetchFromApi').mockReturnValue(
        Promise.resolve({
          ...mockStatusJson,
    ...
    it('returns true if symbol is found, status === TRADING, isSpotTradingAllowed is true and orderTypes contains both LIMIT and MARKET', async () => {
      jest.spyOn(binanceAdapter, 'fetchFromApi').mockReturnValue(Promise.resolve(mockStatusJson))
